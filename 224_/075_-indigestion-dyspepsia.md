...
graph TD
    ...
    B1 -- "< 30분 발생" --> C1["<u>Gastric bloating</u><br/>기능성 소화불량<br/>Accommodation 이상<br/>Gastric outlet 폐쇄"]
    B1 -- "> 30분 발생" --> D1["<u>Small bowel bloating</u><br/>IBS<br/>Celiac Dz<br/>식이<br/>소장 세균 과증식"]
...
```mermaid
graph TD
    Start([소화불량]) --> RF["Red Flag<br/>또는 ≥ 55세?<br/>(국내: ≥40세)"]
    RF -- "Yes" --> Endo["내시경 검사"]
    ...
    HpTest -- "양성" --> HpErad["H. pylori 제균"]
    HpTest -- "음성" --> Pheno["증상 아형 분류"]
    HpErad -- "증상 소실" --> Done["호전"]
    HpErad -- "증상 지속" --> Pheno

    Pheno --> EPS["<u>EPS</u><br/>상복부 통증/작열감"]
    Pheno --> PDS["<u>PDS</u><br/>식후 팽만감/조기 포만감"]
    Pheno --> Mix["<u>혼합형</u><br/>EPS + PDS"]
    ...
    PPI -- "반응(-)" --> TCA["<u>저용량 TCA</u><br/>amitriptyline 10~25 mg hs"]
    Prokin -- "반응(-)" --> TCA
    Both -- "반응(-)" --> TCA
    Both -- "반응(+)" --> Done
    TCA -- "반응(+)" --> Done
