...
* 대량·지속되는 혈변/직장 출혈 또는 실신·빈맥·저혈압 등 혈역학적 불안정
...
* 설명할 수 없는 발열
* 복부 종괴 또는 림프절 종대
* 원인 불명의 육안적 혈변·직장 출혈(혈역학적으로 안정)
* 설명할 수 없는 체중 감소 또는 철결핍빈혈
* ≥50세에서 처음 발생한 IBS 유사 증상

<mark style="color:$info;">**외래 추적 / 추가 평가 계획**</mark> <mark style="color:$info;">- 즉각 위험 낮으나 호전 없으면 의뢰</mark>

* 대장암·IBD·폴립증·셀리악병의 개인력 또는 가족력
* 흡수 장애 또는 지방변
* 표준 치료를 충분히 시행했는데도 지속되거나 양상이 현저히 변한 증상 → 진단·아형·동반 질환 재평가
...
* 증상이 발생한 지 최소 6개월 되었고 최근 3개월간 다음 기준을 충족; 다음 중 ≥2개와 관련되는, 평균 월 ≥3일 발생하는 재발성 복통 또는 복부 불편감
  1. 배변과 관련
  2. 배변 빈도의 변화와 관련
  3. 대변 형태(모양)의 변화와 관련

{% hint style="info" %}
**Rome IV와의 관계** : 2026년 Rome V에서는 복부 불편감이 다시 포함되고 증상 빈도 기준이 평균 주 1일 이상에서 월 3일 이상으로 완화되었다. 2025 서울 컨센서스와 기존 치료 연구는 대부분 Rome IV를 기반으로 하므로, 근거를 해석할 때 적용한 진단기준을 함께 확인한다.
{% endhint %}
...
* 모든 환자에게 동일한 검사 패널을 시행하지 않고 증상 아형·연령·경고징후·노출력에 따라 선택
  * 기본 검사 : CBC ± CRP/ESR
  * IBS-D·IBS-M : fecal calprotectin 또는 lactoferrin을 고려하고, 셀리악병 배제가 필요하면 tissue transglutaminase IgA와 total IgA를 함께 측정
...
* GI 기능 평가 (선택적 시행)
  ...
  * 호흡검사 : lactose breath test는 유당흡수장애, glucose breath test는 SIBO가 의심될 때 선택적으로 시행
...
graph TD
    ...
    Step1["병력·신체검사<br>Rome V 기준 + 적응증별 제한적 검사<br>IBS-D/M: ± fecal calprotectin"]
    Cond1("경고 증상 또는<br>대장암 선별검사 필요?")
    Sub1["Positive Diagnosis 가능<br>• 설명 및 안심<br>• 식이·생활습관 조절<br>• 증상 군집별 약물 (4주)"]
    Cond2("증상 지속?")
    Colonoscopy["경고징후별 표적 평가<br>± 대장내시경·영상검사"]
    Cond3("기질적 이상?")
    NonIBS["Non-IBS:<br>IBD, BAD, 대장암 등"]
    Reassess["복약순응도·용량·아형 변화<br>BAD·현미경적 대장염·배출장애<br>동반 DGBI 재평가"]
    End1["유지 치료·추적 관찰"]
    Treat1["중증도·아형별 약물 조정 (2~4주 간격)<br>• IBS-C: psyllium → PEG → 변비 표적약제<br>• IBS-D: loperamide → ramosetron<br>• 공통: 진경제·페퍼민트 오일·TCA<br>• 허가 외 약물은 별도 설명·절차"]
...
class Cond3,Cond1,Cond2 yellow
classDef sky fill:#e3f2ff,stroke:#2196f3
class Sub1,Treat1,Treat2 sky
classDef L_red fill:#ffcdd2,stroke:#c62828
class Refractory L_red
...
graph TD
    ...
    P1["<u>1단계: 제한기 (3~6주)*</u><br>고FODMAP 식품 제한<br>음식 일기 작성·증상 기록"]
    Cond1("증상 호전?")
    P1_Fail["기질적 질환 재평가<br>또는 다른 유발 인자 탐색<br>(BAD, SIBO, 스트레스 등)"]
    P2["<u>2단계: 단계적 재도입기 (3~6주)</u><br>FODMAP 군별 1가지씩 3일간 시험<br>증상 발현 시 즉시 중단 후 재시도"]
    P3["<u>3단계: 맞춤형 식단 안착기 (장기)</u><br>내성 확인 식품 재도입<br>유발 식품만 최소 제한<br>영양 균형 유지"]
    End2["지속 가능한 삶의 질 향상"]
...
classDef blue fill:#d0e8ff,stroke:#1a6abf
class End2 blue
style P1_Fail fill:#d5f5e3,stroke:#27ae60
```

<p align="center"><strong>Low-FODMAP 3단계 식이 접근</strong> (ACG 2021 조건부 권고)</p>

\*제한기는 3\~6주로 제한; 이후 단계적 재도입으로 개인별 불내성 식품 특정. 가능하면 숙련된 영양사의 지도를 받고, 장기간의 엄격한 제한식은 피한다. (☞ [저FODMAP 식품 목록](075_-indigestion-dyspepsia.md#undefined-19) )
...
**2025 서울 컨센서스 중심 주요 권고**

<table><thead><tr><th width="96">대상</th><th>치료</th><th width="184">근거 수준 / 권고 강도</th></tr></thead><tbody><tr><td><strong>IBS-C</strong></td><td>수용성 섬유, PEG, 난치성 변비에서 prucalopride 고려, lubiprostone, GC-C 작용제</td><td>GC-C 작용제: 높음/강함<br>그 외: 낮음~중등도/약함</td></tr><tr><td><strong>IBS-D</strong></td><td>loperamide, 5-HT₃ 길항제(ramosetron), rifaximin</td><td>5-HT₃ 길항제: 높음/약함<br>그 외: 매우 낮음~중등도/약함</td></tr><tr><td><strong>공통</strong></td><td>진경제, TCA, 선택적 SSRI, probiotics, low-FODMAP 식이, 장-지향 심리치료</td><td>대부분 매우 낮음~중등도/약함</td></tr></tbody></table>

{% hint style="info" %}
국내 가용성·허가 상태가 국제 지침과 다르다. Ramosetron은 국내 IBS-D에 사용할 수 있지만, rifaximin의 IBS-D 사용과 prucalopride·lubiprostone의 IBS-C 사용은 허가·급여 범위를 확인해야 한다. 2025 서울 컨센서스 당시 linaclotide·plecanatide 등 GC-C 작용제는 국내에서 이용할 수 없었다. Eluxadoline·alosetron·tegaserod 역시 국내 치료 선택지가 아니다.
{% endhint %}
...
* 항무스카린성 약물의 부작용 : 입마름, 시각 장애, 어지럼, 소변 저류, 변비, 빈맥; 고령·변비·녹내장·전립선 비대 환자에서 주의
* 매 식전 또는 필요시 투여
...
* \[AGA 2020] 임상시험 외 routine 사용에 반대; \[BSG 2021·2025 서울 컨센서스] 근거의 불확실성을 설명한 뒤 최대 12주 시험적으로 사용할 수 있으며 호전이 없으면 중단
...
* 용량 : 5\~10 ㎎ hs 시작 → 1\~2주 간격으로 5\~10 ㎎씩 점진적 증량; 보통 10\~30 ㎎/d 범위에서 최소 유효용량 유지
...
* prucalopride : 1\~2 ㎎ qd <mark style="color:blue;">\[레졸로]</mark> ([만성변비 허가](https://www.hira.or.kr/rc/insu/insuadtcrtr/InsuAdtCrtrPopup.do?mtgHmeDd=20200501\&sno=3\&mtgMtrRegSno=0003)); IBS-C 적응증 및 IBS-C 대상 RCT는 없음
...
* linaclotide : 성인 IBS-C 290 ㎍ qd (식사 30분 전); 145 ㎍은 성인 만성 특발성 변비 용량
* plecanatide : 3 ㎎ qd
...
> ```
>
> _✽ 진경제(dicyclomine)는 식전 30분 또는 복통 예상 상황 전 복용; 입마름·변비 부작용 확인. 묽은 변·절박변 조절이 필요하면 loperamide를 필요 시 추가하되 복통 자체의 개선 효과는 제한적임._
...
> ```
>
> _✽ Amitriptyline은 정신과적 목적이 아닌 내장 과민성·통증·설사 조절 목적의 중추성 신경조절제로서 저용량 사용임을 환자에게 설명. 약 4주에 내약성과 초기 반응을 확인하고, 견딜 수 있으면 5\~10 ㎎씩 점진적으로 증량하여 8\~12주까지 효과를 평가. 변비·입마름·주간 졸림을 모니터링._
...
> ```
>
> _✽ Ramosetron(이리보)은 변비로 전환되는지 확인하고 용량을 조절. 갑자기 심해지는 복통·혈변이 있으면 즉시 중단하고 허혈성 대장염을 평가. Rifaximin은 국내 IBS-D 허가·급여 대상이 아니고 국내 제형이 미국 허가 용법과 일치하지 않아 routine 병용 처방례에서 제외._
...
* **1단계 (3\~6주)** : 증상을 유발할 수 있는 고FODMAP 식품(마늘·양파·밀 제품·유제품·특정 과일 등)을 제한하며 음식 일기를 작성하세요.
