...
* MetALD(대사이상 동반 알코올 관련 간질환) : 지방간 및/또는 유의 섬유화(F2\~4) + 심대사 위험인자 ≥1개 + 음주량 남 30\~60 g/d, 여 20\~50 g/d에 해당하는 경우 - MASLD보다 섬유화 진행이 훨씬 빠름
...
* [알코올 사용 장애 선별](../230_/189_-alcohol-use-disorder-aud.md#undefined-3) : AUDIT(Alcohol Use Disorders Identification Test) 또는 AUDIT-C 사용 권장 - 구조화되고 비판단적인 방식으로 접근
  ...
  * 전체 AUDIT : ＜8점 저위험(추가 조치 불요) / 8\~15점 1차 진료 수준의 단순 개입(brief intervention) / 16\~20점 전문의뢰 고려 / ≥20점 중독전문의 의뢰(AUD 확진을 위한 DSM-5 기준 평가)
* 알코올 섭취는 낙인·문화적 이유로 과소 보고가 매우 흔함 - 자가보고만으로는 부정확할 수 있어 바이오마커 병행 권장
* 최근 음주력 확인 바이오마커 : phosphatidylethanol(PEth, 혈액) - 최근 3\~4주 이내 음주를 높은 정확도로 검출하며 현재 가이드라인이 1차 보조 검사로 권장(단, 섭취량의 정밀 정량에는 한계); ethyl glucuronide(EtG, 소변)는 최근 5일 이내 음주 검출에 유용; 이 외 EtS, CDT도 활용 가능 - ACG 2024 권고\
...
* 알코올 사용 장애(AUD) 치료 통합 : 행동 치료(동기 강화 면담) + 약물 치료 병행; 금주 유지가 장기 예후 결정 - 9,635명 코호트 연구에서 AUD 약물치료는 간경변 환자의 실대상성(decompensation) 위험을 유의하게 낮춤(보정 OR 0.35)
  * naltrexone : FDA 승인 1차 선택제(보정 OR 0.27); μ-opioid 수용체 길항제; 간독성 모니터링(ALT/AST) 병행 권장 - compensated cirrhosis에서는 신중 사용 가능하나, decompensated cirrhosis·알코올 간염에서는 일반적으로 피하거나 매우 신중히 사용
  ...
  * gabapentin : 오프라벨; 위 코호트에서 실대상성 위험 감소와 연관(보정 OR 0.36) - 신중한 대안으로 고려 가능
  * semaglutide(GLP-1 수용체 작용제) : 비만 동반 AUD 환자 대상 2026년 RCT(Lancet)에서 폭음일 수 유의하게 감소(위약 대비 −13.7%p) - 아직 AUD 적응증 미승인, 신흥 옵션으로 향후 근거 축적 필요
* 약물 주의
...
* Routine use 권고되지 않음 : STOPAH 대규모 RCT에서 pentoxifylline 단독 투여는 알코올 간염 사망률을 개선하지 못함. ACG 2024는 corticosteroid 절대 금기 시에만 대체를 남겨두고 있으며, 현재 임상 현장에서의 실제 사용 빈도는 매우 낮음
...
* MELD 3.0 score(빌리루빈, 크레아티닌, INR, 나트륨, 알부민, 성별 반영) : 10 이상이면 이식 평가 의뢰 고려
* 기존 기준 : 6개월 금주 후 간이식 (전통적 기준)
* 중증 알코올 간염으로 steroid 비반응 시, 6개월 금주 기준 충족 없이도 조기 간이식 고려 가능 - 2024년 설문에서 미국 센터의 97%가 엄선된 환자에게 조기 간이식을 제공한다고 보고
