...
* 다른 질환을 배제하여 진단
* 상부 위장관 경고 징후가 없고 성가신 상복부 통증이나 속쓰림, 조기 포만 &/or 식후 팽만 상태가 ＞8주 지속되면 FD로 진단

### <mark style="color:orange;">Test-and-Treat 전략</mark>

* 내시경부터 하지 말고, H. pylori 검사를 먼저 해서 양성이면 제균, 음성이면 경험적 약물 치료. 그래도 안 나으면 그때 내시경; 경고 증상이 없는 환자에서의 초기 접근 방법&#x20;
* 적용 연령 : 55세(서구 기반), 한국인은 국가암검진 프로그램에 근거하여 40세로 설정

1. H. pylori 검사 우선 (요소 호기 검사 또는 대변 항원 검사)
2. H. pylori 양성 시 제균 치료 시행 → 제균 성공 후 6\~12개월 이상 증상 지속 시 FD로 확진
...
* 내시경 검사가 일률적으로 필요하지는 않으나 중증 또는 경고 증상이 있는 경우에는 고려해야 함 (☞ [위장질환의 감별](074_.md#undefined-5))
* PDS와 위무력증(gastroparesis)은 증상이 겹칠 수 있음; 반복적인 구역·구토가 심한 경우 위 배출 신티그래피(gastric emptying scintigraphy)를 통한 감별을 고려
...
* H. pylori 양성 소화불량 환자에서는 먼저 제균 치료를 시행한다. 제균 성공 후에도 증상이 남는다면, 그때서야 비로소 기능성 소화불량(FD)으로 진단 가능
* 반대로, 제균 후 6\~12개월 이상 증상이 지속적으로 호전되면 이를 H. pylori-associated dyspepsia로 간주하며 FD와 구별. 이 6\~12개월 기준은 임상적 즉시 의사결정 시점이 아닌, 장기적 예후 판단 기준임에 주의
...
²⁾ 기초 검사\
\- H. pylori 검사 (breath/stool)\
\- ≥55세: CBC, (IBS 동반 시) celiac serology

³⁾ 필요시 추가 검사\
\- 비긴급 내시경: 치료 불응 또는 ≥55세(N/V, Plt 상승)\
...
* 초기부터 기능성 소화불량 아형 즉, phenotype에 따라 약제를 선택;  EPS - PPI, PDS - prokinetics 우선, 두 아형은 중복될 수 있으며 GERD overlap도 흔함
...
#### <mark style="color:$primary;">FD Phenotype-Driven Treatment - 아형 기반 치료 선택</mark>
...
* 난치성 overlap 시 저용량 TCA + CBT/behavioral therapy 고려
...
{% hint style="danger" %}
**국내 P-CAB 급여 주의** : P-CAB 제제의 국내 주요 급여 적응증은 GERD, 위궤양, H. pylori 제균 치료임. FD 단독으로는 급여 처리가 어려울 수 있으며, FD-GERD overlap 또는 PPI 불응 사례에서 활용 시 급여 기준을 반드시 확인
{% endhint %}
...
* acotiamide : 100 ㎎ tid ×4wk; M1/M2 무스카린 수용체 차단 및 cholinesterase 억제 기전으로 위 배출·운동 촉진; PDS에서 위약 대비 유의한 증상 제거 입증(RCT); 삶의 질 및 장기 안전성 확인됨 \[Second Asian Consensus 2025]; 특히 식사 관련(meal-related) PDS에서 효과 기대; 보험주의
...
* 특정 균주에서 일부 증상 개선 가능성이 보고되나 균주 간 결과가 일관되지 않으며 현재 연구 단계에 머물고 있음; routine use는 권고되지 않음 \[Second Asian Consensus 2025]
...
<table><thead><tr><th width="162">우세 증상</th><th width="188">가장 먼저 의심</th><th>우선 처방</th></tr></thead><tbody><tr><td>식후 더부룩함·<br>팽만감</td><td>PDS dominant</td><td>prokinetic (mosapride, acotiamide, corydaline)</td></tr><tr><td>조기 포만감</td><td>Accommodation 장애</td><td>buspirone / tandospirone ± acotiamide 병용</td></tr><tr><td>상복부 쓰림/작열감</td><td>EPS dominant</td><td>PPI / P-CAB ± mucosal protective agent</td></tr><tr><td>공복 시 상복부 통증</td><td>Acid hypersensitivity</td><td>PPI + mucosal protective, 재발 시 TCA 고려</td></tr><tr><td>식후 메스꺼움</td><td>Nausea dominant</td><td>prokinetic (metoclopramide 단기 가능)</td></tr><tr><td>스트레스 악화</td><td>Gut-brain amplification</td><td>TCA (amitriptyline) + CBT / 이완요법</td></tr><tr><td>역류 + 더부룩함 동반</td><td>FD-GERD overlap</td><td>PPI + alginate ± prokinetic</td></tr></tbody></table>
...
> ```
>
> _✽불안·우울 동반 또는 1차 약제 단독으로 반응이 없는 경우 저용량 TCA 병용을 고려. 항우울제가 아닌 gut-brain neuromodulator로 사용함을 환자에게 설명. **고령자는 5\~10 ㎎부터 시작**하여 2_~~_4주 간격으로 재평가 후 필요 시 증량 (일반 성인 최대 25_~~_50 ㎎)._
