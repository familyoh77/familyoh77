...
* 전파 경로 : 주로 분변-경구 경로(오염된 물/음식); 구강-경구 경로 가능성도 있음; 가족 내 전파가 가능하여 위암·소화성 궤양 환자의 1차 친족에서 높은 감염률을 보임
...
조기에 제균할수록 cascade 차단 효과가 큼.  위축성 위염 이전 단계에서 제균하면 암 예방 효과가 최대화됨. 위암 고위험 군(위암 가족력, 위 선종 절제 후 등)에서 조기 제균이 특히 중요
{% endhint %}
...
* 위암 고위험 지역 거주(우리나라 포함) - 만성 위염 환자에서도 제균 고려&#x20;
* 기능성 소화불량증
* 양성 위선종 내시경 절제 후

**검사 제외 또는 이익이 불분명한 경우**

* 소화성 궤양 병력 없는 전형적인 GERD
* 미란성 식도염, 바렛 식도, 식도선암 동반 시 (일부 연구에서 _H. pylor&#x69;_&#xC640;의 역상관 관계 보고; 제균 치료의 경과에 미치는 영향은 전반적으로 제한적)
...
* **검사 전 휴약** : 위음성 방지를 위해 비침습 검사(UBT, 대변 항원) 시행 전 휴약필요; 항생제·bismuth 종료 후 4주, PPI 종료 후 2주, H2 차단제 종료 후 1주 이후 검사
  * 혈청 항체 검사는 약제의 영향을 받지 않지만, 항체가 치료 후 수개월 이상 유지되므로 추적 검사로 사용하지 않음
* **상부위장관 출혈(UGIB) 급성기 검사 주의** : 활동성 출혈 시 위 내 혈액이 RUT·조직 검사의 민감도를 현저히 낮춰 위음성을 초래할 수 있음; 출혈 호전 후 UBT 또는 내시경으로 재검
...
flowchart TD
    ...
    L -- 제균 성공 --> M[제균 완료]
    L -- 제균 실패 --> N[이전 치료 방법?]
    N --> S["Bismuth 4제 <br/>치료"] --> O[Levofloxacin 3제 14일<br/>또는 Rifabutin 3제 고려]
    N --> T["3제·동시·순차<br/>치료"] --> K
    O --> P[재검]
    P -- 제균성공 --> M[제균 완료]
...
* 기존 적응증 : 소화성 궤양(활동성·과거력), 위 MALT 림프종, 조기 위암 내시경 절제 후, 위암 가족력(1차 친족), 특발성 혈소판감소성 자반(ITP), 저용량 aspirin 장기 투여 (권고강도: 강함)
* 추가 적응증 : 원인 불명 철결핍빈혈, 양성 위선종 내시경 절제 후, 기능성 소화불량증 (권고강도: 약함)
* 최신 추가 권고 **:** 만성 위염 동반 H. pylori 감염 (위암 예방 목적); 고위험 군에서 검사-치료(test-and-treat) 전략 (권고강도: 약함\~중등도)
...
* 경험적 1차 치료 : bismuth 4제 요법 또는 동시 치료(concomitant therapy)를 우선 고려
* 경험적(empiric) clarithromycin 3제 요법은 우리나라에서 더 이상 1차 선택으로 권장되지 않음

#### <mark style="color:$primary;">1차 치료 요법 선택 기준</mark>

<table><thead><tr><th width="395.57891845703125">임상 상황</th><th>우선 선택 요법</th></tr></thead><tbody><tr><td>clarithromycin 감수성 확인 + macrolide 노출력 없음</td><td>표준 3제 요법 14일</td></tr><tr><td>clarithromycin 노출력 있음 / 감수성 불명</td><td>동시 치료 10일</td></tr><tr><td>고내성 지역에서의 경험적 치료 (한국 대부분)</td><td>bismuth 4제 10~14일</td></tr><tr><td>Penicillin 알레르기</td><td>bismuth 4제 (amoxicillin 제외)</td></tr><tr><td>이전 제균 치료 실패</td><td>감수성 유도 치료 또는 bismuth 4제</td></tr></tbody></table>
