...
* ≥3회/d, ≥4주 지속되는 설사
* 기전상 여러 특성을 지님
* 대부분 장 점막에서의 수분 및 전해질의 흡수-분비에 불 균형이 발생되어 있음
...
* Whipple Dz, Giardiasis, 셀리악병, short bowel syndrome, 소장 세균 과증식, 췌장 외분비 기능 부전(예: chronic pancreatitis, pancreatic cystosis)
...
* 허브 : St. John’s wort, echinacea, 마늘, 인삼, saw palmetto, cranberry, 알로에 염증성-비감염성 설사 (Inflammatory-Noninfection)
...
* 경고 징후에 해당되는 경우 시행
* 혈액 : CBC, ESR, CRP, 전해질(Mg, P, Ca, Na), LFT(단백질, 알부민), TSH, anemia study, INR, anti-tissue transglutaminase IgA(셀리악병), Vit A, Vit /B9/B12, Vit D
...
A. 발생한 지 최소 6개월 되었고 최근 3개월간 통증이나 불편을 주는 복부 팽만이 없는 죽 또는 물 같은 대변 배출이 배변 횟수의 ＞¼에서 발생
...
```mermaid
graph TD
    Start(["만성 설사"]) --> DDx["<b>DDx:</b><br/>1. 급성 설사의 원인들 &nbsp; 5. 기생충 감염<br/>2. Lactose intolerance &nbsp; 6. 약물<br/>3. IBS &nbsp; 7. 전신 질환<br/>4. 췌장 감염"]

    DDx --> Test1["대변 WBC & 잠혈<br/>대장 내시경 및 조직 검사<br/>소장 영상 검사(barium, CT, or MR enterography)"]

    Test1 --> Result1{검사 결과}
    
    Result1 -- "비정상" --> Abnormal["IBD, Cancer"]
    Result1 -- "정상" --> Normal["대변 전해질, 삼투압,<br/>24h 대변 무게, 지방량"]

    Normal --> OsmoticGap{Osmotic gap}

    %% Osmotic gap 증가 경로
    OsmoticGap -- "Osmotic gap 증가" --> FecalFat{Fecal fat}
    FecalFat -- "Fecal fat 증가" --> Malabsorption["• Malabsorption syndrome<br/>• Pancreatic insufficiency<br/>• Bacterial overgrowth"]
    FecalFat -- "Fecal fat 정상" --> OsmoticNormal["• Lactose intolerance<br/>• Sorbitol, Lactulose<br/>• Laxative abuse"]

    %% Osmotic gap 정상 경로
    OsmoticGap -- "Osmotic gap 정상" --> StoolWeight{대변 무게}
    StoolWeight -- "대변 무게 증가" --> Secretory["• Secretory laxative<br/>abuse(>1 kg/24h)"]
    StoolWeight -- "대변 무게 정상" --> Functional["• IBS<br/>• Factitious diarrhea"]

    %% 스타일링
    style Start fill:#f3f0ff,stroke:#dcd6f7
    style DDx fill:#ffebee,stroke:#ffcdd2
    style Abnormal fill:#ffebee,stroke:#ffcdd2
    style OsmoticNormal fill:#ffebee,stroke:#ffcdd2
    style Malabsorption fill:#ffebee,stroke:#ffcdd2
    style Secretory fill:#ffebee,stroke:#ffcdd2
    style Functional fill:#ffebee,stroke:#ffcdd2
    
    style Test1 fill:#f1f8e9,stroke:#dcedc8
    style Normal fill:#f1f8e9,stroke:#dcedc8
    
    style Result1 fill:#fffde7,stroke:#fff9c4
    style OsmoticGap fill:#fffde7,stroke:#fff9c4
    style FecalFat fill:#fffde7,stroke:#fff9c4
    style StoolWeight fill:#fffde7,stroke:#fff9c4
```

**만성 설사 진단 알고리듬**\
Ref. Lange 2020 Current medical diagnosis & treatment 59th Ed. Fig 15-2.
...
* 위생 관리 : 특히 손 청결 유지
* 명확한 진단이 되지 않거나 IBS 진단 기준에 부합하며, 경고 징후에 해당되지 않는 경우에 진단적 치료 시행; 치료에 반응하지 않는 경우 추가 평가 시행
* 항문 주위 피부 보호 : zinc oxide 연고 [보소미](../%EB%B9%84%EB%B3%B4%ED%97%98/), 피부 방수제 [카빌론](../%EB%B9%84%EB%B3%B4%ED%97%98/) 고려
