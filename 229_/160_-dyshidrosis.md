...
```mermaid
graph TD
    A([손/발바닥 소수포 + 가려움]) --> B{편측성 또는 비전형적?}
    B -- 예 --> C[KOH 검사 시행]
    C --> C1{tinea 양성?}
    C1 -- 예 --> C2[항진균제 치료]
    C1 -- 아니오 --> D{급성 중증 또는 광범위한 flare?}
    B -- 아니오 --> D
    D -- 예 --> E[고역가 국소 스테로이드 + wet dressing<br/>필요시 단기 경구 스테로이드]
    D -- 아니오 --> F[냉찜질 또는 wet compress/soaking<br/>+ potent 국소 스테로이드 + 보습제]
    F --> G{4~8주 내 호전?}
    G -- 아니오/잦은 재발 --> H[피부과 의뢰<br/>Patch test, 접촉알레르기 재평가]
    H --> I[국소치료 불응 중증 만성 수부습진<br/>Delgocitinib 또는 Alitretinoin, phototherapy 고려]
    G -- 예 --> J[간헐적 maintenance + 유발인자 회피]
    E --> H
