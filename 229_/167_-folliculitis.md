...
```mermaid
graph TD
    Start(["모낭 중심 구진·농포"]) --> Expo["병력/노출력 확인"]
    style Expo fill:#fff9c4,stroke:#ffe082
    style Start fill:#eeeeee,stroke:#888888,stroke-width:2px

    Expo -->|"온수욕조·수영장 노출 후 1~4일"| Pseudo["Pseudomonas(hot-tub) folliculitis 의심"]
    Expo -->|"장기 국소/전신 steroid 사용력"| SteroidF["Steroid-induced folliculitis 의심"]
    Expo -->|"여드름 + 장기 항생제 사용력"| GNF["Gram-negative folliculitis 의심"]
    Expo -->|"상부 흉배부, 단형성, comedone 없음, 가려움"| Malz["Malassezia folliculitis 의심"]
    Expo -->|"안면부, rosacea 유사, 항생제 저항"| Demo["Demodex folliculitis 의심"]
    Expo -->|"수포성 병변, 통증"| HSV["HSV/VZV folliculitis 의심"]
    Expo -->|"특이 노출력 없음, 전형적 구진농포"| Bact["세균성(주로 S. aureus) 모낭염"]

    Bact --> BactTx["온찜질 + 외용 항생제 우선;<br/>광범위/재발/전신증상 시 전신 항생제 고려"]
    Pseudo --> PseudoTx["대부분 자연 호전 → 대증치료;<br/>지속·중증·면역저하 시 배양 후 ciprofloxacin"]
    Malz --> MalzTx["topical azole; 광범위·재발 시 경구 azole"]
    Demo --> DemoTx["permethrin/topical ivermectin;<br/>난치성 시 피부과 의뢰"]
    HSV --> HSVTx["단순포진/대상포진 챕터의 표준 항바이러스 치료"]
    SteroidF --> SteroidTx["steroid 필요성 재평가·가능 시 조정;<br/>acneiform/Malassezia phenotype에 따라 치료"]
    GNF --> GNFTx["배양 후 항생제 선택; 난치성 시 isotretinoin 고려"]

    style Bact fill:#f96,stroke:#e65100,stroke-width:2px
```
