...
```mermaid
flowchart TD
    A["만성 가려움 ≥6주"] --> B["상세 병력 + 전신 피부진찰<br/>약물 review + focused general physical examination"]
    B --> C{"가려움 시작 당시<br/>원발성 피부병변이 있었는가?"}

    C -->|Yes| D["IFSI Group I<br/>Primarily diseased / inflamed skin"]
    D --> D1["피부질환 평가<br/>eczema/AD, psoriasis, urticaria,<br/>scabies, fungal disease 등"]
    D1 --> D2["필요 시 scraping/culture<br/>skin biopsy ± DIF"]
    D2 --> D3["원인 피부질환 치료"]

    C -->|No| E{"현재 만성 2차 긁음 병변이<br/>뚜렷한가?"}
    E -->|Yes| F["IFSI Group III<br/>Chronic secondary scratch lesions<br/>chronic prurigo / lichenification 등"]
    F --> F1["병변 자체 치료 + underlying cause 평가"]
    F1 --> G

    E -->|No| G["IFSI Group II<br/>Primarily normal skin"]
    G --> H["Systemic / drug / neurologic / pregnancy-related<br/>원인 평가"]
    H --> I["1st-step lab screening<br/>CBC diff·ESR, Cr·urea, AST/ALT·ALP·γ-GT·bilirubin,<br/>LDH, TSH, HbA1c/fasting glucose, ferritin·CRP"]
    I --> J{"병리적 이상?"}

    J -->|Yes| K["이상 소견에 따른 표적 정밀검사<br/>+ 원인 및 증상 치료"]
    J -->|No, 지속| L["2nd-step evaluation<br/>병력·진찰 재평가, parasite test*, CXR, abdominal US<br/>± skin biopsy/DIF"]
    L --> M["선택적 추가검사<br/>SPEP/immunofixation, HBV/HCV, Ca/PTH,<br/>HIV*, Candida*, tryptase/mastocytosis work-up*"]
    M --> N{"원인 확인?"}
    N -->|Yes| K
    N -->|No| O["Chronic pruritus of undetermined origin (CPUO/PUO)<br/>증상 중심 치료 ± 피부과/관련과 의뢰"]
