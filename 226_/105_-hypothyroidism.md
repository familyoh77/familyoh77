...
* 갑상선 호르몬의 생성·분비 부족 또는 시상하부-뇌하수체 축의 기능 이상으로 발생하는 대사 이상 상태
* 스펙트럼 : 무증상(무증상 갑상선저하증) \~ 경증/현성 저하증 \~ (치료하지 않으면) myxedema coma까지 다양한 수준의 증상을 보임
* 유병률 : 일반 인구의 ＞1%, 60세 이상의 5%; '원발성'이 전체 갑상선저하증의 95% 이상 차지
* 경과 : 일과성 갑상선염은 회복될 수 있으나 치료 중단 후 재발할 수 있음
...
* 의식 저하/혼수, 저체온, 서맥, 저혈압, 저나트륨혈증, 저혈당 동반 → Myxedema coma
  * 즉시 응급실로 이송하며, 병원에서는 가능하면 cortisol 검체를 먼저 채취하되 치료를 지연하지 않고 정맥 levothyroxine·경험적 hydrocortisone 및 호흡·체온·전해질 지지 치료를 시행
* 갑상선제 투여 중 새로 발생한 흉통, 불안정 협심증, 혈역학적으로 불안정한 부정맥

<mark style="color:$warning;">**당일~수일 내 평가**</mark>

* 안정형 관상동맥질환이 동반된 환자에서 갑상선제 시작/증량이 필요
* 뇌하수체/시상하부 질환 시사 소견, 다른 뇌하수체 호르몬 이상 동반 → 중추성 갑상선저하증
* 임신부에서 새로 진단된 갑상선저하증
* TSH가 현저히 높고 중증 증상 또는 장기 기능 이상이 동반됨
...
* TBG는 T4/T3 결합능의 약 70\~75%를 차지하는 주된 결합단백질로, 위 표의 약물 외에 질환·생리 상태에 의해서도 변동할 수 있음 (✽ transthyretin·albumin은 결합친화도가 낮아 total 수치에 미치는 영향이 미미함)
  ...
  * 신증후군은 TBG 합성이 줄어드는 것이 아니라 소변으로 소실되는 기전으로, 위 두 경우와 달리 오히려 요구량을 늘리는 방향으로 작용함(☞ 아래 "T4 요구량이 증가하는 상황")
...
* anti-thyroperoxidase Ab(TPO Ab) : 자가면역성 원인 확인 및 무증상 갑상선저하증의 진행 위험 평가에 1회 측정 고려. 반복 검사는 불필요
  * anti-thyroglobulin Ab(TgAb) : TPO Ab 음성이지만 자가면역 갑상선염이 강하게 의심될 때 선택적으로 고려
...
flowchart TD
    ...
    %% TSH Branches
    TSH --> TSH_High["TSH↑<br>검사실 상한 초과*"]
    TSH --> TSH_Normal["TSH 정상"]
    TSH --> TSH_Low["TSH↓<br>검사실 하한 미만*"]

    %% TSH High Path
    TSH_High --> FT4_Meas["fT4 측정"]
    FT4_Meas --> FT4_Low["fT4 ↓"]
    FT4_Meas --> FT4_Normal["fT4 →"]
    FT4_Meas --> FT4_High["fT4 ↑"]

    FT4_Low --> Primary_Hypo["Primary<br>hypothyroidism"] --> Tx["치료"]
    ...
    %% TSH Normal Path
    TSH_Normal --> CentralRisk{"중추성 원인<br>의심?"}
    CentralRisk -->|NO| Euthyroid["Euthyroid"]
    CentralRisk -->|YES| FT4_Meas3["fT4 측정"]
    FT4_Meas3 --> FT4_Low3["fT4 ↓"]
    FT4_Meas3 --> FT4_Normal3["fT4 정상"]
    FT4_Low3 --> Central_Hypo
    FT4_Normal3 --> Euthyroid
...
class TSH,FT4_Meas,FT4_Meas2,FT4_Meas3 lightGreen
classDef yellow fill:#fff9c4,stroke:#ffe082
class TSH_High,TSH_Normal,TSH_Low,FT4_Low,FT4_Normal,FT4_High,FT4_Low2,FT4_NotLow,CentralRisk,FT4_Low3,FT4_Normal3 yellow
...
_\*TSH는 검사실·측정법·연령별 참고구간을 우선 적용함. 대한갑상선학회 2023 권고안은 국내 무증상 갑상선저하증 평가에서 0.6\~6.8 mIU/L를 참고치로 제시함_
...
* 치료 중에는 TSH가 용량 조절 지표로 부적절하므로 fT4를 사용하며, 임상 상태를 함께 보아 대체로 검사실 참고구간의 중간 이상(중간~상부)을 목표로 개별화
...
* 치료 목표 : TSH를 검사실 참고범위(reference range)로 유지 (✽ 일률적으로 특정 수치를 목표로 고정하기보다 참고치 범위를 기준으로 함)
  ...
  * 고령에서는 TSH가 생리적으로 상승할 수 있으므로 연령별 참고구간, 심혈관·골 건강 및 증상을 함께 고려해 목표를 개별화함. 약 7.5 mIU/L은 일부 ≥80세 인구의 연령별 상한으로 제시된 값이지 모든 고령 환자의 일률적 치료 목표는 아님
* 식이 요법 : 별도로 권고되는 식이 요법은 없으며, 요오드 보충은 요오드 결핍이 확인된 경우에만 고려

## <mark style="color:green;">약물 치료</mark>

* 1차 선택 : levothyroxine (T4)
  * liothyronine(T3) : 일상적인 대체제가 아니며, 엄격히 선별한 환자에서만 전문의 감독 아래 제한적 병용 고려

#### <mark style="color:$primary;">Levothyroxine (T4)</mark>

<mark style="color:cyan;">**용법**</mark>

* 용량 : 연령·심장 질환·저하증의 중증도에 따라 결정
  * ＜60세 & 심장 질환(-) : 1.6 ㎍/㎏/d (약 1.5\~1.8 ㎍/㎏/d), 통상 50\~100 ㎍ qd로 시작 <mark style="color:blue;">\[씬지로이드]</mark>
    * [ ] 체중을 기준으로 용량을 계산하되 비만 환자에서는 실제 체중보다 이상체중 또는 제지방량을 기준으로 과량 투여를 피하고, 시판 정제 단위에 맞춰 반올림
...
* 칼슘·철분·알루미늄/마그네슘 제산제, sucralfate, 담즙산 결합수지 등 직접 결합해 흡수를 방해하는 약제와는 4시간 이상 분리; 커피는 약제 복용 최소 30\~60분 경과 후 음용
...
* T4 및 T3는 혈중에서 TBG 등 결합단백질과 결합한 상태로 순환하므로 TBG 농도 변화는 주로 total T4/T3에 영향을 줌. TBG 변화만으로 기계적으로 증감하지 않고 TSH(중추성 갑상선저하증은 fT4)와 임상 상태로 조절
* 고령에서의 감량 고려는 TBG 변화가 아니라 약물 청소율(clearance) 감소 및 체표면적 감소가 주된 기전임
* **금기 · 주의 :** 급성 심근경색, thyrotoxic heart failure, 조절되지 않는 adrenocortical insufficiency에서는 투여하지 않음(또는 원인 교정 후 투여)

<mark style="color:cyan;">**T4 요구량이 증가하는 상황**</mark>

* 임신, 체중 증가, 신증후군, 지속되는 갑상선 이상, 식이 섬유 섭취 증가, 위장관 질환(흡수 감소)
* 흡수 감소 : 두유, 셀리악병, IBD, 유당 불내증, H. pylori 감염, 위축성 위염, 위산 억제제(PPI·H2 차단제), sucralfate, Al/Mg/Ca/Fe 제제, 종합 영양제, cholestyramine, colestipol, raloxifene, sevelamer, orlistat. PPI·H2 차단제는 위산 억제가 기전이므로 단순히 4시간 간격을 두는 것만으로 해결되지 않을 수 있음
* 일부 환자에서 요구량 증가가 보고된 약물 : sertraline — TSH를 추적해 개별 조절
...
* Steroid : TBG를 낮추는 동시에 T4→T3 전환도 억제하는 상반된 기전을 함께 가져 순net effect가 개인차를 보임; 임의로 감량하지 않고 TSH로 판단

#### <mark style="color:$primary;">Triiodothyronine (T3)</mark>

* Levothyroxine에 비해 작용이 강하고 반감기가 짧아 용량 조절이 어려움
* 일상적인 1차 치료 또는 T3 단독요법은 권고하지 않음. levothyroxine 부형제 불내성이 의심되면 먼저 다른 제조사·제형을 시도하고, 예외적 대안은 내분비내과 전문의와 결정
...
1. 현성 갑상선저하증 진단이 확실한지 확인 - 과거 TSH ≥10 mIU/L 또는 치료 전 fT4 저하가 문서로 확인되어야 함; 확인되지 않으면 T3 시도보다 levothyroxine을 6주간 중단 후 TSH를 재검하는 방안을 우선 고려
2. 지속 증상을 설명할 수 있는 동반 질환을 배제 - 갑상선저하증 증상은 비특이적이며, 정상 갑상선 기능인 사람의 60%도 유사 증상을 호소할 수 있음
3. levothyroxine 용량을 먼저 최적화 - TSH를 참고치 하한부(0.3\~2.0 mIU/L)로 3\~6개월간 유지한 후 반응을 평가; 일부에서 TSH 0.1\~0.3 mIU/L의 짧은 관찰을 고려할 수 있으나, 고령·심혈관질환·골다공증 위험 환자에서는 피하고 TSH＜0.1 mIU/L는 허용하지 않음

{% hint style="info" %}
**동반질환 배제를 위한 권장 검사 (BTA/SfE 2023)** : FBC, 전해질/신기능, 간기능, calcium, HbA1c, ferritin, B12, vitamin D, transglutaminase Ab(셀리악병); 체중 감소나 부신부전이 의심되면 오전 9시 cortisol; Epworth 척도로 수면무호흡 선별, HADS 등으로 우울 선별
{% endhint %}

* T3 병용을 고려할 수 있는 조건(모두 충족) : ① 확실한 원발성 갑상선저하증 진단, ② levothyroxine 단독 투여에 증상 지속, ③ levothyroxine이 충분한 기간·용량·TSH 반응으로 적절히 투여됨, ④ 증상을 설명할 다른 동반 질환이 배제됨, ⑤ 내분비내과 전문의가 시작·감독할 의향이 있음
...
* 반감기가 짧아 이론적으로는 1일 2회 분할 복용이 원칙(예 : 아침·오후로 나눔)이지만 국내 유통 liothyronine 정제는 5 ㎍ 단일 함량으로<mark style="color:blue;">\[테트로닌]</mark> 분할이 어려워 실제로는 저용량(예 : 5 ㎍/d) 1일 1회 복용으로 처방되는 경우가 많음 - 처방 시 반감기·혈중 농도 변동을 환자에게 설명하고, 용량이 늘어나는 경우(10 ㎍/d 이상) bid 분할 투여를 우선 고려
* 감량/중단 시 : liothyronine 5 ㎍은 levothyroxine 약 15 ㎍(1:3 비율)로 대체; 용량 변경 6\~8주 후 TSH 재검; 안정적으로 병용 중이며 TSH가 정상인 환자를 일률적으로 감량할 필요는 없음
* DIO2, MCT10 등 유전자 다형성이 T4/T3 병용요법 반응과 관련될 수 있다는 소규모 연구가 있으나, 근거가 충분하지 않아 치료 반응 예측을 위한 유전자 검사는 현재 임상에서 권고되지 않음

{% hint style="danger" %}
**Liothyronine 장기 사용의 안전성 우려** : 과량 투여로 지속적인 TSH 억제가 발생하면 심방세동 등 부정맥과 골소실 위험이 증가할 수 있으므로 TSH를 참고범위 내로 유지하며 장기 추적이 필요하다. 국내 연구(Yi 등, _Thyroid_ 2022)에서는 liothyronine(단독 또는 levothyroxine 병용) 사용이 levothyroxine 단독 사용에 비해 심부전 및 뇌졸중 발생 위험 증가와 연관되었으며, 특히 1년 이상 사용군에서 이러한 연관성이 더 뚜렷하였다. 다만 이는 관찰연구 결과이므로 인과관계를 의미하는 것은 아니다. 장기 사용 시에는 TSH, 심박수, 부정맥(특히 심방세동)을 추적하고, 폐경 후 여성 등 골다공증 위험이 높은 환자에서는 골밀도 평가를 고려한다.
{% endhint %}
...
* 검사 일정 : 원발성 갑상선저하증은 TSH를 중심으로 진단 초기 및 용량 변경 6(±2)주 후 검사 → 안정화 전 3개월마다 검사 → 2회 연속 참고치 내 비슷한 수준이면 6개월 후 및 매년. fT4는 진단 초기, 결과 불일치, 임신, 중추성 갑상선저하증 등에서 함께 측정
...
* TSH가 성공적으로 조절됨에도 저하 증상이 지속되는 경우 : 'T3 병용 고려 전 확인 순서'의 '진단 재확인 → 동반 질환 배제 → levothyroxine 용량 최적화'를 실행하고, 필요시 의뢰
...
* Liothyronine·천연 갑상선 추출물(desiccated thyroid) 복용 중이면 임신 시도 전 LT4 단독으로 전환 - 태아 뇌는 모체에서 오는 T4에 의존하므로 T3 우세 상태는 태아 갑상선호르몬 공급에 불리할 수 있음
  * 전환 비율 : liothyronine 5 ㎍ ≈ LT4 20 ㎍(1:4)

#### <mark style="color:$primary;">임신 중 관리</mark>

* 임신 중 새로 확인된 현성 갑상선저하증 : 지체 없이 LT4를 시작. 대체로 1.5\~1.7 ㎍/㎏/d를 기준으로 산정하고 임신에 따른 요구량 증가(약 20\~30%)를 반영해 개별화
  * 임신 참고치 상한을 넘지만 TSH＜6 mIU/L인 경미한 이상은 fT4, 증상, 임신 주수와 환자 선호를 반영해 즉시 치료하거나 1\~3주 내 재검하는 방안을 공동 결정; TSH ≥6 mIU/L이거나 fT4 저하가 명확하면 치료를 시작
* 이미 LT4를 복용 중인 여성 : 임신 확인 즉시 약 25% 증량(예 : 평소 stable dose에서 주 2회 추가 복용) 후 적정; 이후 임신 12주까지 약 25%, 20주까지 약 50% 증량이 필요한 경우가 많음
  * 임신 전 TSH＜1.5 mIU/L, 임신 전 LT4 용량 ＞100 ㎍/d, 또는 주 2정 추가 방식으로 일괄 증량한 경우 과잉치료 위험이 다소 높으므로 증량 폭을 개별화
* 모니터링(TSH, free/total T4) : 임신 전반기 4주마다 + 임신 3분기 중 최소 1회(약 30주 전후) + 용량 조정 후 4\~6주 후 + 분만 6주 후
* 조절 목표
  * TSH : 검사실·지역의 임신 분기별(trimester-specific) 참고구간이 있으면 최우선 사용
    * 분기별 참고구간이 없으면 1분기에 0.1\~4.0 mIU/L를 대안으로 사용할 수 있음. 해당 검사실의 비임신 상한이 ＞4.5 mIU/L이면 임신 전반기에는 비임신 상한에서 약 0.5 mIU/L를 낮춰 적용하고, 이후 임신 후반기로 갈수록 비임신 범위로 회복하는 방식도 허용됨
  * 이미 LT4를 복용 중인 여성(임신 전·임신 중 공통) : TSH를 참고구간 내, ＜2.5 mIU/L으로 유지하는 것을 목표로 함(임신에 따른 요구량 증가에 대비한 안전역)
...
#### <mark style="color:$primary;">임신 중 무증상 갑상선저하증</mark>

* 임신 1분기에 반복 검사로 지속되는 무증상 갑상선저하증이 확인되면 TSH, TPO Ab, 과거력 및 환자 선호를 함께 고려해 LT4 시작을 공동 결정
* 임신 1분기 이후 처음 발견된 경미한 무증상 갑상선저하증은 일반적으로 새로 LT4를 시작하지 않음. 다만 TSH가 현저히 높거나 fT4가 낮으면 현성 갑상선저하증으로 재분류해 치료
* 치료를 시작한다면 TSH·체중·임신 주수에 따라 보통 25\~75 ㎍/d 범위에서 개별화하고 4주 간격으로 조절
* 임신 중 처음 시작한 경증 환자는 분만 직후 감량 또는 중단을 검토하고 약 6주 후 TSH 재검. 특히 분만 시 LT4 ≤75 ㎍/d이면 중단 시험을 적극 고려
...
* TSH·fT4가 모두 정상인 TPOAb 양성 여성(임신 준비 중, 불임/습관유산 병력, 임신 중 모두 포함)에게는 TSH 수치나 유산력과 무관하게 LT4 치료를 시작하지 않음 - LT4 투여가 산과적 예후를 개선하지 못함
  * [ ] 과거 쓰이던 "TPOAb 양성 + TSH＞2.5는 치료 고려"라는 접근은 최신 근거로 폐기되었으므로 주의; TPOAb 양성 자체는 조산·유산 위험과 연관되나(절대 위험차 +2\~8%), 이 위험은 LT4로 교정되지 않음
...
### <mark style="color:orange;">Postpartum thyroiditis (산후 갑상선염)</mark>

* 자연 경과(3상성, triphasic) : 분만 후 1\~6개월경 파괴성 갑상선기능항진기 → 4\~8개월경 갑상선기능저하기 → 이후 수개월에 걸쳐 자연 회복 - 모든 상을 거치지 않고 한 상만 나타나는 경우도 흔함
...
* **갑상선기능저하기** - LT4 치료 : 증상이 있거나, 수유 중이거나, 6개월 이내 임신을 계획 중인 경우에 시작(무증상이면 4\~8주마다 재검하며 관찰 가능); LT4를 시작했더라도 저하기는 흔히 일과성이므로 분만 후 약 1년 시점에 감량/중단(taper) 시도를 원칙으로 함
* 초기 정상화 후에도 장기적으로 10\~50%가 결국 영구 갑상선저하증으로 이행할 수 있음(위험 인자 : 다분만력, 초음파상 저에코, 초기 저하증 중증도, TPOAb 양성, 고령, 유산력) - 정상화되었더라도 1년 후 재검 또는 저하 증상 발생 시 재검 권고
* Graves병 재발과의 감별 : 발병 시기(산후갑상선염 1\~6개월 vs Graves 3\~12개월), TRAb/TSI(음성 vs 양성 - 가장 확실한 감별점), 갑상선안병증(없음 vs 있을 수 있음), TT3/TT4 비율(전형적으로 ＜20 vs ＞20) - 감별이 애매하면 보존적 경과 관찰 우선, 필요시 의뢰
...
{% hint style="info" %}
**국내 진단 기준 및 중증도 분류 \[대한갑상선학회 무증상갑상선기능저하증 진료 권고안 2023]**\
2013\~2015년 국민건강영양조사 기준 집단(갑상선질환·가족력 없음, 갑상선자가항체 음성, 비임신) 분석에 근거하여 한국인 TSH 참고치를 0.6\~6.8 mIU/L로 제시 - 국제적으로 흔히 쓰이는 4.0\~5.0 mIU/L 상한보다 높음(높은 요오드 섭취와 연관)\
...
* 갑상선 외 상황 : 비갑상선질환증후군(sick euthyroid) 회복기, 부신피질저하, 만성신부전, 고도비만(BMI ＞40) 등은 일시적 TSH 상승 또는 유사 검사 소견을 보일 수 있어 지속 여부와 임상 맥락을 재평가
...
* 기타 : 고령, 계절/일중 변동, 측정 방법에 따른 차이, TSH 측정 방해 요인(이종친화항체, 류마티스인자, 비오틴, macro-TSH, TSH 아형 이상). TSH와 fT4가 생리적으로 맞지 않으면 중추성 질환·갑상선호르몬저항증·측정 간섭을 별도 감별
...
* 진행률 : 연간 약 2\~4%가 현성 갑상선저하증으로 진행하며, 장기 누적 위험은 기저 TSH와 TPO Ab 양성 여부에 따라 크게 달라짐. 일부에서는 TSH가 자연 정상화되므로 첫 이상은 반복 검사로 확인
...
### <mark style="color:orange;">치료 선별</mark>

_✽아래 경증/중증 TSH 구간(6.8\~10 / ＞10 mIU/L)은 대한갑상선학회 2023 기준이며, 국제 가이드라인(주로 4.5\~5.0 / 10 mIU/L 기준 사용)과 다를 수 있음_

<table><thead><tr><th width="124.28570556640625">연령</th><th width="140">중증도</th><th>권고</th></tr></thead><tbody><tr><td>성인(＜70세)</td><td>경증(6.8~10)</td><td>LT4 치료는 일반적으로 권고하지 않음 [Weak, Moderate]</td></tr><tr><td>성인(＜70세)</td><td>중증(＞10)</td><td>관상동맥질환·심부전 동반 시 치료 시행 [Strong, Moderate]; 이상지질혈증 동반 시 개선 목적으로 고려 가능 [Weak, Moderate]</td></tr><tr><td>고령(≥70세)</td><td>경증(6.8~10)</td><td>치료하지 않음 [Strong, High]</td></tr><tr><td>고령(≥70세)</td><td>중증(＞10)</td><td>일반적으로 권고하지 않음 [Weak, Moderate]</td></tr></tbody></table>

* 대사증후군·비만 개선 목적의 LT4 치료는 시행하지 않음
* 우울증 개선 목적(성인 ＜70세) 및 인지기능 개선 목적(고령 ≥70세)의 LT4 치료는 권고하지 않음
...
```mermaid
flowchart TD
    Start["무증상갑상선기능저하증<br>(TSH↑, fT4 정상) 확인"] --> Retest["2~3개월 후 TSH 재검<br>TPO Ab는 1회 측정"]
...
style Start fill:#eeeeee,stroke:#888888,stroke-width:2px
classDef yellow fill:#fff9c4,stroke:#ffe082
class Age,Persist,CADHF,Dyslip yellow
classDef orange fill:#fdebd0,stroke:#e67e22
class ElderlySevere,AdultSevere orange
classDef orange50 fill:#fff3e0,stroke:#e65100
class AdultMild,ElderlyMild orange50
classDef sky fill:#e3f2ff,stroke:#2196f3
class TxConsider,TxYes sky
classDef blue fill:#d0e8ff,stroke:#1a6abf
class Resolve,ElderlyMildTx,ElderlySevereTx,AdultMildTx,Observe blue
style Retest fill:#e8f8e8,stroke:#4caf50
```
...
* 경구 LT4 선택 <mark style="color:blue;">\[씬지로이드]</mark>
* 초기 용량 12.5\~50 ㎍/d; 고령 또는 심혈관질환 위험을 동반한 환자는 12.5\~25 ㎍/d로 시작하여 1\~2개월 간격으로 갑상선 기능을 추적하며 TSH 정상화를 목표로 조정
...
* 하시모토 갑상선염에서 일률적인 저요오드 식이는 권장하지 않음. 다만 다시마 농축액·해조류 추출 보충제 등 과도한 요오드 섭취는 피하도록 교육
...
>
> ```
> 씬지로이드정 0.1 ㎎/T   1T   qd  아침 식전 30~60분
> ```
>
...
>
> ```
> 씬지로이드정 0.025 ㎎/T   1T   qd  아침 식전 30~60분
> ```
>
...
> ```
>
> _✽임신 중 새로 확인된 현성 갑상선저하증은 대체로 1.5\~1.7 ㎍/㎏/d를 기준으로 산정하고 임신에 따른 요구량 증가(약 20\~30%)를 반영해 개별화; 기존 복용자는 확인 즉시 약 25% 증량(예 : 주 2회 추가 복용); 목표 TSH는 임신 분기별 참고구간을 우선 사용하고, 없으면 1분기 0.1\~4.0 mIU/L를 대안으로 고려; 임신 전반기 4주마다 + 3분기 중 1회 + 용량 조정 4\~6주 후 + 분만 6주 후 재평가 (ATA 2026)_
...
>
> ```
> 씬지로이드정 0.075 ㎎/T   1T   qd  아침 식전
> 씬지로이드정 0.025 ㎎/T   0.5T qd  아침 식전
> 테트로닌정 5 ㎍/T       1T   qd  아침 식전 (levothyroxine과 동시 복용)
> ```
>
> _✽예시는 기존 levothyroxine 100 ㎍/d 복용자를 liothyronine 1/17 대체 원칙(BTA/SfE 2023)에 따라 계산한 LT4 85 ㎍/d를 실제 시판 함량에 맞춰 87.5 ㎍/d(75+12.5 ㎍)로 근사한 것; 반정 처방 가능 여부와 분할 정확도를 확인하고 필요하면 75 ㎍/d와 100 ㎍/d 교대 투여 등으로 조정; **반드시 내분비내과 전문의 감독 하에서만 시작**하며, 현성 갑상선저하증 진단이 확실하고 동반질환 배제 및 LT4 최적화 후에도 증상이 지속되는 경우에 한해 시도; 3\~6개월 후 무반응이면 LT4 단독으로 복귀; 임신 중에는 사용하지 않음_
...
### <mark style="color:$primary;">예약일보다 일찍 상담하세요</mark>
