...
flowchart LR
    A{"IRLSSG 5개 기준 충족?"}
    A -- 아니오 --> B[유사 질환 감별]
    A -- 예 --> C[악화 약물·동반 질환·철 상태 평가]
    C --> D{"증상 빈도와 임상적 영향"}
    D -- 간헐형·경증 --> E[비약물 치료 중심]
    D -- 만성 지속형 --> F[철 상태별 치료 및 gabapentinoid]
    F --> G{"불충분 또는 augmentation?"}
    G -- 아니오 --> H[효과·이상반응 정기 추적]
    G -- 예 --> I[도파민 증량 금지·대체 치료·전문의 의뢰]
```
