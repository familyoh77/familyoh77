...
_<mark style="color:$info;">✽ 2024년 Lancet Commission 보고서에서 기존 12가지 위험인자에 미교정 시력 저하와 고LDL 콜레스테롤을 추가하여 총 14가지로 확대. 약 45%는 인구집단 수준의 이론적 예방가능분율 추산치이며, 모든 위험인자를 교정하면 개별 환자의 치매 위험이 실제로 45% 감소한다는 뜻은 아님</mark>_
...
* 기본 평가(Tier 1 '인지 검사 패널') : 거의 모든 환자에서 시행
  * CBC(감별 포함), 신·간기능, 전해질, 혈당, 칼슘·마그네슘·인, TSH, Vit B12, homocysteine, ESR, CRP와 우울 선별
  * homocysteine : B12 혈중 농도로 드러나지 않는 기능적 B12 결핍의 단서이자 뇌혈관 위험 지표
  * ESR/CRP : 잠재된 염증·감염·종양을 거르는 비특이적 선별
  * BUN/Cr 비 ＞20 : 탈수 시사
* 이를 '가역적 치매 검사'라고 부르면 오해 소지가 있음. 이 검사로 찾는 상태가 서서히 진행하는 인지저하의 일차 원인인 경우는 드물고(부분적으로라도 가역적인 치매는 0.3\~0.6%), 기저 신경퇴행성 질환을 악화시키는 동반 상태로 발견되는 경우가 약 9%
* 선택 검사(Tier 2, 임상상에 따라 일부 환자) : ANA, HbA1c, 지질, folate, MMA, 암모니아, 납, RPR, HIV, SPEP, PT/PTT, 흉부 X선, 요검사·요배양, 수면다원검사(수면무호흡 또는 렘수면행동장애 의심 시)
...
* 인지·행동 증상의 원인 평가를 위해 **비조영 MRI**를 기본으로 시행(국내 지침도 MRI 또는 CT를 기본 검사로 권고)
...
* 인지 증상이 있는 환자의 판독지에서 뇌위축, 백질 변성(leukoaraiosis), 미세출혈을 일률적으로 '연령 관련 변화'로 해석하지 않음. 매우 경미하고 미만성이 아니라면 분포와 정도를 확인하고 임상 소견과 대조하며, 필요하면 판독의와 소통
