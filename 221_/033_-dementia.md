...
* 진행성 치매(퇴행성, 비가역적) : Alzheimer Dz(AD), 루이체 치매, 파킨슨병(☞ [파킨슨병 비운동 증상 — 치매](035_-parkinsons-disease.md#비-운동-증상)), 전측두엽 치매
...
1. **시간이 부족할 때(3~5분)** → Mini-Cog : 민감도 높고 교육 수준 영향 적음
2. **기본 선별(7~10분)** → K-MMSE : 가장 광범위하게 사용; 경계 결과(20~24점) 시 K-MoCA 추가
3. **경증 치매·집행 기능 의심(10분)** → K-MoCA : MMSE 천장 효과 보완; MCI 감별에 우수
4. **중증도 평가(면담 15~20분)** → CDR 또는 GDS : 경과 추적·요양등급 판정에 활용
5. **가정 자가 모니터링** → SAGE : 15분, 환자/가족이 직접 시행
{% endhint %}
