...
* 반복적인 발작성 두통을 특징으로 하는 만성 신경계 질환
  * 전형적으로 편측성·박동성의 중등도-중증 통증이 4\~72시간 지속되며 구역·구토, 빛공포증·소리공포증을 동반; 환자의 약 20\~30%(1/4\~1/3)에서 시각·감각 등의 조짐(aura)이 선행
  * 두통이 ＞3개월, ≥15일/월(그중 ≥8일/월 편두통 특징) 지속되면 만성 편두통으로 구분
* 편두통은 전 세계 질병부담연구에서 장애생존연수(years lived with disability, YLD) 기준 주요 원인 중 하나로, 1차 진료에서 저평가·저치료되는 경향이 있음
* 국내 1년 유병률 : 5\~6% - 2009 Korean Headache Survey 6.1%(여성 9.2%, 남성 2.9%), 2018 Korean Sleep-Headache Survey 5.2%; 여성에서 약 3배 흔함 (Kim et al., J Clin Neurol 2012; Kim et al., J Clin Neurol 2021)
...
* 월경 관련 편두통(Menstrual migraine) : 월경 시작일을 day 1로 할 때 day -2\~+3에 최소 3주기 중 2주기 이상 발생
...
* 비교적 흔히 보고되는 요인 : 알코올, 카페인 과다 섭취 또는 갑작스러운 중단, 식사 거름
* 음식 영향은 개인차가 크고 근거가 일관되지 않으므로 특정 음식 또는 첨가제(숙성 치즈, 가공육, 초콜릿, MSG, 인공감미료 등)를 일률적으로 제한하지 않으며, 반복적으로 연관성이 확인되는 경우에만 제한
* 두통 일기로 시간적 연관성이 반복 확인되는 음식만 개인별로 조절하며, 음식에 대한 갈망 자체가 편두통 전구증상일 수도 있음
...
* 편두통 발작은 '전구기 → 조짐(일부 환자) → 두통기 → 후구기'의 시간적 단계로 나타날 수 있음
  * 전구기(prodrome) : 두통 발생 수 시간\~최대 48시간 전(조짐 편두통에서는 aura 이전)에 피로, 하품, 집중력 저하, 목 경직/통증, 기분 변화, 특정 음식에 대한 갈망 등이 나타날 수 있음
  * 조짐(aura) : 편두통 환자의 약 20\~30%(1/4\~1/3)에서 발생; 시각·감각·언어 등 완전히 가역적인 국소 신경학적 증상으로 보통 5\~60분 지속하며, 운동 증상은 최대 72시간까지 지속 가능
  * 후구기(postdrome) : 두통이 소실된 뒤 최대 48시간 동안 피로, 집중력 저하, 기분 변화, 목 경직/통증 등이 지속될 수 있음

{% hint style="info" %}
전구기는 aura와 다른 개념이며, aura는 일시적인 국소 신경학적 증상을 의미함
{% endhint %}
...
* 적절한 급성기·예방 치료에도 반응이 미흡
* 두통 빈도 증가로 약물과용두통(MOH) 위험이 있음
* 만성 편두통으로 진행
...
* 두통 발생 초기에 투여하는 것이 보다 효과적
  * triptan은 두통이 시작되는 시점에 투여하는 것이 권장됨; 조짐(aura) 단계에서의 투여는 효과가 제한적임
...
* Triptans : eletriptan은 강력한 CYP3A4 억제제(ketoconazole, itraconazole, clarithromycin, ritonavir, nefazodone 등) 투여 중 및 투여 중단 후 최소 72시간 이내 병용 금기(국내 미출시이나 안전성 정보로 기재)
...
* lasmiditan : 발작 시 50\~200 ㎎ 1회 <mark style="color:blue;">\[레이보우]</mark>(비급여); 24시간 이내 추가 투여하지 않음
* 부작용 : 졸음, 어지럼증
...
flowchart TD
    ...
    K --> L0["충분하고 지속적인 반응?"]
    L0 -- "Yes" --> H
    L0 -- "No" --> L["부분 반응 또는<br/>호전 후 재발?"]
    L -- "Yes" --> M["허가된 최소 간격 후 재투여<br/>± NSAID 병용"]
    L -- "No" --> N["같은 발작에서 반복 투여하지 않고<br/>다음 발작에서 용량·제형 변경<br/>다른 triptan 또는 다른 계열 고려"]
    classDef question fill:#fff9c4,stroke:#ffe082
    classDef startNode fill:#eeeeee,stroke:#888888,stroke-width:2px
    class A startNode
    class C,E,G,I,L,L0 question
```

<p align="center"><strong>급성 편두통 약물 선택 알고리듬</strong></p>

***

## <mark style="color:green;">편두통의 예방적 약물 치료</mark>
...
* 월 편두통일수 ≥4일 또는 월 중등도\~중증 두통일수 ≥4일인 경우 예방 치료 제공을 권고 \[AAN·AHS 2026]
...
* 예방 치료 목표 : 일반적으로 두통 빈도 ≥50% 감소를 목표로 하되, 만성 편두통에서는 두통일수 ≥30% 감소와 기능·삶의 질 개선도 임상적으로 의미 있는 반응으로 평가할 수 있음; 빈도·강도·동반 증상·급성약 사용·환자별 목표를 함께 평가
...
* 중등도 이상의 효과 근거가 있는 기존 경구 예방약 : propranolol, topiramate, valproate; amitriptyline·metoprolol·flunarizine은 삽화성 편두통에서 낮은 확실성의 근거 \[AAN·AHS 2026]
...
* 주의/금기 : 천식 또는 기관지경련성 질환, 증상성 서맥·저혈압, 고도 방실차단; 고령에서는 기립성 저혈압과 낙상 위험을 고려
...
* valproate : 500\~1,500 ㎎/d <mark style="color:blue;">\[오르필]</mark>; divalproex 250\~1,500 ㎎/d <mark style="color:blue;">\[데파코트]</mark>
  ...
  * 주의/금기 : 간질환, 췌장염, 저혈소판증
  * 임신·가임 관련 금기(편두통 예방 목적, 신경관 결손 등 강력한 기형 유발 위험에 근거) :
    * 임신 중 : 금기
    * 효과적인 피임법을 사용하지 않는 가임 가능 여성 : 금기
    * 가임 가능 여성 : 다른 적절한 대안이 없을 때에 한해 위험 설명과 임신 예방조치(신뢰도 높은 피임법) 시행 후 예외적으로만 고려
...
* Atogepant는 기존 경구 예방치료 2\~4종에 실패한 삽화성 편두통 환자에서도 효과가 입증됨(ELEVATE 임상)
...
* 만성 편두통 예방에 사용; 위약 대비 편두통 발생 약 2일/월 감소 효과
...
* 삽화성 편두통 예방에 telmisartan·nifedipine·enalapril의 근거를 낮은 확실성으로 평가, 널리 사용되는 candesartan은 해당 지침의 분석 기준상 근거 불충분 \[AAN·AHS 2026]
...
* candesartan : 4\~16 ㎎/d <mark style="color:blue;">\[아타칸]</mark>
  * 부작용 : 저혈압, 고칼륨혈증, 신기능 악화
  * 주의/금기 : 임신, 중증 신기능 저하 또는 고칼륨혈증 위험 환자에서 신중 사용
* lisinopril : 10\~20 ㎎/d <mark style="color:blue;">\[제스트릴]</mark>
  * 부작용 : 어지럼증, 두통, 기침, 피로, 저혈압
  * 주의/금기 : 과거 ACE inhibitor 관련 혈관 부종, 유전성/특발성 혈관 부종, 임신
...
* frovatriptan 2.5 ㎎ bid <mark style="color:blue;">\[미가드]</mark> : 반감기가 길어 단기 예방에 적합(국내 허가 외 사용)
* naratriptan 1 ㎎ bid : 근거 연구에서 사용된 대안 용량이나 국내 유통 제형은 2.5 ㎎ 정으로 처방 실행에 제한이 있음(국내 허가 외 사용)
* NSAID(예: naproxen 500 ㎎ bid)도 단기 예방에 사용 가능(편두통 예방은 국내 허가 외 사용; 위장관·신장·심혈관 위험 평가)

### <mark style="color:orange;">조짐 편두통과 호르몬 피임</mark>

* 조짐 편두통에서는 허혈성 뇌졸중 위험을 고려하여 estrogen 함유 복합호르몬피임제(복합경구피임제·패치·질링)를 원칙적으로 피함
* 흡연, 고혈압, 고령 등 혈관 위험인자가 있으면 위험이 더욱 증가할 수 있음
...
* 응급실 의뢰 고려
* 아래 비경구 치료 근거의 상당 부분은 status migrainosus만을 대상으로 한 연구가 아니라, 응급실에서 비경구 치료가 필요한 급성 편두통 전반의 연구에서 도출됨 \[AHS ED 2025]
* IV 수액 : 탈수·경구 섭취 불량·저혈압이 있는 경우 0.9% 생리식염주사액 또는 하트만액 등 등장성 결정질액으로 교정; 모든 환자에게 일률적으로 투여하지 않음
* IV/IM 항구토제 : metoclopramide <mark style="color:blue;">\[맥페란주]</mark>; IV prochlorperazine은 원 지침에서 제공해야 하는(Level A) 우선 권고 약제이나, 국내에서는 상용 주사제 유통 여부를 확인해야 함 \[AHS ED 2025]
...
* IV valproate 500\~1,000 ㎎ : 난치성 또는 다른 1차 응급치료가 적절하지 않은 경우 대안으로 고려 <mark style="color:blue;">\[데파킨주]</mark>
  ...
  * 제품 허가사항에 따라 서서히 정맥 투여(국내 데파킨정맥주사 허가 용법 확인)
...
>
> ```
> 부루펜 400 ㎎/T 1T (두통 발생 시 즉시; 필요시 6~8시간 후 반복, 최대 3회/d)
> 맥페란 10 ㎎/T 1T (구역 동반 시)
> ```
>
> _✽경증\~중등도에서 1차 선택; 두통 시작 초기에 복용하고 반응이 불충분하며 금기가 없으면 1회 800 ㎎까지 고려. 단일 진통제는 월 15일 미만으로 사용(약물과용두통 예방)_
...
> ```
>
> _✽조짐(aura) 단계가 아닌 두통이 실제로 시작되는 시점에 복용; 허혈성 심혈관·뇌혈관 질환, 조절되지 않는 고혈압, hemiplegic/brainstem aura migraine에서는 금기. 트립탄·복합진통제는 월 10일 미만으로 사용_
...
> ```
>
> _✽저용량으로 시작해 서서히 증량(신결석·인지기능 부작용 최소화 목적); 임신 중 사용 금기이므로 가임 가능 여성은 시작 전 임신 계획 여부 확인 및 신뢰도 높은 피임법 권고_
...
> * ⚠️ 빛이 번쩍이거나 시야 변화 등의 **조짐(aura) 증상이 나타나는 단계에서는 복용하지 마시고**, 실제 두통이 시작될 때 복용하십시오.
> * 첫 복용 후 호전되었다가 두통이 다시 생기거나 부분적으로만 호전된 경우에는 약마다 정해진 최소 간격 후 추가 복용할 수 있습니다. 첫 복용에 전혀 효과가 없으면 임의로 반복 복용하지 말고 의사와 치료 방법을 상의하십시오.
...
> **라스미디탄 (레이보우)**
>
> * 트립탄과 달리 혈관수축 작용이 없어 심혈관 질환으로 트립탄 사용이 어려운 환자에서 대안이 될 수 있습니다. 다만 심장질환·고혈압·서맥이 있으면 복용 전 의사와 상의하십시오.
> * ⚠️ **복용 후 최소 8시간 동안 운전이나 기계 조작을 하지 마십시오.** 졸음을 느끼지 못하더라도 반드시 지켜야 합니다.

> **편두통 예방약 (인데놀·토파맥스·에트라빌 등)**
>
> * 예방약은 효과가 나타나기까지 수 주가 걸리며, 충분한 효과 평가는 보통 권장 내약 용량에서 **8\~12주** 후 시행합니다. 임의로 중단하지 마십시오.
> * 토파맥스(topiramate)는 물을 충분히 마셔야 신결석 위험을 줄일 수 있습니다.
> * 가임 가능 여성은 토파맥스(topiramate) 치료 전 임신 계획 여부를 반드시 의사에게 알리고, 치료 중 신뢰도 높은 피임법을 사용하십시오.
> * 인데놀(propranolol) 복용 중에는 갑자기 중단하지 마십시오.

> **진통제 과다 사용 주의**
>
> * 트립탄 또는 복합 진통제는 **한 달 10일 미만**, 단순 진통제(NSAIDs, 아세트아미노펜)는 **한 달 15일 미만**으로 사용하십시오.
> * 이러한 사용이 3개월 넘게 지속되면 '약물과용두통' 위험이 증가하며, 두통이 한 달 15일 이상 발생할 수 있습니다.
...
* **규칙적인 생활** : 수면·식사·기상 시간을 일정하게 유지하고 장시간 공복이나 식사 거름을 피하세요.
* **수분 섭취** : 탈수는 편두통을 유발할 수 있습니다. 수분 제한이 필요한 질환이 없다면 충분한 수분을 섭취해 탈수를 피하십시오.
* **운동** : 규칙적인 유산소 운동이 예방에 도움이 됩니다. 발작 중에는 운동을 삼가십시오.

#### <mark style="color:$primary;">이럴 때는 즉시 병원을 방문하세요</mark>

* 처음 발생했거나 갑작스럽고 평소 조짐과 다른 팔다리 마비·언어 장애·시야 장애, 또는 신경 증상이 60분 이상 지속되는 경우
* 두통이 72시간 이상 지속되는 경우
