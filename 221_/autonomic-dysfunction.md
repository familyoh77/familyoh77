# 자율 신경 기능 장애 Autonomic Dysfunction

## <mark style="color:green;">일반 사항</mark>

* 교감·부교감 신경계의 기능 이상으로 심혈관, 소화기, 비뇨기, 발한, 성기능 등 다양한 장기에 걸친 증상이 발생하는 증후군
* 원발성(특발성) : 기저 질환 없이 자율신경계 자체의 퇴행성 변화
* 이차성 : 기저 질환(당뇨병, 파킨슨병, 알코올 등)에 의한 자율신경 손상 - 임상에서 대부분을 차지함
* 증상이 다양하고 비특이적이어서 신체증상장애와 감별이 필요함 (☞ [신체증상장애](/broken/pages/aa65e012987231fbb7183abc69ba53fd2a8f42c9))

### <mark style="color:$primary;">대표적 증후군</mark>

* 당뇨병성 자율신경병증 : 이차성 자율신경 기능 장애의 가장 흔한 원인; 장기간 고혈당에 의한 자율신경 손상
* 체위성 빈맥 증후군 (POTS, Postural Orthostatic Tachycardia Syndrome) : 기립 시 심박수 ≥30회/분 증가(10분 이내); 주로 젊은 여성; 어지럼, 심계항진, 피로, 인지 저하; 자가면역, **Long COVID** 후 발생 가능 — Long COVID 환자의 상당수에서 POTS 증상이 보고되며 자율신경절 수용체에 대한 자가항체 관련 기전이 유력하게 제시됨
* 순수 자율신경 부전증 (Pure Autonomic Failure, PAF) : 기립성 저혈압이 주증상; 서서히 진행; 파킨슨병·MSA로 이행 가능
* 다계통 위축증 (Multiple System Atrophy, MSA) : 자율신경 부전 + 소뇌/파킨슨 증상 동반; 빠른 진행

## <mark style="color:green;">원인 및 위험 인자</mark>

#### **신경퇴행성 질환**

* 파킨슨병, 루이소체 치매, 다계통 위축증(MSA)

#### **대사성·전신 질환**

* 당뇨병(가장 흔한 이차성 원인), 아밀로이드증, 만성 신부전, 갑상선 기능 이상

#### **자가면역·감염**

* Sjögren 증후군, 자가면역 자율신경절병증, 길랭-바레 증후군
* 바이러스 감염 후 : **Long COVID**(COVID-19 감염 후 12주 이상 지속되는 증상) — POTS 유발의 주요 원인으로 부상; 자율신경절 항체, 소섬유신경병증, 자율신경 조절 이상 등 복합 기전 제시됨

#### **독성·약물**

* 알코올 남용(만성), 항암제(예: vinca alkaloids, platinum계), 항콜린제, α/β 차단제

#### **기타**

* 척수 손상, 부종양 증후군, 특발성

### <mark style="color:$danger;">🚩 Red Flags!</mark>

<mark style="color:$danger;">**즉각 평가/이송 — 생명 위협 가능성**</mark>

* 기립 시 실신 또는 의식 소실
* 심한 서맥·빈맥 동반 (심혈관계 자율신경 부전)
* 급격한 신경학적 악화 (MSA, 길랭-바레 의심)

<mark style="color:$warning;">**당일 의뢰 또는 긴급 평가 권고**</mark>

* 기립성 저혈압이 조절되지 않아 낙상·외상 반복
* 새로 발생한 자율신경 증상 + 파킨슨 증상 동반 (MSA 감별 필요)
* POTS 의심 : 기립 시 심계항진·어지럼 + 젊은 여성

<mark style="color:$info;">**외래 추적 / 추가 평가 계획**</mark>

* 당뇨병 환자에서 설명되지 않는 저혈압, 소화 장애, 배뇨 장애 - 당뇨병성 자율신경병증 평가
* 치료에 반응하지 않는 경우 신경과 의뢰

## <mark style="color:green;">임상 양상</mark>

#### **심혈관계**

* 기립성 저혈압 : 가장 흔하고 임상적으로 중요; 어지럼, 실신, 낙상 (☞ [기립성 저혈압](/broken/pages/eeb73d5aba738810ad368e9404714a6e46abc343))
* 운동 불내성(exercise intolerance) : 운동 시 적절한 심박수 증가 불가
* 앙와위 고혈압 : 기립성 저혈압과 역설적으로 동반될 수 있음

#### **발한·분비**

* 발한 장애(과다 또는 저하), 침 분비 감소, 눈물 분비 감소

#### **소화기계**

* 소화 운동 감소, 위마비(gastroparesis), 식욕 부진, 팽만감, 변비, 설사, 삼킴곤란

#### **비뇨기계**

* 배뇨 지연, 요실금, 불완전 배뇨, 빈뇨, 야간 배뇨

#### **성기능**

* 발기 부전, 사정 장애, 질 건조, 오르가즘 장애

#### **기타**

* 수면 장애, 시야 흐림·동공 반사 저하, 피로, 인지 저하(POTS에서 두드러짐)

## <mark style="color:green;">진단</mark>

### <mark style="color:$primary;">검사</mark>

#### 혈압 측정

* 기립성 저혈압 측정 : 앙와위 후 1·3·5분 혈압/맥박 측정; 수축기혈압 ≥20 mmHg 또는 이완기혈압 ≥10 mmHg 감소 시 양성
  * ✽고혈압 동반 환자 : 수축기혈압 ≥30 mmHg 감소를 기준으로 적용하기도 함
  * ✽**지연성 기립성 저혈압(Delayed OH)** : 기립 후 3분 이내는 정상이나 3\~30분 사이에 혈압이 기준치 이상 감소하는 경우; 초기 자율신경 부전의 징후일 수 있으므로 증상이 있는 경우 10분 이상 측정 권고
* 10분 기립검사 (POTS 선별) : 기립 후 10분 내 심박수 ≥30회/분 증가 시 양성

#### 전문 검사 (의뢰)

* Heart rate variability (HRV) test : 심호흡 시 심박수 변동 평가
* Valsalva response test : 자율신경 반사 평가
* Sudomotor function test (QSART) : 발한 기능 평가
* Tilt table test : POTS·기립성 저혈압 확진

#### 기저 질환 감별 검사

* 혈당/HbA1c, TSH, 전해질, CBC, B12
* 필요 시 : 항ganglionic AChR 항체(자가면역 자율신경절병증), 부종양 항체

### <mark style="color:$primary;">감별 진단</mark>

* 신체증상장애 : 객관적 자율신경 검사 정상; 증상에 대한 과도한 집착; 심리적 요인
* 불안장애/공황장애 : 발작성; 심계항진·발한이 두드러짐; 지속적 자율신경 이상 없음
* 빈혈·탈수 : 기립성 저혈압 유발; 혈액 검사로 확인
* 약물 부작용 : 항고혈압제, 이뇨제, α차단제 등

***

## <mark style="background-color:$warning;">Management</mark>

### <mark style="color:$primary;">치료 방침</mark>

* 원인 질환 치료가 우선 - 기저 질환 조절로 증상 호전 가능
* 증상별 대증 치료 병행
* 비약물 치료를 기본으로 하고 필요 시 약물 추가

### <mark style="color:$primary;">비약물 치료</mark>

**기립성 저혈압 / POTS**

* 수분 섭취 증가 (2\~3 L/일), 염분 섭취 증가 (6\~10 g/일)
* 압박 스타킹 착용 (하지 정맥 귀환량 증가)
* 기상 시 천천히 일어나기; 오랜 기립 피하기
* 소량 자주 식사 (식후 저혈압 예방)
* 규칙적인 운동 (수평 운동 우선 - 수영, 누워서 하는 저항 운동)
* 고온 환경·알코올 회피

**소화기 증상**

* 소량 자주 식사, 저지방·저섬유 식이(위마비 시)
* 취침 시 상체 거상

**비뇨기 증상**

* 정해진 시간 배뇨 훈련, 이중 배뇨법(배뇨 후 잠시 후 재시도)

### <mark style="color:$primary;">약물 치료</mark>

**기립성 저혈압**

* fludrocortisone : 혈장량 증가; 0.1\~0.3 ㎎/d; \[부작용] 앙와위 고혈압, 저칼륨혈증, 부종
* midodrine : α1 작용제; 2.5\~10 ㎎ tid (취침 전 복용 금지); \[부작용] 입모근 반응, 앙와위 고혈압
* pyridostigmine : 콜린에스터라제 억제; 30\~60 ㎎ bid\~tid; 특히 POTS에서 고려

**POTS**

* β차단제(저용량) : propranolol 10\~20 ㎎; 심박수 조절
* ivabradine : If 채널 차단; 심박수 감소; β차단제 불내성 시 대안

**소화기 증상**

* metoclopramide, domperidone : 위장관 운동 촉진 (단기)

**기저 질환 치료**

* 당뇨병성 자율신경병증 : 혈당 조절 최적화 (☞ [당뇨병](/broken/pages/366b8c4562fe9dcc3d5530cb7d5d419dc6c169aa))
* 알코올 남용 : 금주 (☞ [알코올 사용 장애](/broken/pages/5cb121983d66199103514aff8be5ed9499982850))
* 파킨슨병 (☞ [파킨슨병](/broken/pages/05fe549923b096359980ddf3606920891dce3ac5))

***

### <mark style="color:purple;">질병코드</mark>

G90 자율신경계통의 장애

F45.3 신체형자율신경기능장애

***

## <mark style="color:orange;">처방례</mark>

### <mark style="color:$primary;">기립성 저혈압</mark>

{% tabs %}
{% tab title="기본" %}
* 수분 2\~3 L/일, 염분 6\~10 g/일 섭취 교육
* 압박 스타킹 착용
* midodrine 2.5\~5 ㎎ tid (취침 3\~4시간 전 마지막 복용)
{% endtab %}
{% tab title="혈장량 부족 동반" %}
* fludrocortisone 0.1 ㎎/d → 반응 불충분 시 0.2\~0.3 ㎎/d로 증량
* 전해질(K⁺) 및 혈압 모니터링
{% endtab %}
{% endtabs %}

### <mark style="color:$primary;">POTS (체위성 빈맥 증후군)</mark>

{% tabs %}
{% tab title="기본" %}
* 수분·염분 섭취 증가 + 압박 스타킹
* propranolol 10\~20 ㎎ bid (저용량; 심박수 조절 목적)
{% endtab %}
{% tab title="β차단제 불내성" %}
* ivabradine 2.5\~5 ㎎ bid
{% endtab %}
{% tab title="콜린에스터라제 억제" %}
* pyridostigmine 30\~60 ㎎ bid\~tid
{% endtab %}
{% endtabs %}

***

{% hint style="info" %}
**자율신경 기능 장애, 약보다 생활 습관이 먼저입니다**

어지럼증과 두근거림이 심해도 수분과 염분을 충분히 섭취하고, 천천히 일어나는 습관만으로도 상당히 호전될 수 있습니다. 약은 생활 습관 교정을 충분히 실천한 후에도 증상이 지속될 때 추가합니다.
{% endhint %}

### <mark style="color:$primary;">환자 안내</mark>

**일상생활 수칙**

* 아침에 일어날 때 : 누운 채로 발목을 10회 펌핑한 후 → 천천히 앉기 → 30초 후 일어서기
* 물을 하루 2\~3컵 더 마시고, 음식을 약간 짜게 드셔도 됩니다 (단, 심부전·신부전이 있으면 의사와 상의)
* 압박 스타킹은 아침에 누운 채로 착용하세요
* 오랫동안 서 있거나 더운 환경(목욕탕, 사우나)은 피하세요
* 식사는 소량씩 자주 (한 번에 많이 먹으면 식후 혈압이 떨어질 수 있습니다)
* 규칙적인 운동은 수영이나 누워서 하는 운동부터 시작하세요

**이런 증상이 생기면 바로 병원으로**

* 일어서다가 의식을 잃거나 쓰러진 경우
* 심한 두근거림과 함께 가슴 통증이 생긴 경우
* 증상이 갑자기 심해진 경우

***
