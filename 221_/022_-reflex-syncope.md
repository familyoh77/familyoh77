...
* 재발성 VVS가 있는 42세 이상에서 제한적으로 고려 가능하지만 근거가 일관되지 않음. 소아·청소년에서는 유익성 없음 \[ACC/AHA/HRS 2017]
* VVS 치료 목적으로 routine 처방하지 않음 (VVS에 급여적응증 없음)
...
* fluoxetine의 효과 가능성은 추가 검증이 필요하며, paroxetine이 VVS 재발을 증가시킨다는 근거는 확립되지 않음 (VVS에 급여적응증 없음)
...
* Tilt 유발 교차연구(POST VI, Eur Heart J 2019)와 소규모 파일럿 RCT(Tajdini 등, 2021, n＝46)에서 실신·전실신 감소가 보고되었으며, 특히 저혈압 우세 표현형에서 효과 가능성이 시사됨
...
> ```
> 유발 인자 회피 및 재발 대처 교육
> 수분 ≥2 L/d, 식염 총 6~9 g/d 섭취 (평소 식사 포함, 개별화)
...
> ※ 앙와위·기립 혈압과 배뇨 증상 확인. 수분·식염 섭취 및 counter-pressure maneuver 교육 병행. VVS는 국내 허가효능에 별도 적응증으로 명시되어 있지 않음
> ```

> **처방례 3.** 저-정상 혈압의 재발성 VVS - midodrine 대안
>
> ```
> 플로리네프 0.1 ㎎/T  0.5T  조식 후  (반응에 따라 1T으로 조정 고려)
...
>
> ```
> 푸로작 10 ㎎/C  1C  조식 후 1주간, 이후 내약성·혈압을 확인하여 20 ㎎/d로 증량 고려
