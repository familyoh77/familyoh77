...
* 임상 아형 : 아형만으로 원인질환이나 진행 방향을 확정할 수 없음
  * 기억상실형(amnestic) 또는 비기억상실형(non-amnestic) : 알츠하이머병과 연관되는 경우가 많음&#x20;
  * 단일영역(single-domain) 또는 다영역(multiple-domain) : 혈관성 인지장애·레비소체병·전두측두엽변성 등에서 나타날 수 있음
...
{% hint style="info" %}
2024 Lancet Commission은 생애주기 전반의 14개 교정 가능한 치매 위험요인을 제시함 (괄호 : 인구기여위험도). 14개 요인 전체는 치매의 약 45%를 설명하는 것으로 추정됨.\
...
* 시간\~수일 내 갑자기 발생하거나 하루 중 변동하는 인지·주의력·의식 변화  `섬망` `약물독성` `대사성뇌병증` `감염`&#x20;
...
* 최근 시작·증량한 약물 이후 뚜렷한 인지 악화  `약물유발 인지장애`
* 학대·방임·금전 착취가 의심되거나 혼자 지내기 위험한 상태
...
* 알츠하이머병 바이오마커 검사 또는 항아밀로이드 치료 적격성 평가가 필요한 경우<mark style="color:green;">단</mark>
...
* 인지·기능 평가
  * 검증된 인지 선별검사와 함께 [IADL](../231_/215_-elderly.md#adl-iadl)을 평가
  ...
  * CIST와 MMSE 검사는 MCI 확진 검사가 아니며, 특히 K-MMSE-2는 정상 노화와 MCI의 구분 민감도에 한계가 있음
  * CIST 또는 MMSE가 정상 범위이더라도 환자·보호자가 분명한 변화를 호소하거나 병전기능이 높다면 K-MoCA 또는 정밀신경심리검사(SNSB, CERAD-K)를 고려
* 기본 검사
...
#### <mark style="color:$primary;">한국판 몬트리올 인지평가(</mark>[<mark style="color:$primary;">K-MoCA</mark>](033_-dementia.md#k-moca)<mark style="color:$primary;">)</mark>
...
* 2026년, 인지적으로 건강한 19\~90세 성인 1,177명의 연령·교육수준별 K-MoCA 총점, 6개 영역 지수 및 K-MoCA-22 규준이 제시 됨. 연령과 교육은 총점 변이의 약 66%를 설명했으며, 가능하면 이 연령·교육수준별 규준을 참고. \[Kang et al. Dement Neurocogn Disord 2026;25(3):164]
  * 적용 예시 : 환자의 연령·교육수준에 해당하는 규준군의 평균·표준편차 또는 제시된 백분위와 비교. 낮은 백분위만으로 MCI를 진단하지 않고, 보호자 병력·IADL·인지영역별 양상과 함께 판단
  * 이 연구는 연령·교육수준별 규**준** 연구이지 MCI 진단 절단점 연구가 아니므로, 규준표의 특정 점수를 보편적인 정상/MCI 절단점으로 사용하지 않음
* 국내에는 서로 다른 한국판이 공존한다 : K-MoCA(Kang 2009, 한림대)와 MoCA-K(Lee 2008, 서울대; MCI 선별 절단점 22/23). 판본마다 문항·규준·절단점이 다르므로 사용한 판본의 기준만 적용한다.
...
{% hint style="warning" %}
MoCA는 저작권이 있는 검사로, 공식 정책상 웹사이트에 검사지나 시행 지침을 게시하지 않으며, 출판물 수록에는 별도 허가가 필요. 시행·채점자는 공식 교육·인증 요건을 확인
{% endhint %}
...
* 아밀로이드 양성은 인지 기능이 정상인 고령자에서도 흔함(50세 약 10%, 90세 약 44%) \[JAMA 2015]
  * 바이오마커 양성만으로 현재 인지 증상의 원인을 알츠하이머병으로 단정하지 않고, 임상 양상·영상 소견과 부합하는지 함께 판단해야함. 반대로 검증된 검사의 음성 결과는 알츠하이머병을 원인에서 배제하는 데 유용
...
* 약물·물질, [수면무호흡증](029_-insomnia-sleep-disorder.md#obstructive-sleep-apnea-osa), 감각 저하, [갑상선기능](../226_/105_-hypothyroidism.md)이상, [Vit B12 결핍](../230_/192_-anemia.md#vitamin-b12-cobalamin-deficiency) 및 기타 전신질환
...
flowchart TD
    ...
    D -->|독립성 저하<br/>또는 비전형| H["<u>치매 또는 비전형·급속 진행</u><br/>당일~수일 내 평가<br/>전문의 의뢰"]
...
* [코그테라](https://www.emocog.com/ko/)(Cogthera)
  ...
  * 확증임상에서 치료 반응률 59.2%로 대조군(28%)보다 높았으며, 이상사례 빈도는 군간 차이가 없었음
  * 보건복지부 고시 제2026-114호에 따라 2026년 6월 1일부터 건강보험 비급여 항목 TX013으로 적용
...
* 2026년 제출된 국내 퇴행성·혈관성 MCI 임상재평가(852명 통합 분석, 3건의 임상 중 2건)의 전체 분석에서 사전 지정 1차 유효성 평가변수(인지기능 유지·개선 비율)는 통계적 유의성 기준을 충족하지 못함. 일부 사전 계획 보조분석·순응군 분석에서 개선 신호가 있었으나 해석 논쟁이 있으며, 식약처의 최종 판단은 2026년 하반기 발표 예정
* [급여기준](https://www.hira.or.kr/rc/insu/insuadtcrtr/InsuAdtCrtrPopup.do?mtgHmeDd=20181201\&sno=1\&mtgMtrRegSno=0007) : 치매 외 적응증인 MCI(F06.7)에서는 2025년 9월 21일부터 선별급여 본인부담 80%가 적용되므로 처방 전 비용과 근거의 불확실성을 설명. 경구용 뇌대사개선제는 원칙적으로 1종만 급여가 인정되어 병용 처방 제한
* 투여하기로 한 경우 치료 목표와 평가 시점을 미리 정하고 객관적·기능적 이득이 없거나 이상반응이 있으면 중단을 고려

### <mark style="color:orange;">항아밀로이드 단클론항체</mark>

* 일반 MCI가 아니라 아밀로이드 병리가 확인된 알츠하이머병으로 인한 MCI 또는 경증 알츠하이머병 치매가 치료 대상이다.
...
* ARIA-E, ARIA-H, 대뇌출혈과 주입반응이 발생할 수 있고 드물게 중증·치명적일 수 있다. 치료 전 MRI와 미세출혈·표재철침착 등 위험 평가, 정기 MRI, 항응고제·혈전용해제 관련 출혈 위험 검토 및 APOE ε4 유전자형 상담이 필요하다.
  * 특히 APOE ε4/ε4 동형접합자는 비보유자·이형접합자보다 ARIA-E와 중증·증상성 ARIA 위험이 높으므로, 치료 전에 유의한 위험 증가와 대안에 대해 충분히 설명하고 공동 의사결정한다. 필요하면 유전상담이 가능한 인지신경 전문기관에 의뢰한다.
  * APOE 검사는 일반 MCI의 치매 전환을 예측하기 위한 일률적 검사가 아니라, 주로 항아밀로이드 치료 전 ARIA 위험 설명과 의사결정에 활용한다.
* lecanemab \[레켐비] : 국내에서 알츠하이머병으로 인한 MCI 또는 경증 알츠하이머병 치매 성인 환자 치료제로 허가·출시. 2026년 9월 2일 기준 건강보험 비급여이며 약제 외에도 아밀로이드 확인검사·MRI·주입 및 모니터링 비용을 함께 설명한다.
* donanemab \[키썬라] : 미국 등 해외 허가(유럽은 APOE ε4 비보유자·이형접합자로 적응증 제한); 국내 미허가(2026년 9월 2일 기준, 식약처 허가 심사 중이며 연내 출시는 업계 전망). 국내 허가 시 적응증 범위를 확인한다.
...
> * 치매 진단이 없는 경우 약값의 80%를 본인이 부담하며, 뇌대사개선제는 원칙적으로 한 가지만 보험이 인정됩니다.
