# 긴장형두통 Tension Type Headache, TTHA

## <mark style="color:green;">일반 사항</mark>

* 다음의 특징을 갖는 두부 양측의, 압박 또는 조이는 느낌의 두통
  * 중등도 이하의 강도
  * 감정적 스트레스, 어깨/목 근 긴장과 관련
  * 동반 증상 : 두부/경부/어깨 근육 경직/압통, 피로, 과민, 집중력 저하
  * 성인 유병률 약 26%; 질병부담(years lived with disability, YLD)은 15\~49세에서 최고. 연령 증가에 따라 episodic type은 감소, chronic type은 증가
* 원발성 두통의 가장 흔한 형태임.
* 두통 일기(headache diary) : 진단 분류와 치료 반응 평가에 가장 유용한 도구. 초진 시 작성을 권고하고, 1개월 후 재평가에 활용

### <mark style="color:$danger;">🚩 Red Flags!</mark>

☞ [두통 Red Flags](015_-headache.md#red-flags) 참조

### <mark style="color:orange;">분류 \[IHS classification ICHD-3]</mark>

1. Infrequent episodic TTHA
   1. associated with pericranial tenderness
   2. not associated with pericranial tenderness
2. Frequent episodic TTHA
   1. associated with pericranial tenderness
   2. not associated with pericranial tenderness
3. Chronic TTHA
   1. associated with pericranial tenderness
   2. not associated with pericranial tenderness
4. Probable TTHA
   1. probable infrequent episodic tension type headache
   2. probable frequent episodic tension type headache
   3. probable chronic tension type headache

## <mark style="color:green;">원인</mark>

* 불명

### <mark style="color:orange;">추정 기전</mark>

* peripheral nociceptors 활성 → myofascial pain → episodic TTHA
*   지속적인 nociceptors 자극 → central pain pathway 민감화

    → chronic TTHA

### <mark style="color:orange;">위험 인자</mark>

**주요 위험 인자** (근거 강함, 임상적 영향 큼)

* 정신적 스트레스 : 가장 강력한 단일 유발인자; 만성화와도 직결
* 불안, 우울 : 만성화의 독립적 예측인자; 동반 시 치료 반응도 저하
* 수면 장애 : 수면 부족/과다 모두 위험; 만성화 기여
* 나쁜 자세 / 장시간 고정 자세 : 현대 직장인에서 가장 흔한 구조적 원인; 경부·어깨 근긴장과 직접 연결

**중간 위험 인자** (근거 중등도, 교정 가능)

* 식사 거름 / 탈수 : 혈당·수분 변화가 두통 역치 저하; 교정이 쉬워 우선 지도
* 육체적 과로 : 근긴장 직접 유발
* DJD, 턱관절 이상, 이갈이 : 근막 긴장의 구조적 원인; 별도 치료 필요
* 여성 호르몬 : 월경 주기 연관성 있으나 조절 어려움

**부수적 위험 인자** (근거 약함 또는 영향 적음)

* 특정 음식 (카페인, 알코올, 초콜릿) : 편두통보다 연관성 약함; 개인차 큼
* 흡연 : 간접적 영향
* 냄새, 소음, 빛 : 편두통에서 더 두드러짐; TTHA에서는 근거 약함
* 약물 (항고혈압제, nitrate, SSRI 등) : 빈도 낮으나 간과하기 쉬움

## <mark style="color:green;">임상 양상 및 진단</mark>

### <mark style="color:orange;">Episodic tension-type headache (삽화긴장형두통)</mark>

A. 아래의 진단 기준 B\~D를 충족하는 두통이 다음 기간 동안 최소한 10번 발생

* Infrequent(저빈도) : 월평균 ＜1일, 연간 ＜12일 발생
* Frequent(고빈도) : ＞3개월 동안 월평균 1\~14일, 연간 12\~179일 발생

B. 지속 시간 : 30분\~7일

C. 다음 네 가지 두통 특성 중 ≥2가지 해당

⓵ 양측\
⓶ 압박 또는 조임(비-박동성)\
⓷ 경증\~중등증 강도\
⓸ 걷기, 계단 오르기 등 일상적 신체 활동에 의해 악화되지 않음

D. 다음 모두에 해당

⓵ 구역 또는 구토 없음\
⓶ 빛공포증 또는 소리공포증 모두 없거나 하나만 존재

E. 다른 ICHD-3 진단으로 더 잘 설명되지 않음\*

_\*개연편두통과 저빈도삽화긴장형두통의 진단 기준 모두를 만족하는 두통은 저빈도삽화긴장형두통으로 진단_

### <mark style="color:orange;">Chronic tension type headache (만성긴장형두통)</mark>

A. 아래의 진단 기준 B\~D를 충족하는 두통이 ＞3개월 동안 월평균 ≥15일, 연간 ≥180일 발생

B. 지속 시간 : 수 시간\~수 일, 또는 지속(unremitting)

C. 다음 네 가지 두통의 특성 중 ≥2가지 해당

⓵ 양측\
⓶ 압박 또는 조임(비-박동성)\
⓷ 경증\~중등증 강도\
⓸ 일상적 신체 활동(예: 걷기, 계단 오르기)에 의해 악화되지 않음

D. 다음 모두에 해당

⓵ 빛공포증, 소리공포증 또는 경미한 구역 중 모두 없거나 하나만 존재\
⓶ 중등도 이상의 구역이나 구토는 없음

E. 다른 ICHD-3 진단으로 더 잘 설명되지 않음<sup>1\~3)</sup>

_1) 한 달에 25일간 두통이 있는 어떤 환자에서 8일은 편두통에 부합되고 17일은 긴장형두통에 부합된다면 이 환자는 두 진단 기준 모두에 해당되지만 만성편두통으로만 진단함_\
_2) 진단 기준 A\~E를 만족하는 두통이 처음 발생 시점 24시간 이내부터 매일 지속된 것이 명백하다면 신생매일지속두통으로 분류하며, 두통 발생시점을 기억하지 못하거나 불확실하다면 만성긴장형두통으로 분류_\
_3) 약물과용두통과 만성긴장형두통의 진단 기준을 모두 충족하는 경우 둘 다 진단함. 약물 과용을 중단한 후에도 만성 두통이 지속되면 약물과용두통 진단은 철회될 수 있음_

#### <mark style="color:$primary;">두개 주위 근육 촉진 (Pericranial Palpation)</mark>

* ICHD-3는 긴장형두통을 두개 주위 근육 압통 유무에 따라 하위 분류함. 촉진은 진단의 객관성을 높이고 치료 전후 비교에 활용; 단, 압통이 있다고 해서 진단의 필수 조건은 아님
* 검사 부위 : 전두근(Frontal), 측두근(Temporal), 교근(Masseter), 흉쇄유돌근(SCM), 두판상근(Splenius), 승모근(Trapezius)
* 검사 방법 : 2nd, 3rd finger로 작은 원을 그리듯 회전하며 강하게 압박 (수기 촉진법 표준)
* 점수화 : 각 부위를 0\~3점으로 점수화(Local tenderness score)하여 기록 → 치료 전후 객관적 비교 가능

### <mark style="color:orange;">긴장형두통 vs 경부인성두통 (CGH) 감별</mark>

<table><thead><tr><th width="106">구분</th><th>긴장형두통 (TTHA)</th><th>경부인성두통 (CGH)</th></tr></thead><tbody><tr><td>분류</td><td>원발성 두통</td><td>2차성 두통 (경추 질환 기인)</td></tr><tr><td>통증 부위</td><td>양측성 (전두부, 측두부, 후두부 전체)</td><td>심한 단측성 (고정된 한쪽)</td></tr><tr><td>통증 양상</td><td>압박감, 조이는 느낌 (Band-like)</td><td>비박동성, 후두부→안구 방사</td></tr><tr><td>발생 기전</td><td>근육 긴장, 스트레스, 중추 감작</td><td>C1~C3 신경근 연관통 (Referred pain)</td></tr><tr><td>유발 요인</td><td>스트레스, 피로, 수면 부족</td><td>목의 움직임, 특정 자세, 경추 압박</td></tr><tr><td>경추 가동 범위</td><td>대개 정상 (근육 경직은 있을 수 있음)</td><td>ROM 감소 뚜렷함</td></tr><tr><td>동반 증상</td><td>빛/소리 공포(드묾), 구역(없음)</td><td>같은 쪽 어깨/팔 통증, 어지럼증</td></tr><tr><td>진찰 소견</td><td>두개 주위 근육의 광범위한 압통</td><td>상부 경추(C1~C3) 분절 가동성 저하</td></tr><tr><td>확진적 검사</td><td>임상적 진단 (신경학적 검사 정상)</td><td>신경 차단술(Block) 시 통증 즉시 소실</td></tr></tbody></table>

### <mark style="color:orange;">긴장형두통 vs 편두통 감별</mark>

<table><thead><tr><th width="120">구분</th><th>긴장형두통 (TTHA)</th><th>편두통 (Migraine)</th></tr></thead><tbody><tr><td>통증 부위</td><td>양측성 (전두부·측두부·후두부 전체)</td><td>편측성 (60~70%; 양측도 가능)</td></tr><tr><td>통증 양상</td><td>압박감, 조이는 느낌 (Band-like, 비박동성)</td><td>박동성 (Pulsating)</td></tr><tr><td>통증 강도</td><td>경증~중등증</td><td>중등증~중증</td></tr><tr><td>신체 활동</td><td>악화 없음</td><td>일상 활동으로 악화</td></tr><tr><td>구역/구토</td><td>없음</td><td>흔함</td></tr><tr><td>빛·소리 공포</td><td>없거나 하나만 (경미)</td><td>양쪽 모두 흔함</td></tr><tr><td>전조 증상</td><td>없음</td><td>약 25%에서 전조(시각·감각·언어) 동반</td></tr><tr><td>지속 시간</td><td>30분~7일</td><td>4~72시간</td></tr><tr><td>트립탄 반응</td><td>일반적으로 효과 제한적 (반응 시 편두통 요소 혼재 재평가)</td><td>효과적</td></tr><tr><td>악화 요인</td><td>스트레스, 수면 부족, 근긴장</td><td>호르몬 변화, 특정 음식, 빛·소음</td></tr></tbody></table>

※ 혼합형 주의 : 두 진단이 동일 환자에서 공존할 수 있음. 트립탄에 반응하는 두통은 편두통 요소가 혼재될 가능성이 높으므로 편두통으로 재평가

## <mark style="background-color:$warning;">Management</mark>

### <mark style="color:orange;">치료 원칙</mark>

{% hint style="info" %}
**Clinical Pearl** : 양측성·압박성·활동으로 악화되지 않고 구역·구토가 없는 두통 → 긴장형두통을 우선 고려
{% endhint %}

* 삽화형 : 급성기 진통제 투여 + 유발 인자 교정
* 만성형 : 예방 약물(항우울제) + 비약물 치료 병행; 급성기 진통제 병용 가능
* 약물과용두통 주의
  * ⚠️ **진통제 남용은 두통을 '치료'하는 것이 아니라 오히려 '유발'한다** — 잦은 진통제 사용이 반발성 두통 또는 약물과용두통으로 이어질 수 있음 (특히 카페인 함유 복합제)
  * 단순 NSAIDs/acetaminophen은 월 15일, 복합진통제는 월 10일 초과 시 약물과용두통 기준에 해당됨
  * 약물에 의한 두통 발생을 예방하기 위해 진통제 사용을 ≤2일/주로 제한할 것을 권고

## <mark style="color:green;">비약물 치료</mark>

* 규칙적 식사, 균형 잡힌 식사; 아침 식사를 거르지 않음
* 충분한 휴식/취침, 규칙적 취침/기상; 소음이 적은 어두운 방에서 휴식
* 스트레스 관리 : 가벼운 산책을 포함하여 작업에서 잠시라도 벗어남
* 측두부/뒷목에 대한 온/냉찜질 및 마사지, 뜨거운 목욕/샤워
* 스트레칭 : 어깨, 목의 긴장을 풀어주는 스트레칭(수시 또는 아침, 저녁 각 15분 이상)
* 규칙적 운동 : 유산소 운동 및 근력 강화 운동 30\~60분씩 주 4\~6회
* 자세 교정 : 거북목(Forward head posture) 교정; 모니터 높이를 눈높이에 맞추고, 화면과의 거리 50\~70 cm 유지; 장시간 스마트폰 사용 시 시선을 아래로 향하지 않도록 주의
* 이완 요법 : 요가, 명상, 인지행동치료(CBT), 바이오피드백
* 향기 요법 : 페퍼민트 oil을 전두부에 도포(일부에서 유효)

## <mark style="color:green;">약물 치료</mark>

※ 편두통 치료제는 순수 TTHA에 효과 없음

* CGRP 표적 치료제(항CGRP 단클론항체, gepant 계열) : 편두통에는 승인되어 있으나 순수 긴장형두통에는 효과 미입증; 무분별한 처방 주의
* Triptan : 트립탄에 반응하는 두통은 긴장형두통이 아니라 편두통 요소가 혼재되어 있을 가능성이 매우 높음 → 편두통 재평가 필요

### <mark style="color:orange;">1차 선택 - 급성기 진통제</mark>

* ibuprofen : 400\~800 ㎎ q8h 필요시 <mark style="color:blue;">\[부루펜]</mark>
  * ibuprofen이 acetaminophen보다 효과적인 경향이 있음 (다만 effect size는 크지 않음)
* naproxen : 275\~500 ㎎ q8\~12h 필요시 <mark style="color:blue;">\[낙센, 아나프록스]</mark>
* acetaminophen : 1,000 ㎎ → 이후 650\~1,000 ㎎을 필요시 6시간마다; 최대 4 g/d, 간/신 장애 시 2 g/d <mark style="color:blue;">\[타이레놀]</mark>
* aspirin : 500\~1,000 ㎎ 필요시 6시간마다 <mark style="color:blue;">\[로날]</mark>

### <mark style="color:orange;">2차 선택 - 급성기</mark>

#### <mark style="color:$primary;">Caffeine 복합제</mark>

* caffeine 130/acetaminophen 500/aspirin 500 ㎎ q6h 필요시
  * ⚠️ 효과는 있으나 **1차로 권장되지 않음**; 약물과용두통 위험이 단순 진통제보다 현저히 높으므로 사용 빈도를 엄격히 제한 (월 10일 이내)

#### <mark style="color:$primary;">Opioid 유사제</mark>

* ⚠️ **원칙적으로 사용 권장되지 않음** — 만성화 및 약물과용두통(MOH) 위험이 매우 높음; 다른 모든 치료 옵션을 시도한 후 예외적 상황에서 단기·제한적으로만 고려
* tramadol : 100 ㎎ <mark style="color:blue;">\[트리돌]</mark> (☞ [opioids](../220_/001_-pain.md#opioids))
* 복합제 : tramadol/acetaminophen <mark style="color:blue;">\[울트라셋]</mark>

#### <mark style="color:$primary;">진통 보조제</mark>

* 수면 작용이 있는 항히스타민제나 dopamine 차단 항구토제가 진통제의 효과를 강화시킬 수 있음
  * diphenhydramine : 25\~50 ㎎ q4\~6hr, 일반적으로 150 ㎎/d 이하 권장 [디펙타민](../%EB%B9%84%EB%B3%B4%ED%97%98/)
  * hydroxyzine : 25\~50 ㎎ hs or 50\~100 ㎎/d #3\~4 <mark style="color:blue;">\[아디팜]</mark>
  * metoclopramide : 5\~10 ㎎ tid\~qid <mark style="color:blue;">\[맥페란]</mark> — TTH에서는 **구역 동반 시에 한해** 보조적으로 사용; 일반적인 진통 보조 목적의 routine 사용은 근거 제한적

### <mark style="color:orange;">예방 치료 - 만성긴장형두통</mark>

※ 다음 중 하나에 해당할 때 예방 치료 고려. 최소 2\~3개월 투여 후 효과 평가

* 만성 긴장형두통 (월 ≥15일 발생)
* 고빈도 삽화형 (월 8\~14일 발생) 중, 두통으로 인한 일상생활 장애가 크거나 급성기 약물 효과가 부족한 경우
* 진통제 사용 빈도가 잦아 약물과용두통으로 진행될 위험이 높은 경우

#### <mark style="color:$primary;">1차 - 항우울제</mark>

* 근거 수준 가장 높음
* amitriptyline : 10 ㎎/d 취침 시 시작 → 25\~75 ㎎/d까지 점진 증량 <mark style="color:blue;">\[에트라빌]</mark> ← 만성 TTHA 예방 1차 선택 (2024 VA/DoD 가이드라인)
  * 부작용 : 구갈, 졸림, 변비, 체중증가 (항콜린 작용); 취침 전 복용으로 완화
  * 항콜린 부작용이 심해 순응도가 떨어질 경우 → nortriptyline으로 교체 고려 : 10\~50 ㎎/d 취침 시 <mark style="color:blue;">\[센시발]</mark> (항콜린 부작용 상대적으로 적음)
* mirtazapine : 15\~30 ㎎/d 취침 시 <mark style="color:blue;">\[레메론]</mark>
  * amitriptyline 부작용으로 전환 시 대안; 수면 개선 효과 동반
* venlafaxine XR : 37.5 ㎎/d 시작 → 75\~225 ㎎/d <mark style="color:blue;">\[이팩사 XR]</mark>
  * 우울·불안 동반 환자에서 선호; 혈압 모니터링 필요
  * ✽SSRI는 TCA에 비해 TTHA 예방 근거 부족; 동반 우울/불안 치료 목적으로는 활용 가능

#### <mark style="color:$primary;">기타</mark>

* 국소 주사 : lidocaine, triamcinolone (보험 주의)
  * ✽Dry needling(근막 유발점 주사): 측두근(Temporalis)·승모근(Trapezius) 타겟이 일부 RCT에서 통증 강도 및 빈도 감소 효과 보고. 도수치료(경추 운동, manual therapy) 병행 시 시너지 효과
  * ✽보툴리눔 독소: 만성형에서 시도 보고 있으나 근거 미흡, 비급여
* 근이완제 (근거 제한적; 일부 환자에서 선택적으로 고려)
  * cyclobenzaprine : 5\~10 ㎎ tid, 서방형 15\~30 ㎎ qd <mark style="color:blue;">\[본렉스]</mark>
  * tizanidine : 2\~4 ㎎ hs 또는 bid\~tid; 졸음·저혈압 부작용 주의 (예방 효과 연구 있으나 근거 제한적)
* riboflavin : 200\~400 ㎎/d; 편두통 성향이 혼재된 긴장형두통 환자에서 예방 목적으로 고려 (순수 TTHA에서의 근거는 낮음)

#### <mark style="color:$primary;">2차 / 제한적 선택</mark>

* 근거 부족하나 시도 가능
* topiramate : 25\~100 ㎎/d (소량부터 점진 증량); 일부 만성 TTHA에서 통증 빈도 감소 보고가 있으나 근거 제한적 <mark style="color:blue;">\[토파맥스]</mark>
* Valproates : 만성 TTHA 예방에 사용된 보고가 있으나 부작용(기형 유발, 체중증가, 간독성) 및 근거 수준을 고려하여 신중히 결정

### <mark style="color:orange;">동반 질환 고려</mark>

* 턱관절 장애(TMD) / 이갈이 : 측두근·교근 압통이 심한 경우 구강 내 장치(Splint) 또는 치과 협진 고려; 단순 두통 치료만으로는 호전 제한적
* 수면 무호흡증 : 아침에 심한 만성 두통은 수면 무호흡과의 연관성 반드시 확인 → 수면 다원검사(PSG) 의뢰 고려
* 불안·우울 동반 : 만성화의 독립적 예측인자; 항우울제 선택 시 동반 이환을 고려하여 venlafaxine 또는 mirtazapine 우선 고려

***

### <mark style="color:red;">질병코드</mark>

G44.2 긴장형두통

***

***

## <mark style="color:purple;">처방례</mark>

**처방례 1.** 급성기 - 일반

```
부루펜 400 mg/T  1~2T  필요시 (q8h, 최대 2,400 mg/d)
타이레놀 이알 650 mg/T  1T  필요시 (ibuprofen 효과 부족 또는 위장 장애 시 대안)
```

**처방례 2.** 급성기 - NSAIDs 금기 (위장 장애, 신기능 저하, 임부 등)

```
타이레놀 500 mg/T  2T  필요시 (q6h, 최대 4 g/d)
아디팜 25 mg/T  1T  취침 시  (진통 보조, 수면 개선)
```

**처방례 3.** 만성, 예방 - 일반

```
에트라빌 25 mg/T  1T  취침 시  (필요시 10 mg으로 시작하여 점차 증량)*
아나프록스 275 mg/T  1T  필요시
```

\* **노인 환자** : 항콜린 부작용(구갈, 변비, 섬망 위험) 우려 시 → 5 mg부터 시작하거나, nortriptyline 10\~50 ㎎/d 취침 시로 대체 고려 <mark style="color:blue;">\[센시발]</mark> (항콜린 부작용 상대적으로 적음)

**처방례 4.** 만성, 예방 - amitriptyline 부작용으로 전환 시

```
레메론 15 mg/T  1T  취침 시
아나프록스 275 mg/T  1T  필요시
```

**처방례 5.** 만성, 예방 - 우울·불안 동반 시

```
이팩사 XR 37.5 mg/T  1T  조식 후  (2주 후 75 mg으로 증량 고려; 혈압 모니터링)
아나프록스 275 mg/T  1T  필요시
```

***

### <mark style="color:$success;">핵심 복약 지도</mark>

> **급성기 진통제 (부루펜·타이레놀·낙센 등)**
>
> * 두통이 시작될 때 복용하십시오. 위장 장애가 있는 경우 식후에 드십시오.
> * **진통제는 한 달에 15일(복합제는 10일)을 초과하여 복용하지 마십시오.** 너무 자주 복용하면 약물과용두통이 생겨 오히려 두통이 매일 나타날 수 있습니다.
> * 카페인이 들어간 복합 진통제(예: 게보린·펜잘)는 약물과용두통 위험이 더 높으므로 특히 주의하십시오.

> **예방약 - 에트라빌(amitriptyline) 등 항우울제**
>
> * 취침 전에 복용합니다. 처음에는 소량으로 시작하여 서서히 늘립니다.
> * 두통 예방 목적이므로 우울증이 없어도 처방될 수 있습니다. 용도를 오해하지 마십시오.
> * 입마름·졸음·변비가 생길 수 있습니다. 불편하시면 담당 의사와 상의하십시오.
> * 효과가 나타나기까지 **2\~4주**가 걸립니다. 임의로 중단하지 마십시오.

> **언제 다시 병원을 방문해야 하나요?**
>
> * 두통 빈도나 강도가 점점 심해지는 경우
> * 진통제를 한 달에 15일 이상 복용하게 되는 경우
> * 두통과 함께 팔다리 힘 빠짐·언어 장애·시야 이상이 나타난 경우 — 즉시 응급실 방문

***

### <mark style="color:blue;">환자 안내서</mark>

{% hint style="info" %}
**긴장형두통, 생활 습관 개선이 핵심입니다**

긴장형두통은 스트레스·근육 긴장·수면 부족이 주된 원인이며, 생활 습관 개선과 적절한 진통제 사용으로 대부분 잘 조절됩니다.
{% endhint %}

#### <mark style="color:$primary;">긴장형두통이란 무엇인가요?</mark>

* **뇌에 이상이 생긴 것이 아니라, 근육 긴장과 스트레스에서 비롯되는 두통입니다.** 걱정하지 않으셔도 됩니다.
* 머리 전체를 띠로 조이는 듯한 압박감이 느껴지는 두통으로, 두통 중 가장 흔한 형태입니다.
* 보통 양쪽 머리에서 느껴지며, 오후에 심해지는 경우가 많습니다.
* 구역·구토는 없으며, 운동으로 악화되지 않는 것이 특징입니다.

#### <mark style="color:$primary;">어떻게 치료하나요?</mark>

* **급성기** : 이부프로펜(부루펜), 나프록센(낙센), 아세트아미노펜(타이레놀) 등 진통제를 복용합니다.
* **만성형 예방** : 항우울제(에트라빌 등)를 취침 전 소량 복용하면 두통 발생 횟수를 줄이는 데 효과적입니다.
* **비약물 치료**도 매우 중요합니다(아래 참조).

#### <mark style="color:$primary;">생활 속 실천 사항</mark>

* **목·어깨 스트레칭** : 매일 아침·저녁 15분 이상 어깨와 목 스트레칭을 하십시오. 장시간 같은 자세로 앉아 있다면 30\~40분마다 스트레칭을 해주세요.
* **규칙적 생활** : 수면 시간과 식사 시간을 일정하게 유지하고, 아침을 거르지 마십시오.
* **온열 요법** : 뒷목과 어깨에 따뜻한 찜질이나 마사지가 도움이 됩니다.
* **운동** : 규칙적인 유산소 운동(주 4\~6회)이 만성 긴장형두통 예방에 효과적입니다.
* **스트레스 관리** : 이완 요법, 명상, 산책 등이 두통 예방에 도움이 됩니다.

#### <mark style="color:$primary;">진통제 복용 시 꼭 지켜주세요</mark>

* 진통제는 **한 달 15일(복합제는 10일) 이내**로 사용하십시오.
* 자주 먹어야 할 정도라면 예방 치료가 필요한 상태일 수 있으니 의사와 상의하십시오.

***
