...
<mark style="color:$danger;">**즉각 응급 조치 및 이송**</mark> <mark style="color:$danger;">- 기질적 질환 또는 즉각적 위해 가능성</mark>
...
* 새로운 신경학적 증상(경련, 의식 변화, 보행 장애)이 처음 나타난 경우 - FNSD 진단 전 반드시 기질적 원인 평가
...
* 만성 경과로 기능 장애(직장·가정 생활)가 현저히 악화
...
* 다양한 신체 증상이 지속 또는 재발; 기질적 원인으로 완전히 설명되지 않거나, 증상의 강도·지속·고통이 의학적 소견에 비해 과도함
...
* 실험실/영상 검사 : 감별 진단 및 기초 평가 목적
* 심리 검사 : MMPI(다면적 인성검사), PHQ-15(신체화 증상 중증도 평가)
...
1. "이 증상은 **계속** 있나요, 아니면 **갑자기 몰려오나요**?" → 지속형 = SSD/FNSD, 발작형 = 공황/POTS
2. "**서 있을 때** 더 심해지나요?" → 예 = POTS 의심
3. "검사보다 **증상 자체**가 더 걱정되시나요, **혹시 무슨 병에 걸린 것은 아닐까**가 더 걱정되시나요?" → 전자 = SSD, 후자 = 질병 불안 장애

<table><thead><tr><th width="104">항목</th><th width="140">SSD</th><th width="140">FNSD</th><th width="128">Panic</th><th>POTS (dysautonomia)</th></tr></thead><tbody><tr><td>증상 양상</td><td>만성·다양</td><td>신경학적</td><td>발작성</td><td>기립 관련</td></tr><tr><td>시간 패턴</td><td>지속적</td><td>갑작/변동</td><td>10~30분 peak</td><td>upright 시</td></tr><tr><td>핵심 문제</td><td>반응의 과도성</td><td>기능적 신경 mismatch</td><td>fear circuit</td><td>자율신경 불안정</td></tr><tr><td>Trigger</td><td>stress</td><td>stress</td><td>internal trigger</td><td>standing</td></tr><tr><td>객관적 소견</td><td>거의 없음</td><td>exam inconsistency</td><td>없음</td><td>HR ↑ (≥30 bpm)</td></tr><tr><td>환자의 주된 호소</td><td>"증상이 너무 힘들다"</td><td>"움직일 수 없다"</td><td>"죽을 것 같다"</td><td>"서면 쓰러질 것 같다"</td></tr></tbody></table>
...
{% hint style="info" %}
**혼합형을 놓치지 마세요**\
만성 경과의 신체 증상 환자에서 SSD만 진단하고 치료했는데 호전이 없다면 공황, POTS, 섬유근육통 등 **동반 진단**을 재검토하십시오. 특히 Long COVID 이후 환자에서 자율신경 증상과 SSD가 중첩되는 경우가 증가하고 있습니다.
{% endhint %}
...
PHQ-15 높음 + SSD-12 낮음 = 신체 증상은 많지만 반응이 적응적 → SSD 가능성 낮음\
PHQ-15 낮음~중등도 + SSD-12 높음 = 증상은 경미하나 반응이 과도 → SSD 또는 질병 불안 장애\
PHQ-15 높음 + SSD-12 높음 = 전형적 SSD 양상
{% endhint %}
...
* 증상은 기질적 질환으로 완전히 설명되지 않거나, 증상의 강도·지속·고통이 의학적 소견에 비해 과도함
* **기질적 질환의 존재가 SSD 진단을 배제하지 않음** (예: IBS, 섬유근육통, 만성 편두통과 공존 가능)
...
```mermaid
graph TD
    Start([신체 증상 호소 환자]) --> RedFlag{🚩 Red Flag<br/>확인}
    RedFlag -->|있음| Referral([응급/전문의 의뢰])
    RedFlag -->|없음| BasicEval[기본 평가<br/>병력·약물·정신사회 스트레스<br/>신체진찰<br/>최소 검사: CBC, ESR/CRP,<br/>TSH, 전해질, glucose]
    BasicEval --> Explainable{설명 가능한<br/>의학적 질환?}
    Explainable -->|예| TreatDisease([해당 질환 치료])
    Explainable -->|아니오/불충분| Pattern{증상 패턴<br/>분류}
    Pattern -->|신체 증상 뚜렷<br/>+ 고통/집착 큼| SSD[SSD 의심]
    Pattern -->|증상 경미<br/>+ 병에 대한 공포 중심| IAD[질병 불안 장애]
    Pattern -->|신경학적 deficit<br/>+ inconsistency| FNSD[FNSD]
    Pattern -->|발작성<br/>+ 자율신경 폭발| Panic([공황장애])
    Pattern -->|기립 관련<br/>+ HR 증가| POTS([POTS/Dysautonomia])
    SSD --> Screen[PHQ-15 + SSD-12 평가<br/>A: 증상 심각도<br/>B: 과도한 인지·정서·행동 반응 ≥1]
    IAD --> Screen
    FNSD --> Screen
    Screen --> Framing[진단 설명<br/>❌ 이상 없습니다<br/>✔ 신경계가 과민해져<br/>증상이 실제로 강하게 느껴지는 상태]
    Framing --> Treatment[치료 전략<br/>• 단일 주치의 + 정기 방문<br/>• 기능 회복 중심 목표<br/>• CBT 1차<br/>• SSRI/SNRI, 통증 시 duloxetine<br/>• 불필요 검사/약물 점진적 중단]
    Treatment --> FU[4~6주 단위 추적<br/>기능 개선 여부 확인]
...
<p align="center"><strong>신체증상장애 1차 진료 알고리듬</strong></p>
...
{% hint style="info" %}
**진단 자체가 치료의 절반이다**\
SSD 환자의 치료에서 가장 중요한 단계는 **진단을 어떻게 전달하는가**이다. 첫 1\~2회 방문에서 방향이 결정되며, 잘못된 프레이밍은 만성화와 doctor shopping의 원인이 된다.
{% endhint %}

### <mark style="color:orange;">치료 방침</mark>

* **공감 및 지지** : 기질적 근거가 없는 모든 문제가 정신적 문제는 아님. 환자의 괴로움을 인정하고 라포(rapport) 또는 치료적 제휴 관계를 형성
* **단일 주치의 원칙** : 한 명의 의사가 지속적, 정기적으로 진료; doctor shopping 차단
* **정기 방문 스케줄링** : "증상이 있을 때" 방문이 아닌 **"계획된(scheduled) 방문"** — 4\~6주 간격이 표준
...
* 심리 치료 : **인지행동치료(CBT)가 1차 치료** (가장 근거 수준 높음); 이완 치료, 가족 치료, 운동, 여가 활동
* **iCBT(internet-based CBT)** : 해외에서 효과 입증되고 있으며, 접근성 제한 상황에서 대안으로 주목 — 한국어 플랫폼 확산은 제한적
* 필요시 의뢰 또는 다른 분야(특히 정신과)와 협력

### <mark style="color:orange;">진단 설명의 기술 (Explanation & Framing)</mark>

단순 reassurance("검사 정상입니다")는 오히려 역효과를 낼 수 있다. 환자는 "의사가 내 증상을 찾지 못했다"로 해석하여 불안이 증폭되고 doctor shopping으로 이어진다. **구조화된 설명(structured explanation)** 과 **기능적 진단 프레이밍(functional diagnosis framing)** 이 효과적이다.

#### <mark style="color:$primary;">❌ 금지 문장 vs ✔ 권장 문장</mark>

<table><thead><tr><th width="280">❌ 피해야 할 표현</th><th>✔ 권장 표현</th></tr></thead><tbody><tr><td>"검사 다 정상입니다"</td><td>"검사에는 큰 문제가 없지만, <strong>증상은 분명히 실재</strong>합니다"</td></tr><tr><td>"아무 이상이 없어요"</td><td>"몸의 구조는 괜찮은데, <strong>신경계가 과민해진 상태</strong>입니다"</td></tr><tr><td>"스트레스 때문입니다"</td><td>"스트레스가 <strong>몸과 뇌의 연결</strong>에 영향을 주어 실제 증상을 만들어낼 수 있습니다"</td></tr><tr><td>"정신과 가세요"</td><td>"지금 약물과 생활 관리로 제가 함께 보고, <strong>필요하면 심리치료도 같이</strong> 고려해 봅시다"</td></tr><tr><td>"꾀병 아니에요"<br><em>(부정적 프레이밍은 오히려 의심을 암시)</em></td><td>"이런 증상은 실제 <strong>많은 분들이 경험</strong>하고, <strong>치료로 좋아지는 질환</strong>입니다"</td></tr></tbody></table>

#### <mark style="color:$primary;">Doctor behavior checklist</mark>

<table><thead><tr><th width="50%">✔ 해야 할 것</th><th>❌ 하지 말 것</th></tr></thead><tbody><tr><td>계획된 정기 방문 스케줄링 (4~6주)</td><td>"증상 있을 때 오세요" — 증상-방문 연결 강화</td></tr><tr><td>단일 주치의 유지</td><td>다른 의사/병원으로 쉽게 의뢰</td></tr><tr><td>환자 호소 경청 + 진찰은 꼭 수행</td><td>reassurance 반복 ("아무 이상 없어요" 되풀이)</td></tr><tr><td>기능(일상 활동, 직장 복귀) 체크</td><td>증상 점수만 체크</td></tr><tr><td>증상을 "신경계 과민화"로 설명</td><td>"검사 정상"으로만 설명하고 귀가</td></tr><tr><td>불필요 약물·검사의 점진적 중단</td><td>새 증상마다 새 검사 추가</td></tr></tbody></table>

### <mark style="color:orange;">진료 실패 패턴 — 피해야 할 함정</mark>

1. **과도한 검사의 반복** : 새 증상마다 새 검사 → 환자의 "내가 뭔가 있는 게 분명하다"는 믿음을 강화, 만성화 촉진
2. **Reassurance 루프** : "검사 정상" 반복 → 단기 안도 → 불안 재발 → 재검사 요구 → 반복
3. **진단 회피** : 명확한 진단명 전달을 꺼림 → 환자는 doctor shopping
4. **의사 변경** : 여러 의사가 각자 조금씩 다른 설명 → 환자 혼란, 치료 동맹 형성 실패
5. **신체·정신 이분법** : "몸 아니면 마음" 프레임 → 환자는 "내 증상을 가짜로 본다"고 느낌
6. **단일 진단 고집** : SSD + Panic, SSD + POTS 등 혼합형을 놓침

### <mark style="color:orange;">환자 유형별 접근</mark>

<table><thead><tr><th width="170">환자 유형</th><th width="200">특징</th><th>핵심 대응 전략</th></tr></thead><tbody><tr><td>High utilizer<br>(과다 이용형)</td><td>잦은 방문·응급실 이용·중복 검사</td><td>단일 주치의 + 정기 방문으로 "필요 시 방문"을 대체; 불필요 검사 거절 원칙 공유</td></tr><tr><td>Reassurance seeker<br>(안심 추구형)</td><td>"진짜 이상 없는 거 맞죠?" 반복 질문</td><td>단순 reassurance 대신 구조화된 설명 1회 → 반복 질문에는 "지난번에 함께 확인한 내용입니다" 일관 응답</td></tr><tr><td>Avoidant type<br>(회피형)</td><td>불안이 커서 오히려 진료 회피</td><td>라포 형성 우선; 최소 검사부터 단계적 노출; 진단 지연 위험 주의</td></tr><tr><td>Somatic catastrophizer<br>(재앙화형)</td><td>모든 증상을 최악으로 해석</td><td>인지행동치료(CBT) 의뢰; SSRI/SNRI 조기 시작 고려</td></tr></tbody></table>

### <mark style="color:orange;">질환별 치료 추가 원칙</mark>

**기능성 신경학적 증상 장애(FNSD / 전환장애)**

* "기질적 질환이 아닌 기능성 문제"임을 비대립적으로 설명; "기능성(functional)"이라는 표현이 환자 수용도 높음
* 진찰 소견(예: Hoover's sign 양성)을 환자에게 직접 보여주며 "뇌-근육 연결 문제"로 설명하면 수용도 향상
...
* 1차 약물 : **SSRI 또는 SNRI** (현대 practice의 기본)
* 통증 우세형(pain-predominant) : **SNRI(duloxetine)** 우선 고려
* TCA : 수면 장애 동반 또는 신경병성 통증에 저용량 사용 고려 (부작용 주의)
* Hypochondriasis(질병 불안 장애), dysmorphic disorder : SSRI가 유효
...
* 다음 사항을 반드시 준수
  ...
  * 의존성·내성 형성 위험; 신체증상장애 환자는 **약물 의존 취약군**임을 인지
  * **장기 사용 시 오히려 신체 증상 악화** : GABA 시스템 downregulation, **통각 민감화(hyperalgesia)** 및 interoceptive hypervigilance를 유발하여 장기적으로 통증 역치를 낮추고 신체 감각 모니터링을 증가시킴 — 중추 감작과 반대 방향의 중재 효과
...
* SSD에서는 **우울증 단독 치료 용량보다 훨씬 낮은 저용량(10\~25 ㎎ 시작, 25\~75 ㎎ 유지)** 에서 통증·수면·중추 감작 조절 효과를 얻는 경우가 많음
* 주의 : 고령, 녹내장, BPH, 갑상선항진증, 심혈관 질환, 부정맥, 간질환, 당뇨병, MAOI 투여
* 부작용 : 입마름, 시야 흐림, 변비, 요 정체, 빈맥, 혼돈; 고령자에서 기립성 저혈압·낙상·인지 저하 주의
* amitriptyline : 10\~25 ㎎/d 시작, 통상 유지 25\~75 ㎎/d (취침 시); 우울증 동반으로 증량이 필요하면 100\~150 ㎎/d까지 고려 <mark style="color:blue;">\[에트라빌]</mark>
* nortriptyline : 10\~25 ㎎/d 시작, 25\~75 ㎎/d 유지 <mark style="color:blue;">\[센시발]</mark>; amitriptyline 대비 항콜린 부작용 적어 고령자 선호
* imipramine : 25 ㎎/d 시작, 50\~150 ㎎/d (공황·야뇨 동반 시 고려) <mark style="color:blue;">\[이미프라민]</mark>
...
> **진료를 어떻게 받으면 좋을까요?**
>
> * **한 명의 주치의**에게 꾸준히 진료받으십시오. 여러 병원을 돌아다니면 검사가 중복되고 오히려 회복이 늦어집니다.
> * **정기적으로(4\~6주 간격)** 방문하십시오. "증상이 심할 때만" 병원에 오는 것보다, 증상과 무관하게 계획된 방문을 하는 것이 훨씬 효과적입니다.
> * 진료 시에는 **"이번에 어떤 증상이 새로 생겼는지"** 보다 **"지난 한 달간 내가 얼마나 일상생활을 잘 했는지"** 를 중심으로 이야기해 주십시오.
...
* **약물 치료** : 항우울제가 신체 증상과 동반된 불안·우울을 함께 완화합니다. 효과는 4\~8주에 걸쳐 서서히 나타나며, 증상이 좋아진 뒤에도 최소 6개월 이상 유지 후 의사 지시에 따라 서서히 감량합니다 (갑작스러운 중단 시 어지럼, 저림, 전기 충격 같은 느낌(brain zaps), 구역 등 중단 증상이 나타날 수 있음)
* **심리 치료** : 인지행동치료(CBT)가 가장 근거가 높습니다. 증상에 대한 과도한 걱정을 줄이고 대처 방식을 개선합니다
* **지속적인 진료** : 한 명의 의사에게 꾸준히 진료받는 것이 불필요한 검사를 줄이고 신뢰 관계 형성에 도움이 됩니다
