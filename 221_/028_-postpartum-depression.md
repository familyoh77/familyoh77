...
* 빈도 : \~20%
* 임신 중 발생 : 임신 중 주요우울증 유병률 \~12%; 이 중 5\~6%가 SSRI로 치료받음 (JAMA Psychiatry 2026)
...
* _과거 짧은 수면에도 에너지 유지되는 시기(경조증) 경험_\
  &#xNAN;_&#xC0B0;후 1\~2주 내 급격한 발병_\
  &#xNAN;_&#xC591;극성 장애 가족력_\
  &#xNAN;_&#xD56D;우울제 시작 후 증상 악화 또는 혼재성 양상 출현_\
  &#xNAN;_※ 산후우울증으로 보이는 환자의 일부(일부 연구 최대 \~20%)는 양극성 스펙트럼일 수 있음_
...
## <mark style="color:green;">임신 중 우울증 치료 결정</mark>

* 임신 중 무작위대조군연구(RCT)는 윤리적 제약으로 시행되지 않아, 근거는 관찰 연구에 의존함; 관찰 연구의 핵심 한계는 **적응증에 의한 교란(confounding by indication)** - 즉, SSRI를 지속하는 산모는 대개 중증·재발성 질환을 가진 경우가 많아 결과 차이가 약물 자체가 아닌 기저 질환 중증도를 반영할 수 있음
* 미치료·저치료된 모성 우울증 자체가 조산(OR 1.46), 저체중출생(OR 1.90), 제왕절개(위험차 +3.5%p), 임신오조(OR 5.2), 임신성 고혈압·전자간증(3배) 위험과 연관됨 - SSRI 노출 위험과 반드시 함께 저울질해야 함 (JAMA Psychiatry 2026)
* **심장기형 재평가** : 2000년대 초기 연구는 SSRI(특히 paroxetine) 1분기 노출과 심장기형 위험 2\~3배 증가를 보고했으나, 모성 우울증 자체를 교란변수로 보정한 대규모 코호트에서는 연관성이 소실됨(보정 후 OR 1.06, 95% CI 0.93\~1.22) - 초기 경고의 상당 부분이 우울증 자체의 영향이었을 가능성이 높음
* **자폐스펙트럼장애 등 신경발달장애** : 형제자매 비교 연구를 포함한 보정 후에는 SSRI 노출과의 연관성이 대부분 소실되거나 크게 감소함
* 임신 전부터 SSRI를 복용 중이던 경우, 중단 시 재발 위험이 유의하게 높음 : 전문 주산기 정신건강 프로그램 코호트에서 중단군 재발 위험 HR 5.0(95% CI 2.8\~9.1); 덴마크 코호트에서 중단 시 정신과적 응급상황 위험 HR 1.25(95% CI 1.00\~1.55); 4회 이상 삽화 병력이 재발의 강력한 위험인자
* 상담 원칙 : 미치료 질환의 모체·태아 위험과 SSRI 노출의 잠재적 위험·이익을 균형 있게 제시하는 구조화된 의사결정 과정 권고; "무위험(zero-risk)" 선택지는 없음을 설명
...
Red flag 확인
   ...
   └─ No
        ↓
중증도 평가
   ├─ 경증  → CBT/IPT ± 경과 관찰 (2~4주 후 재평가)
   ├─ 중등증 → SSRI + 심리치료 병행
   └─ 중증  → SSRI ± 비정형 항정신병제 / 정신건강의학과 의뢰
        ↓
2~4주 후 재평가
        ↓
반응 부족 시
...
* **수면 박탈 미개입** → 가장 강력한 가역적 악화 요인을 방치
...
* paroxetine : 모유 이행량 적음; 단, 임신 후기 SSRI 사용 시 신생아 지속성 폐고혈압(PPHN) 위험이 보정 OR 1.28(95% CI 1.01\~1.64)로 소폭 증가할 수 있어 임신 중 신규 시작은 신중 (paroxetine 특이적이 아닌 SSRI 계열 공통 주의사항); 수유 중에는 이 위험 없음
* nortriptyline : TCA 계열은 과량 복용 시 치명적이므로 자살 위험이 있는 경우 1차 선택으로는 피함
* 신생아 모니터링 : 모유 수유 중 항우울제 복용 시 영아의 졸림, 수유 장애, 체중 증가 불량 여부 관찰
* 신생아 적응 증후군(Poor Neonatal Adaptation Syndrome, PNAS) : 임신 후기 SSRI 노출 신생아의 최대 30%에서 발생; 증상 - jitteriness(신경과민), 호흡곤란, 과민성, 빈맥, 수유 곤란; **대부분 자기제한적이며 2주 내 지지치료로 소실**, 장기 발달 이상과의 관련성은 낮음; SSRI+benzodiazepine 병용 시 위험 유의하게 상승 - 병용 시 신생아 관찰 강화
...
* 다음 임신을 계획 중인 경우, 임신 전 중단보다 지속 복용이 재발 위험을 낮춤(중단 시 재발 HR 5.0) - 임신 계획 상담 시 이 근거를 함께 설명
...
#### <mark style="color:orange;">선별 도구</mark> <a href="#undefined-25" id="undefined-25"></a>
...
#### <mark style="color:orange;">아버지 우울증의 특이적 표현</mark> <a href="#undefined-26" id="undefined-26"></a>

* 일반적인 우울감보다 다음 양상으로 나타나는 경우가 흔함. 반드시 확인해야 함
  * 과민성, 분노, 충동적 행동
  * 회피 행동 (귀가 회피, 일중독)
  * 알코올 등 물질 사용 증가
  * 아기 또는 배우자와의 정서적 거리감
...
### <mark style="color:orange;">외래 선별 프로토콜</mark>

```
1단계 : PHQ-2 또는 EPDS 단축형 (3문항)
        ↓ 이상 시
2단계 : EPDS 전체 또는 PHQ-9
        ↓ 양성 시
3단계 : 정신건강의학과 의뢰 또는 적극 개입
```

▶ 부부 동시 선별 강력 권고

<table><thead><tr><th width="160">결과</th><th width="286">대응</th></tr></thead><tbody><tr><td>산모만 양성</td><td>교육 + 추적 관찰</td></tr><tr><td>배우자만 양성</td><td>교육 + 추적 관찰</td></tr><tr><td>둘 다 양성</td><td>적극 개입 - couple-based 접근</td></tr></tbody></table>

### <mark style="color:orange;">치료 접근</mark>

* 약물보다 psychoeducation, 수면 개입, 역할 분담 조정이 우선
* 우울 증상이 지속되거나 중등도 이상 시 약물 치료 고려 (☞ [우울증](027_-depression.md))
...
* 임신 중 우울증을 치료하지 않고 두는 것이 약 복용보다 산모와 아기 모두에게 더 위험할 수 있습니다.
* 예전에는 일부 항우울제가 아기의 심장 기형과 관련 있다고 알려졌으나, 최근 대규모 연구에서는 이 연관성이 우울증 자체의 영향으로 설명되는 경우가 많고, 약 자체의 위험은 처음 알려진 것보다 훨씬 작다는 것이 밝혀졌습니다.
* 임신 후반기에 약을 복용한 경우 아기가 태어난 직후 며칠간 예민하거나 처지는 모습을 보일 수 있으나, 대부분 2주 이내에 특별한 치료 없이 좋아집니다.
* 임신을 계획하고 계시다면, 약을 미리 끊기보다는 담당 의사와 함께 지속 여부를 상의하시는 것이 재발 예방에 도움이 됩니다.
