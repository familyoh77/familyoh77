...
{% hint style="info" %}
전염력은 객담 도말 양성, 공동성 병변, 기침 강도, 균량과 밀접하게 연관된다. PCR(Xpert/NAAT) 양성 단독이면서 도말 음성인 환자는 전염력이 상대적으로 낮을 수 있으나, 최종 판단은 임상 소견을 종합하여 결정한다.
{% endhint %}
...
{% hint style="danger" %}
⚠️ **결핵을 놓치기 쉬운 상황**\
다음과 같은 경우 결핵이 간과되는 사례가 많다. 의심 역치를 낮추고 적극적으로 검사한다.
...
* 체중 감소 없는 초기 결핵
* COVID-19 이후 지속되는 기침·체중 감소·비전형 폐침윤 — post-viral syndrome으로 간주하기 전에 결핵을 배제한다
...
#### <mark style="color:$primary;">Xpert</mark><sup><mark style="color:$primary;">®<mark style="color:$primary;"></sup> <mark style="color:$primary;"></mark><mark style="color:$primary;">MTB/RIF / Xpert MTB/RIF Ultra</mark>
...
**PCR 양성 지속은 격리 해제의 배제 기준이 아니다**: PCR은 사균(dead bacilli)의 DNA를 수개월간 검출할 수 있으므로, 격리 해제 여부는 도말검사 음전 및 임상 소견을 기준으로 판단한다.
{% endhint %}
...
{% hint style="info" %}
**최근 LTBI 치료 트렌드**\
최근 LTBI 치료는 rifamycin 기반 단기 요법(4R, 3HR, 3HP, 1HP)을 우선 고려하는 추세이다. 기존 9H(INH 9개월) 대비 복약 완료율이 높고 간독성 위험이 낮다. RIF 사용이 불가능한 경우(중증 간질환, 중요 약물 상호작용 등)에 한해 9H 또는 6H를 선택한다.
{% endhint %}
...
**자연 치유된 결핵 병변에서의 검사 과정**

***

```mermaid
graph TD
    Start["흉부 X선/CT상<br/>활동성 결핵 가능성이 낮은 치유 결핵 소견"]
    Q1{"과거 적절한<br/>결핵 치료력?"}
    HasTx(["비활동성 결핵"])
    Exam["검사<br/>· 과거 흉부 X선과 비교<br/>· 세균학적 검사 예: 가래<br/>· 경과 관찰"]
    ActiveTb(["활동성 결핵"])
    ActiveTx(["결핵 치료"])
    Inactive["비활동성 결핵"]
    Q2{"결핵 발병<br/>고위험군?"}
    HighRisk(["LTBI 치료"])
    NotHigh["LTBI 검사"]
    LTBINeg(["LTBI 음성 - 경과 관찰"])
    LTBIPos(["LTBI 양성 - LTBI 치료"])
    ...
    style Start fill:#e8f4f8,stroke:#4dabcc
    style HasTx fill:#d3f9d8,stroke:#8ce99a
    style Exam fill:#ebfbee,stroke:#8ce99a
    style ActiveTb fill:#fff5f5,stroke:#ffc9c9
    style ActiveTx fill:#fff5f5,stroke:#ffc9c9
    style Inactive fill:#fff9db,stroke:#fab005
    style HighRisk fill:#ebfbee,stroke:#8ce99a
    style LTBINeg fill:#f8f9fa,stroke:#ced4da
    style LTBIPos fill:#ebfbee,stroke:#8ce99a
```
...
{% hint style="info" %}
1) 칼슘이 침착된 단순 결절 및 폐쇄부와 기저부의 흉막 비후는 제외\
2) 고위험군: HIV 감염, 장기 이식으로 면역억제제 복용자, TNF 대항제 사용자
{% endhint %}
...
graph TD
    %% 정상 면역 성인 경로
    SN["LTBI 진단 검사가 필요한<br/>정상 면역 성인"]
    ...
    BCG -->|"2회 이상 접종<br/>또는 접종력 불확실"| IGA1
    TST1 -->|"10 mm 이상"| PosN1
    TST1 -->|"10 mm 미만"| NegN1
    IGA1 -->|양성| PosN2
    IGA1 -->|음성| NegN2
    ...
    %% 면역 저하 성인 경로
    SI["LTBI 진단 검사가 필요한<br/>면역 저하 성인"]
...
<table><thead><tr><th width="80">요법</th><th width="185">처방</th><th width="90">기간</th><th width="110">권고 순위</th><th width="100">복약 완료율</th><th width="100">간독성 위험</th><th>비고</th></tr></thead><tbody><tr><td>4R <strong>(표준)</strong></td><td>Rifampicin 단독</td><td>4개월</td><td>✅ 1순위</td><td>높음</td><td>낮음</td><td>가장 널리 쓰이는 표준 요법; INH 내성·부작용 시에도 선택 가능</td></tr><tr><td>3HR <strong>(표준)</strong></td><td>INH + Rifampicin</td><td>3개월</td><td>✅ 1순위</td><td>높음</td><td>보통</td><td>4R과 함께 우선 고려</td></tr><tr><td>1HP <strong>(최신)</strong></td><td>INH + Rifapentine 매일</td><td>1개월 (28일)</td><td>✅ 최신 권고</td><td>매우 높음</td><td>낮음</td><td>BRIEF TB 연구 근거; 짧은 기간으로 선호도 급증; 전문가 처방 권고</td></tr><tr><td>3HP</td><td>INH + Rifapentine 주 1회</td><td>3개월 (12주)</td><td>🔵 대체 요법</td><td>높음</td><td>낮음</td><td>직접 복약 확인(DOT) 권장; 전문가 처방 권고</td></tr><tr><td>9H <strong>(차선)</strong></td><td>INH 단독</td><td>9개월</td><td>⚠️ 차선책</td><td>낮음</td><td>높음</td><td>RIF 사용 불가 시에만 선택 (중증 간질환, 중요 약물 상호작용)</td></tr><tr><td>6H</td><td>INH 단독</td><td>6개월</td><td>⚠️ 차선책</td><td>낮음</td><td>높음</td><td>9H에 비해 효과 다소 낮음; RIF 사용 불가 시 대안</td></tr></tbody></table>
