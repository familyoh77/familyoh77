...
{% hint style="warning" %}
**국내&#x20;**_**M. pneumoniae**_**&#x20;macrolide 내성 주의** : 국내 내성률이 소아 70–90%, 성인에서도 높게 보고됨. 비전형성 폐렴 의심 시 macrolide에 48–72시간 반응이 없으면 doxycycline 또는 respiratory fluoroquinolone으로 조기 전환 고려
{% endhint %}
...
#### <mark style="color:$primary;">Diehr Rule - 성인 폐렴의 임상 진단 Tool</mark>

　　_<mark style="color:$info;">Ref. Diehr P et al. J Chronic Dis 1984;37(2):215–25.</mark>_

* 배점 : 콧물 −2점, 인후통 −1점, 근육통 +1점, 야간 발한 +1점, 가래(하루 종일) +1점, 호흡수 ＞25/분 +2점, 체온 ＞37.8℃ +2점.
* 판정 : 점수가 높을수록 폐렴 가능성 증가 (1점≒8.8%, 2점≒10.3%, ≥4점≒29.4%).&#x20;
* 단독 사용보다 임상 판단의 보조 도구로 활용

#### <mark style="color:$primary;">중증 CAP 진단 기준 (ATS/IDSA)</mark>
...
* Procalcitonin (PCT)
  * 항생제 시작 결정에는 권장되지 않음
  * 세균성 vs 바이러스성 감별의 보조 지표로 활용 가능 (세균성에서 상승, 바이러스성에서 정상 경향); 단 민감도·특이도가 제한적이므로 단독 판단은 금물
  * PCT 감소 추세는 항생제 중단 판단에 보조적으로 활용 가능
...
<table><thead><tr><th width="134.21051025390625">감별 질환</th><th width="280">Key Clue</th><th>감별 검사</th></tr></thead><tbody><tr><td>폐색전증 (PE)</td><td>흉막성 통증 + 정상 또는 비특이적 CXR, 빈맥, 위험 인자</td><td>D-dimer, CT-PA</td></tr><tr><td>울혈성 심부전</td><td>양측성 침윤 + 발목 부종, 기좌호흡, BNP 상승</td><td>BNP/NT-proBNP, 심초음파</td></tr><tr><td>결핵</td><td>아급성 경과, 체중 감소, 야간 발한, 상엽 병변, 공동</td><td>CXR/CT, AFB 도말·배양, IGRA</td></tr><tr><td>악성 종양</td><td>비해소성 폐렴, 흡연 병력, 체중 감소</td><td>흉부 CT, 기관지경</td></tr><tr><td>호산구성 폐렴</td><td>항생제 반응 없음, 혈액 호산구↑, 약물·기생충 노출</td><td>CBC differential, BAL</td></tr><tr><td>과민성 폐렴</td><td>항원(조류·곰팡이 등) 노출 병력, 재발성 경과</td><td>항원 회피 시 호전 여부, HRCT</td></tr></tbody></table>

***

```mermaid
flowchart TD
    A([외래 CAP 의심<br/>CRB-65 0–1점]) --> B["Red Flag?"]
    B -- "SpO₂<92%, RR≥30<br/>SBP<90, 의식변화<br/>청색증" --> C[즉각 이송 / 응급]

    B -- 없음 --> D["MRSA / Pseudo<br/>위험 인자?"]
    D -- "과거 호흡기 분리력<br/>최근 입원+IV 항생제" --> E[입원 / 전문의뢰<br/>확장 치료]
    D -- 없음 --> F["결핵 가능성?"]

    F -- "아급성, 체중감소<br/>야간 발한, 상엽 병변" --> G[FQ 보류<br/>TB 평가 선행]
    F -- 없음 --> H["Phenotype"]

    H -- "전형적<br/>고열·화농성" --> I[동반 질환?]
    H -- "비전형적<br/>마른기침·경증" --> J[doxycycline<br/>100 ㎎ bid ×5d]
    H -- "흡인성<br/>의식저하·구토" --> K["amox-clav<br/>(혐기성 루틴 ❌)"]
    H -- "모호<br/>(전형/비전형 불분명)" --> H2["amox-clav<br/>+ doxycycline<br/>(병용으로 커버)"]

    I -- 없음 --> L[amoxicillin<br/>1 g tid ×5d]
    I -- 있음 --> M[선호 방식]
    M -- 병용 --> N[amox-clav<br/>+ doxycycline]
    M -- 단순화 --> O["Respiratory FQ<br/>(결핵 배제 후)"]
    ...
    P -- "✅ 호전" --> Q[5일 완료<br/>안정 기준 충족 시 중단]
    P -- "⚠ 부분 호전" --> R[Step-up<br/>항생제 조정]
    P -- "❌ 악화" --> S[CXR 재확인<br/>입원 / 의뢰]
style A fill:#eeeeee,stroke:#888888,stroke-width:2px
classDef yellow fill:#fff9c4,stroke:#f39c12
class B,D,F,H,I,M,P yellow
classDef L_red fill:#ffcdd2,stroke:#c62828,stroke-width:2px
class C,E,S L_red
style G fill:#fff3e0,stroke:#e65100

    style R fill:#ff9966,stroke:#cc3300
    style Q fill:#d0e8ff,stroke:#1a6abf
...
1. Confusion : 사람, 장소, 시간에 대한 착란
2. Uremia : BUN ＞19 ㎎/㎗
3. Respiratory rate ≥30회/분
4. BP : SBP ＜90 또는 DBP ≤60 ㎜Hg
5. ≥65세

* 외래 진료에서는 2번 항목을 제외한 CRB-65 이용을 권장; 혈액 검사가 가능한 경우 CURB-65 사용 권장
* 배점 : 각 항목에 1점씩 부여
* 판정 및 조치
  * 0–1점 = 외래 치료
...
<table><thead><tr><th width="281.6842041015625">위험 인자</th><th width="180.75494384765625">배점</th></tr></thead><tbody><tr><td><em>상황 요인</em></td><td></td></tr><tr><td>연령 (남성)</td><td>나이(yr)=점수</td></tr><tr><td>연령 (여성)</td><td>나이(yr)−10=점수</td></tr><tr><td>Nursing home 거주</td><td>+10</td></tr><tr><td><em>동반 질환</em></td><td></td></tr><tr><td>암</td><td>+30</td></tr><tr><td>만성 간질환</td><td>+20</td></tr><tr><td>CHF</td><td>+10</td></tr><tr><td>만성 신장 질환</td><td>+10</td></tr><tr><td>뇌혈관 질환</td><td>+10</td></tr><tr><td><em>진찰 소견</em></td><td></td></tr><tr><td>Altered mental status</td><td>+20</td></tr><tr><td>호흡수 ≥30/분</td><td>+20</td></tr><tr><td>SBP ＜90 ㎜Hg</td><td>+20</td></tr><tr><td>체온 ≥40℃ 또는 ＜35℃</td><td>+15</td></tr><tr><td>맥박 ≥125/분</td><td>+10</td></tr><tr><td><em>검사 소견</em></td><td></td></tr><tr><td>동맥혈 pH ＜7.35</td><td>+30</td></tr><tr><td>BUN ＞29 ㎎/㎗</td><td>+20</td></tr><tr><td>Na ＜130 m㏖/L</td><td>+20</td></tr><tr><td>Glucose ＞249 ㎎/㎗</td><td>+10</td></tr><tr><td>Hematocrit ＜30%</td><td>+10</td></tr><tr><td>PaO₂ ＜60 ㎜Hg</td><td>+10</td></tr><tr><td>흉부 X선상 pleural effusion</td><td>+10</td></tr></tbody></table>
...
* **MRSA /&#x20;**_**P. aeruginosa**_**&#x20;위험 인자** : 과거 호흡기 검체 분리력이 가장 중요한 위험 인자; 최근 입원(90일 내) AND 비경구 항생제 투여력이 그 다음
{% endhint %}
...
* Macrolide (azithromycin, clarithromycin)는 국내 내성률을 고려하여 단독 1차 선택에서 제외. 단, 지역 내 내성률이 ＜25%임이 확인된 경우 또는 특수 상황(doxycycline 금기 등)에서 고려 가능
...
```mermaid
flowchart TD
    A(["항생제 치료<br/>48–72시간 후"]) --> B[임상 반응?]

    B -- "✅ 명확한 호전<br/>(해열, 증상 개선)" --> C[현 처방 유지]
    C --> D["5일 후<br/>안정 기준 충족?"]
    D -- 예 --> E[항생제 중단]
    D -- 아니오 --> F["재평가 반복<br/>(최소 5일 보장)"]

    B -- "⚠ 부분 호전<br/>(발열 지속·불완전 개선)" --> G[추정 원인?]
    G -- "스펙트럼 부족" --> H["amox → amox-clav<br/>또는 doxy 추가"]
    G -- "Atypical 미커버" --> I["macrolide 추가<br/>또는 doxy로 전환"]

    B -- "❌ 호전 없음 / 악화" --> J[즉시 재평가]
    J ---> K[CXR 재확인]
    K --> L[소견?]
    L -- "삼출·농흉·농양" --> M[입원 + 추가 처치]
    L -- "진단 오류 가능" --> N["PE / CHF / TB<br/>/ 악성 종양 감별"]
    L -- "내성 또는 비전형" --> O["Respiratory FQ 전환<br/>또는 입원 IV 항생제"]
style A fill:#e3f2ff,stroke:#2196f3,stroke-width:2px
classDef yellow fill:#fff9c4,stroke:#f39c12
class B,D,G,L yellow
classDef sky fill:#e3f2ff,stroke:#2196f3
class H,I,C sky
classDef orange50 fill:#fff3e0,stroke:#e65100
class N,O orange50
    style E fill:#d0e8ff,stroke:#1a6abf
...
#### <mark style="color:$primary;">1. 진단 오류 (Diagnostic Error)</mark>
...
#### <mark style="color:$primary;">2. 숙주 요인 (Host Problem)</mark>
...
#### <mark style="color:$primary;">3. 병원체·약제 요인 (Pathogen / Drug Issue)</mark>
...
* 예방접종 : [폐렴](../231_/210_-vaccination.md#pneumococcal-pneumonia) 및 [인플루엔자](../231_/210_-vaccination.md#influenza)&#x20;
* 금연
* 손 씻기, 건강한 생활 습관 (충분한 영양 섭취, 규칙적 운동)

### <mark style="color:orange;">성인 폐렴구균 예방접종 권고</mark>

* 기존 PPSV23 중심에서 단백접합백신(PCV)으로 패러다임이 전환됨. 2024년 10월 ACIP는 보편 접종 연령을 65세 → 50세로 하향 확장하고, PCV21(CAPVAXIVE)을 추가함
* 국내 현황 (2025) : 대한감염학회 2025년 개정 지침에서 50세 하향 조정은 아직 국내 공식 권고에 미포함 (국내는 65세 이상 또는 19–64세 고위험군 기준 유지). PCV21(CAPVAXIVE)은 2025년 8월 27일 식약처 허가 후 시판 중 (약 18–20만 원). 국내 급여·보험 기준은 별도 확인 필요

<table><thead><tr><th width="167.89471435546875">대상</th><th width="286.84222412109375">권고 접종 방법 (ACIP 2025)</th><th>비고</th></tr></thead><tbody><tr><td>50세 이상<br>(PCV 미접종 또는 접종력 불명)</td><td>PCV21 단독 1회 <em>(preferred)</em><br>또는 PCV20 단독 1회<br>또는 PCV15 1회 → PPSV23 (≥1년 후)</td><td>PCV21이 비용-효과 면에서 우선; PCV20·PCV21 접종 후 PPSV23 불필요</td></tr><tr><td>50세 이상 (과거 PPSV23만 접종)</td><td>PCV21 또는 PCV20 1회 추가<br>(마지막 접종으로부터 ≥1년)</td><td>접합백신 미접종 상태이므로 PCV 추가 권고</td></tr><tr><td>50세 이상 (과거 PCV13 접종력)</td><td>PCV21 또는 PCV20 1회<br>(마지막 폐렴구균 백신으로부터 ≥5년)</td><td>PCV13+PPSV23 완료자는 shared decision-making으로 추가 고려</td></tr><tr><td>19–49세 고위험군*</td><td>PCV21 또는 PCV20 단독 1회<br>또는 PCV15 → PPSV23 순차</td><td>만성 질환·면역 저하 등 위험 인자 있을 때</td></tr><tr><td>면역 저하자 (무비증, HIV, 이식 등)</td><td>PCV21 또는 PCV20<br>또는 PCV15 → PPSV23 (8주 후)<br>→ 5년 후 PPSV23 반복 고려</td><td>전문의 상담 권장</td></tr></tbody></table>

\* 고위험군 : 만성 폐·심·간·신질환, 당뇨병, 알코올 남용, 흡연, 무비증, 면역 저하 등

_<mark style="color:$info;">Ref. Kobayashi M et al. MMWR 2025;74(1):1–8; CDC Adult Immunization Schedule 2025 (Addendum rev. July 2025); 대한감염학회 성인 예방접종 지침 2025</mark>_
