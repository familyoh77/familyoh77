...
* 현재 COVID-19 + 인플루엔자 A/B **콤보 RAT**가 널리 사용됨; RSV 포함 multiplex RAT도 활용 가능
...
* **세균 검사** : 심한 증상 (광범위 폐렴, 호흡 부전, 저혈압, 발열) 환자, 초기 호전 후 악화 환자에서 고려
...
graph TD
    ...
    R1["Oseltamivir - 1차 선택<br/>발병 시점 무관, 즉시 시작<br/>중증: 최대 10일 투여"]
    R2["Oseltamivir 또는 Baloxavir<br/>증상 뚜렷하거나 환자 선호 시<br/>shared decision (보험 기준 확인)"]
    R3["대증치료만<br/>항바이러스제 불필요<br/>비고위험군 경증에 한함"]
    R4["Peramivir IV<br/>응급실·입원 초기 bridging<br/>경구 가능 시 oseltamivir 전환"]
    ...
    style Q1 fill:#fff9e6,stroke:#ffcc00,stroke-width:2px
    style Q2 fill:#fff9e6,stroke:#ffcc00,stroke-width:2px
    style Q3 fill:#fff9e6,stroke:#ffcc00,stroke-width:2px
    style R1 fill:#e6f3ff,stroke:#99ccff
    style R2 fill:#e6f3ff,stroke:#99ccff
    style R3 fill:#e6ffe6,stroke:#99cc99
    style R4 fill:#e6f3ff,stroke:#99ccff
```

<p align="center"><strong>항바이러스제 선택 알고리듬</strong></p>

<p align="center"><em><mark style="color:$info;">Ref. IDSA Influenza Clinical Practice Guidelines / CDC Influenza Antiviral Recommendations / WHO Clinical Practice Guidelines for Influenza (2024)</mark></em></p>
...
{% hint style="danger" %}
⚠️ **스테로이드 routine 사용 금지**

인플루엔자 감염에서 **명확한 적응증(천식·COPD 급성 악화, 중증 상기도 부종 등) 없이** 고용량 스테로이드를 투여하면 바이러스 배출 기간 연장 및 사망률 증가 → **루틴한 스테로이드 사용은 피한다** \[IDSA]
{% endhint %}
...
* 비고위험군에서 증상 발생 <48시간인 경우 질병 기간 단축·증상 완화 목적으로 투여 가능; 증상이 뚜렷하거나 환자 선호가 높은 경우 **shared decision**으로 결정 (보험 급여 기준 확인 필요)
* 중증(severe) 환자에서는 치료 기간 연장 가능 (oseltamivir 최대 10일)
...
* 중증(severe) 환자 : oseltamivir 조건부 권고
...
* **용법** : 아래 용량을 1일 2회, **5일간** 투여; 중증(severe) 환자에서는 **최대 10일 연장** 가능

<table><thead><tr><th width="240">연령/체중</th><th>치료 용량 (1회)</th></tr></thead><tbody><tr><td>생후 2주\~1세 미만</td><td>3 ㎎/㎏</td></tr><tr><td>1\~12세, ≤15 ㎏</td><td>30 ㎎</td></tr><tr><td>1\~12세, >15\~23 ㎏</td><td>45 ㎎</td></tr><tr><td>1\~12세, >23\~40 ㎏</td><td>60 ㎎</td></tr><tr><td>1\~12세, >40 ㎏</td><td>75 ㎎</td></tr><tr><td>≥13세 (비만 포함)</td><td>75 ㎎ (고도 비만 환자도 표준 용량 유지)</td></tr></tbody></table>
...
* **특징** : **단회 경구 투여**; 인플루엔자 B형에서 oseltamivir 대비 증상 회복 기간 >24시간 추가 단축 (CAPSTONE-2, 2019); oseltamivir 대비 **바이러스 배출(viral shedding) 중단 속도가 유의하게 빠름** → 감염 확산 방지 측면에서 유리
...
* 등교·등원·출근 중지 기간 : **해열제 없이** 정상 체온 회복 후 **24시간**이 경과할 때까지 \[질병관리본부, 2022]
  * 해열제를 사용한 경우 : 마지막 해열제 투약 시점으로부터 **48시간** 경과 후 \[질병관리본부, 2022]
  * ✽ CDC 등 일부 국제 기준은 "해열제 사용 여부와 무관하게 **발열 소실 후 24시간**"을 적용; 기관 방침에 따라 선택
...
{% hint style="info" %}
**심혈관 질환자에서의 인플루엔자 예방 및 치료의 중요성**

인플루엔자 백신 접종과 적절한 항바이러스제 치료는 기저 심혈관 질환이 있는 환자에서 **급성 심근경색 및 심부전 악화 위험을 유의하게 감소**시킵니다. 특히 최근 1년 이내 심혈관 사건 병력이 있는 환자에서 매년 예방접종을 반드시 권고합니다.
{% endhint %}
