...
* 신용어로는 알부민뇨(albuminuria)가 단백뇨 중 알부민 성분만을 지칭하는 좁은 개념이며, KDIGO 2024 만성콩팥병 진료지침은 신손상의 주된 표지자로 알부민뇨(UACR)를 우선 사용할 것을 권고함
...
* 의의 : 당뇨병콩팥병증 조기 진단 및 신부전 예측 지표, 고혈압 환자의 심혈관 사망 위험 인자; KDIGO 2024는 eGFR 수준과 무관하게 알부민뇨 증가 자체가 심혈관·신장 예후 악화와 비례 관계임을 강조
...
* 육안적 혈뇨 + 단백뇨 + 급격한 신기능 저하 동반(급속진행사구체신염, RPGN 의심) → 즉시 신장내과 응급 의뢰
...
* 단백뇨와 혈뇨가 함께 확인되는 경우(사구체 질환 강력 의심) → 신장내과 의뢰
...
* 단백뇨·혈뇨의 단계별 상세 접근(Dipstick → UACR → UPCR → 24시간 단백뇨의 정량 흐름, 및 혈뇨의 현미경 검사 → RBC morphology → 비뇨기과/신장내과 의뢰 흐름)는 (☞ [신장질환의 진단](095_-kidney-disease-diagnosis.md)) 챕터에서 총괄적으로 다루며, 본 챕터의 아래 진단 원칙 및 알고리듬은 단백뇨에 특화된 실전 접근을 정리한 것임 (☞ [혈뇨](110_-hematuria.md) 챕터도 함께 참조)
...
* 채뇨 방법 : 검사일 전날 밤 취침 시 배뇨 후 아침 첫 소변 채취 (☞ [신장질환의 진단](095_-kidney-disease-diagnosis.md#채뇨-방법))
...
* 만성콩팥병 위험도가 높은 환자(예: 당뇨병, 고혈압) : 처음부터 단백뇨 정량 검사(UACR) 실시 → 단백뇨 진단 시 만성 신질환에 대한 평가
...
```mermaid
graph TD
    Start([현미경적 소변검사]) -->|음성| Dip[시험지봉 검사]
    Start -->|양성 - 신 질환에 부합되는 소견| H24[24시간 요단백 검사]

    Dip -->|Trace\~2+| Recheck[1달 후 2\~3회 재검]
    Dip -->|3+\~4+| H24

    Recheck -->|음성| Transient[일과성 단백뇨]
    Recheck -->|양성| H24
    Transient --> Reassure1[안심시킴: 추구검사 필요 없음]

    H24 -->|요단백 ＜2 g/d| CrCl1[CrCl 검사]
    H24 -->|요단백 ＞2 g/d| CrCl2[CrCl 검사]

    CrCl1 -->|정상| Age{연령 ＜30세?}
    CrCl1 -->|감소| Sympt1[유증상 단백뇨]

    Age -->|Yes| Ortho[기립성 단백뇨 평가]
    Age -->|No| Isolated[고립성 단백뇨]
    Ortho -->|양성| Reassure2[안심시킴: 혈압 측정, 요검사; 매 1\~2년]
    Ortho -->|음성| Isolated
    Isolated --> Follow6mo[혈압 측정, 요검사, 신기능검사; 매 6개월]

    CrCl2 -->|정상| Sympt2[유증상 단백뇨]
    CrCl2 -->|감소| Sympt3[유증상 단백뇨]

    Sympt1 -->|명백한 기저 질환| Treat1[기저 질환 치료 및 안정 또는 CrCl 호전까지 매달 추구검사]
    Sympt1 -->|원인 불명| Refer1[의뢰]

    Sympt2 -->|기저 질환 확인됨| Treat2[기저 질환 치료: 매달 혈압 측정, 요/신 기능검사; 의뢰 고려]
    Sympt2 -->|원인 불명| Refer1

    Sympt3 --> Refer2[의뢰]

    style Start fill:#f96,stroke:#e65100,stroke-width:2px
    style H24 fill:#f96,stroke:#e65100,stroke-width:2px
    style Reassure1 fill:#e1f5fe,stroke:#01579b
    style Reassure2 fill:#e1f5fe,stroke:#01579b
    style Follow6mo fill:#e1f5fe,stroke:#01579b
    style Refer1 fill:#f3e5f5,stroke:#4a148c,stroke-width:2px
    style Refer2 fill:#f3e5f5,stroke:#4a148c,stroke-width:2px
    style Treat1 fill:#fff3e0,stroke:#e65100
    style Treat2 fill:#fff3e0,stroke:#e65100
```
...
<p align="center"><em><mark style="color:$info;">Ref. KDIGO 2024 Clinical Practice Guideline for the Evaluation and Management of CKD. 상세 단계별 흐름은 (☞ [신장질환의 진단](095_-kidney-disease-diagnosis.md)) 챕터 참조.</mark></em></p>
...
* 혈압 목표 : KDIGO 2024는 표준화된 진료실 혈압 측정을 전제로 수축기혈압(SBP) ＜120 ㎜Hg를 제시(2B); 실제 일차의료 현장에서는 표준화 측정이 어려운 경우가 많아 ≤140/90 ㎜Hg를 실질적 목표로 삼는 경우가 흔하며, 고령·허약·기립성 저혈압 위험군에서는 130\~139/80\~89 ㎜Hg 수준의 완화된 목표를 예시로 개별화함 (☞ [고혈압](095_-hypertension.md))
...
* Na 섭취 제한 : ＜2 g/d(소금으로 5 g/d) (☞ [고혈압](095_-hypertension.md))
...
* KDIGO 2024 권고 : 2형 당뇨병 동반 CKD, eGFR ≥20 mL/min/1.73㎡ 환자에서 SGLT2 억제제 치료 권고(1A); 단백뇨를 동반한 CKD는 당뇨병 동반 여부와 무관하게 병용 고려(신기능·알부민뇨 감소, 심신 보호 효과)
...
* finerenone <mark style="color:blue;">\[케렌디아]</mark> : 2형 당뇨병 동반 만성콩팥병에서 ACEI/ARB 병용 시 알부민뇨 감소 및 신장·심혈관 사건 위험 감소(FIDELIO-DKD, FIGARO-DKD, FIDELITY 통합분석)
...
* semaglutide : 2형 당뇨병 동반 만성콩팥병 환자에서 주요 신장 사건 위험 감소(FLOW trial, NEJM 2024) - eGFR slope 개선, 알부민뇨 감소 효과가 SGLT2 억제제 병용 여부와 무관하게 확인됨
...
>
> ```
> 케렌디아 10 ㎎/T  1T  qd (신기능에 따라 20 ㎎까지 증량)
...
> * NSAID 병용은 신기능 저하 위험을 높이므로 가능한 피하십시오
...
> * 정기 추적 검사에서 단백뇨가 악화되거나 신기능이 저하되는 경우
