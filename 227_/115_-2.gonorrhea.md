...
* STI 검사 : 임질이 진단된 환자는 다른 [성매개질환](115_-1.STI.md)(HIV, 매독, 클라미디아 등)에 대한 검사를 요함
...
<table><thead><tr><th width="231.90478515625">질환</th><th width="220">임질과의 차이</th><th>감별 포인트</th></tr></thead><tbody><tr><td>비임균성 요도염(NGU, 클라미디아·마이코플라스마 등)</td><td>잠복기가 상대적으로 김(1~3주)</td><td>그람염색에서 diplococci 미관찰; NAAT로 원인균 확인</td></tr><tr><td>트리코모나스 질염 / <br>세균성 질염</td><td>화농성보다는 악취 동반 분비물, 소양감 두드러짐</td><td>생리식염수 습식도말에서 편모충 관찰; 임질 배양·NAAT 음성</td></tr><tr><td>반응성 관절염(Reactive arthritis)</td><td>결막염·요도염·관절염 triad</td><td>임균 배양·NAAT 음성; 최근 요도염/장염 병력</td></tr></tbody></table>

***

```mermaid
graph TD
    A([요도/자궁경부/인두/직장 <br/>증상 또는 고위험 노출]) 
--> B[검체 채취: NAAT 1차 ± 배양]
    B --> C[임균 양성?]
    C -- No --> D[다른 원인 감별]
    C -- Yes --> E[합병증 징후?<br>DGI/PID/부고환염 등]
    E -- No --> F[ceftriaxone 500 ㎎ <br>단회 IM/IV]
    E -- Yes --> G[입원, 전문과 협진, <br>정맥 항생제]
    F --> H[클라미디아 배제됨?]
    H -- No --> I[plus doxycycline <br>100 ㎎ bid ×7일]
...
* 치료 원칙: 합병증이 없는 요도·자궁경부·직장·인두 감염은 ceftriaxone 단회 근육/정맥주사로 치료하며, 클라미디아 동반 여부가 확인되지 않은 경우 doxycycline을 추가
...
{% hint style="info" %}
**Doxy-PEP(노출 후 예방요법)** - 세균성 STI(매독·클라미디아·임질) 공통 예방 전략 (☞ [성매개감염병](115_-1.STI.md))&#x20;
{% endhint %}
...
* ceftriaxone : 500 ㎎ ×1회 IM (체중 150 ㎏ 미만) <mark style="color:blue;">\[트리악손]</mark>
  * 체중 150 ㎏ 이상인 경우 : ceftriaxone 1 g ×1회 IV 권장 \[성매개감염 진료지침 2023]

- [ ] CDC는  1 g 용량도 IM 투여를 권고하지만, IM의 통증 문제로 국내에서는 고용량 투여는 IV를 권고

* 클라미디아 감염이 배제되지 않은 경우, 아래 1차 선택제·대체제 요법 모두에 doxycycline 100 ㎎ bid ×7일을 추가  <mark style="color:blue;">\[독시사이클린]</mark>

**대체제**

* spectinomycin : 2 g IM ×1회; 유통이 거의 되지 않아 사용이 제한적
  * 인두 감염에는 효과가 낮아(치료 성공률 -  50% 내외) 권장하지 않음
* gentamicin 240 ㎎ IM ×1회 <mark style="color:blue;">\[겐타마이신 주]</mark> _plus_ azithromycin 2 g ×1회 <mark style="color:blue;">\[지스로맥스]</mark> (세팔로스포린 중증 알레르기에서만 제한적으로 고려)
* cefixime : 800 ㎎ ×1회 <mark style="color:blue;">\[슈프락스]</mark> (ceftriaxone 투여가 어려운 경우 선택)
  * cefixime 단독은 항임균 효과가 상대적으로 낮으므로 클라미디아 배제가 안 된 경우 doxycycline을 추가 - 일반적으로 azithromycin 병용은 권장하지 않음
  * 인두 감염 완치율이 낮아(약 92% 미만) 인두 감염에는 권장하지 않음
