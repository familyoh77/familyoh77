...
| [**피임 방법**](ca://s?q=%ED%94%BC%EC%9E%84_%EB%B0%A9%EB%B2%95_%EC%A2%85%EB%A5%98) | [**완벽한 사용(%)**](ca://s?q=%ED%94%BC%EC%9E%84_%EC%99%84%EB%B2%BD%ED%95%9C_%EC%82%AC%EC%9A%A9_%EC%8B%A4%ED%8C%A8%EC%9C%A8)¹⁾ | [**전형적 사용(%)**](ca://s?q=%ED%94%BC%EC%9E%84_%EC%A0%84%ED%98%95%EC%A0%81_%EC%82%AC%EC%9A%A9_%EC%8B%A4%ED%8C%A8%EC%9C%A8)¹⁾ |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| IUC progesterone                                                               | 0.2                                                                                                                       | 0.2                                                                                                                       |
| IUC copper                                                                     | 0.6                                                                                                                       | 0.8                                                                                                                       |
| Implanon                                                                       | 0.05                                                                                                                      | 0.05                                                                                                                      |
| Vasectomy²⁾                                                                    | 0.1                                                                                                                       | 0.15                                                                                                                      |
| Tubal ligation³⁾                                                               | 0.5                                                                                                                       | 0.15                                                                                                                      |
| Progest (Depo-Provera)                                                         | 0.3                                                                                                                       | 3                                                                                                                         |
| 복합 호르몬제⁴⁾                                                                      | 0.3                                                                                                                       | 9                                                                                                                         |
| Diaphragm                                                                      | 6                                                                                                                         | 12                                                                                                                        |
| 콘돔, 남성용⁵⁾                                                                      | 2                                                                                                                         | 18                                                                                                                        |
| 콘돔, 여성용                                                                        | 5                                                                                                                         | 21                                                                                                                        |
| Sponge, 살정제제                                                                   | 9–20                                                                                                                      | 12–28                                                                                                                     |
| 질 외 사정                                                                         | 4                                                                                                                         | 22                                                                                                                        |
| 자연 피임법⁶⁾                                                                       | 0.4–5.9                                                                                                                   | 24–85                                                                                                                     |

IUC=intrauterine contraceptive. Ref. SOGC. Canadian Contraception Consensus. 2015. Chap 1. Table 2.

¹⁾ 완벽한 사용=철저하고 지속적인 사용. 전형적 사용=잘못된 사용을 포함\
²⁾ 시술 후 즉시 효과는 없음. 최소 1회의 정액 배출을 요하며 정액 속의 정자 수 확인이 필요할 수 있음(≤1%에서 재시술이 필요); 전립선/고환암/심혈관 질환 증가와 관련 없음\
³⁾ 시술 후 완전 폐색까지 수개월 소요. 영상 검사로 폐색이 확인될 때까지(최소 3개월) 다른 피임법 사용; 시술 후 임신 시 자궁외임신의 위험이 높음(피임을 하지 않는 경우보다는 낮음); 난소암 위험 감소 효과\
⁴⁾ BMI ＞30 여성에서 효과가 저하될 수 있음; 완벽하지 않은 사용 시 피임 실패율이 높음\
⁵⁾ 라텍스 콘돔은 STI 예방 효과가 있음; 사용 방법에 따라 피임 실패율이 크게 달라짐\
⁶⁾ 모유 수유 시 출산 후 6개월까지 피임 작용이 있으나 확실한 피임법은 아님
...
* 2, 3주차 미복용 시 대처
  ...
  * 3일간 미복용 : 새로운 주기로 복용 시작 또는 7일 연속 복용할 때까지 다른 피임법 사용(해당 주기의 hormone-free interval은 두지 않음)
...
flowchart TD
    ...
    A -- "≤7일" --> B1["•오늘 새로운 피임 방법 적용 시작<br>•if LMP 5~7일전 →<br>7일간 backup method<br>•if 주사제 → backup method 필요 없음"]
    A -- ">7일" --> B2["LMP 이후 unprotected sex?"]

    B2 -- "없음" --> C1["•오늘 피임 시작<br>•7일간 backup method"]
    B2 -- "있음" --> C2["소변 임신 검사"]

    C2 -- "양성" --> D1["결과에 대하여 상담"]
    C2 -- "음성" --> D2["당장 피임제 투여 개시를 희망?"]

    D2 -- "yes" --> E1["if ≤5일 unprotected sex<br>→ levonorgestrel 투여*"]
    D2 -- "no" --> E2["if ≤5일 unprotected sex<br>→ ulipristal or levonorgestrel 투여*"]

    E1 --> F1["•오늘부터 새로운 피임법 시작<br>•7일간 backup method 시행<br>•2~4주 후에 임신 검사 반복"]
    E2 --> F2["•피임 방법들에 대하여 상담, 교육<br>•다음 월경까지 대체 피임 방법 사용<br>•다음 월경 시작 후 5일 내 피임법 개시<br>(주사제는 7일 내 시행)"]
    ...
    class A,B2,D2,E1,E2 yellow
    class C2 green
    class B1,C1,D1,F1,F2 blue
```

\*levonorgestrel은 비만자에서는 효과가 저하됨; progestin은 ulipristal의 작용을\
방해하므로 ulipristal 투여 5일 내에는 복합 호르몬제 투여는 피함

복합 호르몬 피임제(pill, patch, ring, injection, implant) 시작 알고리듬\
Ref. Initiating Hormonal Contraception. AFP 2021;103(5). Fig 1
...
* 장점 : 수유에 대한 영향이 없음, 복합제보다 두통/위장 장애 부작용이 적음, 월경 실혈 및 월경통 적음, estrogen 금기 환자에서 사용할 수 있음(stroke, MI, VTE 위험을 증가시키지 않음)
...
* 부작용 : 경구제와 비슷; 불규칙 출혈 후 점차 무월경, 두통, 체중 증가, 골밀도 감소(골밀도 감소는 혈중 estradiol 수준 감소와 관련되며 투약 중단 후 회복되고 골절 위험 증가는 입증되지 않음)
...
flowchart TD
    ...
    B2 -- "양성" --> C1["결과에 대하여 상담"]
    B2 -- "음성" --> C2["LMP 이후 unprotected sex?"]

    C2 -- "없음" --> D1["•오늘 IUD 삽입<br>•7일간 backup method"]
    C2 -- "있음" --> D2["•임신 초기 가능성 설명<br>•임신 시의 IUD의 위험 리뷰<br>•if ≤5일 관계 → 금일 응급 피임"]

    D2 --> E["오늘 IUD 시술 희망?"]

    E -- "yes" --> F1["•오늘 IUD 삽입<br>•7일간 backup method<br>•2주 후 재방문 임신 검사"]
    E -- "no" --> F2["•경구제 등의 피임법 권고<br>•2주 후 재방문 임신 검사"]
    ...
    class A,C2,E yellow
    class B2 green
    class B1,C1,D1,D2,F1,F2,G1,G2 blue
```

Progestin IUD 시작 알고리듬\
Ref. Initiating Hormonal Contraception. AFP 2021;103(5). Fig 2
...
* 응급 피임 방법으로 경구 응급 피임제 + Copper IUD(성교 5일 내) 삽입이 가장 효과적이며, 단일 방법으로는 copper IUD가 가장 효과적임
* levonorgestrel 복용 후 24시간 내/ulipristal acetate 복용 5일 후 호르몬 피임을 시작할 수 있으며, 호르몬 투여 후 levonorgestrel 복용자는 7일간/ulipristal acetate 복용자는 14일간 다른 방법으로 피임 또는 성관계 중지
...
| [**복용까지의 시간**](ca://s?q=%EC%9D%91%EA%B8%89%ED%94%BC%EC%9E%84%EC%95%BD_%EB%B3%B5%EC%9A%A9%EC%8B%9C%EA%B0%84_%ED%9A%A8%EA%B3%BC) | [**Levonorgestrel**](ca://s?q=Levonorgestrel_%ED%94%BC%EC%9E%84%ED%9A%A8%EA%B3%BC) | [**Ulipristal acetate**](ca://s?q=Ulipristal_acetate_%ED%94%BC%EC%9E%84%ED%9A%A8%EA%B3%BC) |
| ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| \~24시간                                                                                                                         | 2.5%                                                                               | 0.9%                                                                                       |
| \~72시간                                                                                                                         | 2.2%                                                                               | 1.4%                                                                                       |
| \~120시간                                                                                                                        | 2.2%                                                                               | 1.3%                                                                                       |
