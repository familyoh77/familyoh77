...
```mermaid
flowchart TD
    A["임신이 아니라고 reasonably certain 할 수 있는가?"]

    A -- "yes" --> B["오늘 호르몬 피임법 시작"]
    A -- "no" --> C["최근 5일 내 unprotected sex?"]

    C -- "no" --> D["IUD를 제외한 호르몬 피임법<br>오늘 quick start 가능"]
    C -- "yes" --> E["응급피임 필요성 평가"]

    E --> F["LNG EC 선택"]
    E --> G["UPA EC 선택"]

    F --> H["호르몬 피임법 즉시 시작/재개 가능"]
    G --> I["5일간 progestin 함유 피임법 시작/재개 보류<br>6일째부터 시작"]

    B --> J["방법별 필요 시 backup method"]
    D --> K["방법별 필요 시 backup method<br>2~4주 후 임신 검사 반복"]
    H --> L["새 피임법 시작 후 7일간 backup*"]
    I --> M["새 피임법 시작 후 7일간 또는<br>다음 월경까지 중 먼저 오는 시점까지 backup*"]
...
flowchart TD
    ...
    A -- "yes" --> B["오늘 IUD 삽입 가능"]
    A -- "no" --> C["최근 5일 내 unprotected sex?"]

    C -- "yes" --> D["응급피임으로 Cu-IUD 적응 여부 평가"]
    C -- "no" --> E["IUD 삽입 보류"]

    D -- "Cu-IUD 선택/적합" --> F["오늘 Cu-IUD 삽입<br>지속 피임으로 그대로 사용"]
    D -- "Cu-IUD 미선택/부적합" --> G["경구 EC 및 임시 피임법 고려<br>IUD는 임신 배제 후 삽입"]

    E --> H["IUD 외 피임법 임시 사용 가능<br>2~4주 후 임신 검사/임신 배제"]
    G --> H
    H --> I["임신이 배제되면 IUD 삽입"]

    B --> J["LNG-IUD를 월경 시작 후 >7일에 삽입한 경우<br>7일간 backup method*"]
    F --> K["Cu-IUD는 삽입 즉시 피임 효과"]
