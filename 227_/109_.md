...
* 신장 질환 파트의 진단 도구 챕터로, 소변 검사·신장 기능 검사·영상 검사 등 신장 질환 전반에 공통으로 사용되는 진단 방법을 다룸
...
* 이뇨제 포함 3종 이상 항고혈압제에도 조절되지 않거나, 4종 이상 항고혈압제를 복용 중인 치료 저항성 고혈압 → 조기 의뢰
* 반복적으로 확인되는 혈청 칼륨 이상(고칼륨혈증 등) → 조기 의뢰
...
* 만성신장병(CKD, KDIGO 2024) : 신장의 구조 또는 기능 이상이 **3개월 이상 지속**되며 건강에 영향을 미치는 경우; eGFR과 알부민뇨(ACR) 두 축으로 병기 분류(☞ 아래 CKD 병기표)
...
* 급성 신손상(AKI, KDIGO criteria) : 다음 중 하나
  ...
  * 요량 ＜0.5 ㎖/㎏/h가 6시간 이상 지속
* AKI 진단은 eGFR 감소가 아니라 **혈청 Cr의 변화와 요량**을 기준으로 판단 — eGFR 공식은 신기능이 안정된 상태(steady state)를 전제하므로 AKI에서는 부정확
* CKD·AKI 감별이 중요 — 만성화를 시사하는 소견 : 빈혈, 부갑상선호르몬 상승, 초음파상 신장 크기 감소·피질 두께 감소·에코 증가

**AKI 병기(KDIGO Stage 1\~3)**

<table><thead><tr><th width="90">병기</th><th width="220">혈청 Cr 기준</th><th>요량 기준</th></tr></thead><tbody><tr><td>Stage 1</td><td>기저치 대비 1.5\~1.9배 또는 ≥0.3 ㎎/㎗ 상승</td><td>＜0.5 ㎖/㎏/h가 6\~12시간</td></tr><tr><td>Stage 2</td><td>기저치 대비 2.0\~2.9배</td><td>＜0.5 ㎖/㎏/h가 ≥12시간</td></tr><tr><td>Stage 3</td><td>기저치 대비 ≥3배 또는 Cr ≥4.0 ㎎/㎗ 상승 또는 신대체요법(RRT) 시작</td><td>＜0.3 ㎖/㎏/h가 ≥24시간 또는 무뇨 ≥12시간</td></tr></tbody></table>

* Cr과 요량 기준 중 더 심한 병기를 적용; KDIGO는 2023년부터 AKI 가이드라인 개정 작업을 진행 중(AKI-AKD 정의 통합 등)이나, 위 Stage 1\~3 기준(2012)은 현재도 표준으로 사용됨
...
{% hint style="info" %}
단백뇨·혈뇨의 상세 접근 알고리듬(Dipstick→UACR→UPCR→24시간 단백뇨 / 육안혈뇨→현미경→RBC morphology→CT·비뇨기과 의뢰)은 각각 (☞ [단백뇨](111_-proteinuria.md))·「혈뇨」 챕터에서 다룸 — 중복을 피하기 위해 이 챕터에는 넣지 않음
{% endhint %}
...
* 대상 : 불확실한 진단, 비전형적 증상, complicated UTI 의심, 신우신염 의심, 임신, 남성, 초치료 실패, 재발, 항생제 내성균 의심, 도뇨관 유치, ＞65세에서 **증상이 있는 경우**의 항생제 투여 결정 시(IDSA — 연령 자체가 아닌 증상 유무가 배양 적응증의 핵심)
...
* Spot urine ACR이 24시간 소변 채집보다 권장됨 — 환자 부담이 적고 신뢰도는 유사; 가능하면 **아침 첫 소변**으로 측정하는 것이 가장 권장됨(KDIGO 2024)
...
* BUN/Cr ratio : 정상=10\~20/1 (비율 ＞20/1인 경우 prerenal azotemia 시사; 단, 상부위장관 출혈·고단백 식이·고용량 steroid·이화항진 상태 등 prerenal 없이도 BUN만 상승하는 신외성 원인이 있어 병력과 함께 해석)
...
* CrCl(㎖/분) = {(140-연령) × 몸무게(㎏)} ÷ {72 × s-Cr} × 0.85(if female)
  * Cockcroft-Gault 공식은 **체표면적(1.73㎡) 보정을 하지 않는 실측 청소율(㎖/분)**이며, MDRD·CKD-EPI(㎖/분/1.73㎡, 체표면적 보정)와 단위가 다르므로 혼동하지 않도록 주의 — 약물 용량 계산 시에는 이 실측치가 그대로 사용되는 경우가 많음
...
FDA 2020년 개정 가이던스 이후, 대부분의 약물은 신기능 저하 시 용량 조절 기준으로 Cockcroft-Gault CrCl 대신 **eGFR(CKD-EPI)**을 사용하도록 권장됨. 다만 DOAC(예: dabigatran) 등 임상시험이 CrCl 기반으로 설계된 일부 약물은 여전히 Cockcroft-Gault 공식을 따라야 하므로, 개별 약제의 첨부문서를 확인할 것. 국내 실무에서는 HIRA 급여 기준·약제 첨부문서가 여전히 CrCl(Cockcroft-Gault)을 요구하는 경우가 있으므로, 약제별 기준을 개별 확인 필요.
{% endhint %}
...
* 신장 기능 평가 도구로 활용; 신장의 여과 능력을 반영
* 일반적으로는 Cr을 기초로 하는 계산식에 의해 산출한 eGFR을 사용
  * 현재(NKF-ASN 2021 합의, KDIGO 2024)는 인종 보정을 제거한 **CKD-EPI 2021 크레아티닌 공식을 1차 권장식**으로 사용 (MDRD는 과거 표준식으로, 아래 참고 박스 참조)
  * 국내 대부분의 검사실은 IDMS-traceable(표준화) 크레아티닌 측정법을 사용하므로, 실무에서는 IDMS-traceable 방식의 공식 적용이 일반적
...
* CKD는 **원인(Cause)-GFR-알부민뇨(CGA)** 형식으로 표기 : 예) diabetic kidney disease G3aA2
