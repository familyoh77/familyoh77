...
```mermaid
graph TD
    A["선별검사 논의: 공유 의사결정<br/>①의사-환자 참여 ②정보 공유<br/>③공감대 형성 ④결정에 대한 합의"] --> B["PSA 기반 선별검사 결정"]
    B --> C["PSA 선별검사 시작 연령<br/>평균 위험: 45~50세<br/>증가된 위험: 40~45세"]
    C --> D["PSA Test"]
    D --> E{"PSA/위험도 평가"}
    E -->|정상| F["선별검사 간격<br/>2~4년마다(50~69세)<br/>위험도에 따라 개인화·중단"]
    F -.선별검사 지속 결정.-> D
    E -->|증가| G["Confirmatory PSA<br/>+ 직장수지검사(선택)"]
    G --> H{"PSA/위험도 재평가"}
    H -->|증가| I["생검 등 진행 논의<br/>(비뇨의학과 의뢰)"]
    H -->|정상| F
