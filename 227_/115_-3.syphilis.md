...
* 전염 경로 : 감염 병소에 대한 직접 접촉(성관계), 감염 산모의 경태반 수직감염(출산 중 산도 접촉에 의한 감염은 드묾)
  * 수혈에 의한 전염은 극히 드묾 (✽공여자의 혈액을 4℃ 냉장 저장 시 대개 24\~48시간 이내 사멸하나, 균량에 따라 최대 96시간까지 생존이 보고된 연구도 있음)
* 법정감염병 : 3급 (전수감시) (☞ [성매개감염병](115_-1.STI.md))
* 유병률 (국내, 2024년) : 연간 신고 2,790명(인구 10만 명당 5.4명); 남성이 78.0%로 여성 대비 발생률 약 3.5배; 20\~30대가 58.6% 차지; 병기별로는 조기 잠복매독(43.7%) > 1기(35.2%) > 2기(18.8%) > 3기(1.8%) > 선천매독(0.4%) 순
...
* 원인균 : _Treponema pallidum_ subsp. _pallidum_ - 나선형 스피로헤타(spirochete); 표준 배지에서 배양되지 않아 암시야 현미경 또는 분자 진단으로만 직접 확인 가능
* 위험 요인
...
{% hint style="info" %}
**Reverse sequence screening** : 최근 다수의 대형 검사실은 treponemal test(EIA/CIA)를 먼저 시행하고 양성인 경우에만 non-treponemal test로 확인하는 역순 알고리듬을 사용함. 두 검사 결과가 불일치하면(예: treponemal 양성 + non-treponemal 음성) 두 번째 treponemal test(TPPA 등)로 재확인 필요. ✽국내 일부 검사실은 아직 non-treponemal test 우선 알고리듬을 사용하므로, 검사실 정책 확인 후 해석 필요
{% endhint %}

### <mark style="color:orange;">암시야 현미경 검사</mark>

* Darkfield microscopy of skin lesion : 병변에서 직접 균을 확인; 병변 초기 혈청 검사 위음성 구간에서 유용

#### <mark style="color:cyan;">**단계별 검사 민감도**</mark>

<table><thead><tr><th width="140">병기</th><th width="156.6666259765625">Non-treponemal</th><th width="183.3333740234375">Treponemal-specific</th><th>Darkfield</th></tr></thead><tbody><tr><td>1기</td><td>78~86%</td><td>76~84%</td><td>80%</td></tr><tr><td>2기</td><td>100%</td><td>100%</td><td>80%</td></tr><tr><td>3기</td><td>71~73%</td><td>94~96%</td><td>-</td></tr><tr><td>잠복매독</td><td>95~100%</td><td>97~100%</td><td>-</td></tr></tbody></table>
...
* 신경학적 증상이 있으면 반드시 시행하며, HIV 감염에서 CD4 ≤350/㎣ 또는 non-treponemal titer ≥1:32인 경우 검사 역치를 낮춰 고려할 수 있음 (단, 무증상 환자에서 이 기준만으로 시행할 임상적 이득은 확립되지 않음)
...
```mermaid
graph TD
    A([매독 의심 병력·병변 또는 <br/>선별검사 대상]) 
--> B[Non-treponemal test:<br/>RPR/VDRL]
    B --> C[양성?]
    C -->|음성이나 병변 존재| D[초기 위음성 가능: 2~4주 후 <br/>재검 또는 darkfield 고려]
    C -->|양성| E[Treponemal-specific <br/> test로 확인: FTA-ABS/TPPA]
    E --> F[병기 확인]
    F --> F1[1기·2기<br/>·조기 잠복매독] --> G[Early syphilis 치료]
    F -->F2[후기 잠복·3기<br/>·병기 불명] --> H[Late syphilis 치료]
    F -->F3[신경학적·안과<br/>·이과 증상 동반] -- CSF 검사 --> I[신경매독 치료]

style A fill:#eeeeee,stroke:#888888,stroke-width:2px
classDef yellow fill:#fff9c4,stroke:#ffe082
class C,F yellow
classDef white fill:#fff,stroke:#333
class F1,F2,F3 white
classDef sky fill:#e3f2ff,stroke:#2196f3
class G,H,I sky
classDef lightGreen fill:#e8f8e8,stroke:#4caf50
class B,D,E lightGreen
```
...
* **Azithromycin은 매독 치료에 권장되지 않음** : 과거 azithromycin 2 g 단회 요법이 대체제로 쓰인 적이 있으나 현재는 권장되지 않음. _T. pallidu&#x6D;_&#xC758; macrolide 내성이 광범위하게 보고되어 다른 대체제(doxycycline, ceftriaxone)를 모두 사용할 수 없는 극히 예외적 상황에서만 고려하도록 제한함 \[CDC]. 사용 시 밀착 혈청 추적이 필수.
...
* benzathine penicillin G : 240만 U IM 1회 (다른 항생제 추가 효과는 없음) <mark style="color:blue;">\[마이신 주]</mark>
  * [ ] 조기 매독에서 benzathine penicillin G 단회 투여는 3회 반복 투여 대비 치료 성적 차이가 없으며, 이는 HIV 감염 여부와 무관함이 대규모 RCT로 확인됨
...
* ceftriaxone : 2 g qd IM/IV ×10\~14일 (최적 용량·기간에 대한 근거는 제한적)
...
* procaine penicillin G : 240만 U/d IM + probenecid 500 ㎎ qid ×10\~14일

✽ 안매독·이매독(시력저하·포도막염·이명·난청 등)은 CSF 검사 결과와 무관하게 위 신경매독 요법과 동일하게 치료함(CDC 2021)

- [ ] Procaine penicillin은 정맥으로 주입되면 급성 정신증·경련 등(Hoigne syndrome)을 유발할 수 있으므로 반드시 근육주사로 투여
...
* CSF 검사 재시행(정상화될 때까지 통상 6개월 간격) - 단, HIV 미감염이거나 HIV 감염이라도 ART 복용 중이며 혈청학적·임상적 반응이 충분하면 반복 CSF 검사는 불필요
...
* CD4 ≤350/㎣ 또는 non-treponemal titer ≥1:32인 경우 무증상 신경매독 위험이 높아 CSF 검사 역치를 낮춰 고려&#x20;
...
{% hint style="info" %}
**Doxy-PEP(노출 후 예방요법)** - 세균성 STI(매독·클라미디아·임질) 공통 예방 전략으로, 매독에서도 유의한 감염 감소 효과가 보고됨 (☞ [성매개감염병](115_-1.STI.md#doxy-pep))
{% endhint %}
