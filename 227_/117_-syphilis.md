...
* *Treponema pallidum* 감염에 의한 만성 전신성 성매개질환
* 다른 이름 : lues
...
* 치료받지 않은 환자에서의 late syphilis; cardiovascular syphilis, gummatous Dz를 포함한 증상 발생
...
```mermaid
graph TD
    A([매독 의심 병력·병변 또는 선별검사 대상]) --> B[Non-treponemal test<br/>RPR/VDRL]
    B --> C{양성?}
    C -->|음성이나 병변 존재| D[초기 위음성 가능<br/>2~4주 후 재검 또는 darkfield 고려]
    C -->|양성| E[Treponemal-specific test로 확인<br/>FTA-ABS/TPPA]
    E --> F{병기 확인}
    F -->|1기·2기·조기 잠복매독| G[Early syphilis 치료]
    F -->|후기 잠복·3기·병기 불명| H[Late syphilis 치료]
    F -->|신경학적·안과·이과 증상 동반| I[CSF 검사 → 신경매독 치료]
...
과거 azithromycin 2 g 단회 요법이 대체제로 쓰인 적이 있으나, *T. pallidum*의 macrolide 내성이 광범위하게 보고되어 CDC 2021 지침은 다른 대체제(doxycycline, ceftriaxone)를 모두 사용할 수 없는 극히 예외적 상황에서만 고려하도록 제한함. 사용 시 밀착 혈청 추적이 필수.
{% endhint %}
...
>
> ```
> 벤자틴 페니실린G(benzathine penicillin G) 120만 U/V  2V  1회 IM
> ```
>
> _✽ 좌우 둔부에 나누어 주사(1V씩); 주사 후 30분간 관찰(아나필락시스 대비); 치료 시작 24시간 이내 발열 등 Jarisch-Herxheimer 반응 가능함을 사전 안내_ <mark style="color:blue;">\[마이신 주]</mark>

> **처방례 2.** Early syphilis, 페니실린 사용 불가 시(비임신)
>
> ```
> 독시사이클린(doxycycline) 100 ㎎/C  2C #2  ×14일
> ```
>
...
>
> ```
> 벤자틴 페니실린G(benzathine penicillin G) 120만 U/V  2V  1주 간격 IM ×3회 (총 6V)
> ```
>
> _✽ 1주 간격을 넘겨 재방문 시(특히 14일 초과) 전체 요법 재시작 원칙; 임신부도 동일 요법_ <mark style="color:blue;">\[마이신 주]</mark>
...
> 3. 임신부에서는 이 반응이 조기진통을 유발할 수 있어 산과와 사전 협의가 필요합니다.
