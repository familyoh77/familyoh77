...
flowchart LR
    subgraph Col1 [" "]
        ...
        NodeA["급성 renal colic 의심"] --> NodeB["병력·신체검사<br>U/A ± urine culture<br>Cr/eGFR ± CBC/CRP"]

        NodeB --> NodeC{"응급/복잡성 소견?<br>• 발열/UTI 또는 sepsis<br>• anuria<br>• solitary kidney/양측 폐쇄<br>• 신기능 악화<br>• 조절되지 않는 통증·구토"}

        NodeC -- yes --> NodeD["즉시 영상검사<br>+ 비뇨의학과 의뢰"]

        NodeD --> NodeE{"폐쇄 + 감염/sepsis?"}
        NodeE -- yes --> NodeF["응급 감압<br>stent 또는 nephrostomy<br>+ 즉시 항생제"]
        NodeE -- no --> NodeG["적극적 치료 평가"]

        NodeC -- no --> NodeH["영상검사<br>US 우선 고려<br>필요 시 non-contrast CT"]

        NodeH --> NodeI{"요관결석 확인?"}
        NodeI -- no --> NodeJ["다른 진단 평가"]
        NodeI -- yes --> NodeK(("A"))
    end
    ...
    subgraph Col2 [" "]
        ...
        NodeK2(("A")) --> NodeL{"적극적 제거술 적응증?<br>• 자연 배출 가능성 낮음<br>• 지속 통증/폐쇄<br>• 신기능 저하"}

        NodeL -- yes --> NodeM["비뇨의학과 의뢰<br>URS/SWL 등 고려"]
        NodeL -- no --> NodeN["보존적 치료<br>• NSAID 중심 진통<br>• 적절한 수분섭취<br>• 결석 회수 안내"]

        NodeN --> NodeO{"MET 고려 대상?"}
        NodeO -- yes --> NodeP["α-blocker MET 고려<br>예: tamsulosin 0.4 mg qd<br>(off-label)"]
        NodeO -- no --> NodeQ["경과 관찰"]

        NodeP --> NodeR{"경과 악화?"}
        NodeQ --> NodeR

        NodeR -- "발열/UTI 또는 sepsis 의심" --> NodeD
        NodeR -- "난치성 통증,<br>신기능 악화, 지속 구토" --> NodeM
        NodeR -- no --> NodeS{"결석 배출 확인?"}

        NodeS -- yes --> NodeT["stone analysis<br>재발 위험도 평가"]
        NodeS -- no --> NodeU["증상·결석 크기/위치에 따라<br>적절한 시점에 추적 영상"]

        NodeU --> NodeV{"자연 배출 실패 또는<br>지속 폐쇄/증상?"}
        NodeV -- yes --> NodeM
        NodeV -- no --> NodeR
    end
    ...
    class NodeC,NodeE,NodeI,NodeL,NodeO,NodeR,NodeS,NodeV decision;
    class NodeD,NodeF,NodeG,NodeJ,NodeM alert;
    class NodeB,NodeH,NodeT,NodeU greenBox;
    class NodeN,NodeP,NodeQ blueBox;
    class NodeK,NodeK2 conn;
```
