...
* 약물 : carbonic anhydrase inhibitor(topiramate, zonisamide, acetazolamide), steroid, antiretroviral protease inhibitor(indinavir), 통풍 치료제(probenecid), 이뇨제(furosemide, bumetanide, torsemide, triamterene), 충혈제거제(guaifenesin, ephedrine)
...
* 기저 질환 : 부갑상선항진증, 통풍, 비만, 당뇨병, distal renal tubular acidosis, IBD, 흡수 억제(비만) 수술, short bowel syndrome
...
* 24시간 소변 : 요량, Cr, pH, Ca, P, Na(＞150 m㏖/d-Na 섭취 과다), uric acid, oxalate, citrate, sulfate(＞20 mEq/d-동물성 단백질 섭취 과다)
* 혈액 : Na, P, Ca, uric acid, Cr, PTH(칼슘 증가 시)

### 영상 검사

* 통증이 해소된 환자의 25%에서는 체내에 결석이 남아 있으므로 육안으로 결석 배출이 확인되지 않았으면 후속 영상 검사를 요함
* 복부 X선
...
* 통증, 압통
  * 복부 : 위장관 질환, 담낭염&#x20;
  * 옆구리 : 월경통, 대상포진, 근골격 염증/경직, 신우신염, 담낭 연관통(우측), 난소 낭종 비틀림
...
flowchart LR
    subgraph Col1 [" "]
        direction TD
        NodeA["급성 Renal colic<br>신체검사상 요로 결석 의심"] --> NodeB{"U/A상 혈뇨?"}

        NodeB -- no --> NodeC["다른 진단 고려"]
        NodeB -- yes --> NodeD["진통제"]

        NodeD --> NodeE{"• 증상 완화가 진통제로 불충분한가?<br>• 요로 감염 또는 완전 폐쇄가 의심되는가?<br>• 신장 기능이 한쪽에만 유지되는가?<br>• 임신이 의심되는가?<br>• 환자가 >60세인가? 특히 동맥병증을 앓고 있는가?"}

        NodeE -- yes --> NodeF["즉시 의뢰"]
        NodeE -- no --> NodeG["• U/A, 소변 배양 검사<br>• 1주 내 영상 검사"]

        NodeG --> NodeH{"신장 결석 확인?"}
        NodeH -- no --> NodeI["다른 진단 고려"]
        NodeH -- yes --> NodeJ(("A"))
    end

    subgraph Col2 [" "]
        direction TD
        NodeA2(("A")) --> NodeK{"결석 크기 >10 mm?<br>Hydronephrosis?"}

        NodeK -- yes --> NodeL["의뢰"]
        NodeK -- no --> NodeM["• 통증 완화를 위한 약물 투여<br>• 결석 배출을 위한 약물 치료<br>• 1~14일마다 영상 검사<br>• 소변에서의 결석 수집"]

        NodeM --> NodeN{"4~6주 내 결석 배출?"}

        NodeN -- no --> NodeO["의뢰"]
        NodeN -- yes --> NodeP["• 병력<br>• Cr, Ca, uric acid levels<br>• U/A, 소변 배양 검사<br>• 추가 영상검사 고려"]

        NodeP --> NodeQ{"결석 재발 고위험?"}

        NodeQ -- no --> NodeR["• 충분한 수분 섭취, 균형 있는 식사, 적정 체중 유지<br>• 재발 시 이뇨제, citrate, allopurinol 투여 고려"]
        NodeQ -- yes --> NodeS["• 추가의 metabolic evaluation<br>• 개별적 예방법 강구"]
    end

    %% 두 서브그래프 간 비가시성 연결선 추가 (좌우 정렬 강제)
    Col1 ~~~ Col2

    style Col1 fill:none,stroke:none
    style Col2 fill:none,stroke:none

    classDef startNode fill:#ECE6F4,stroke:#D1C4E9,stroke-width:1px;
    classDef decision fill:#FEFCE8,stroke:#FEF08A,stroke-width:1px;
    classDef alert fill:#FEE2E2,stroke:#FCA5A5,stroke-width:1px,color:#991B1B;
    classDef greenBox fill:#DCFCE7,stroke:#86EFAC,stroke-width:1px;
    classDef blueBox fill:#E0F2FE,stroke:#7DD3FC,stroke-width:1px;
    classDef conn fill:#475569,stroke:#334155,stroke-width:1px,color:#FFFFFF;

    class NodeA startNode;
    class NodeB,NodeE,NodeH,NodeK,NodeN,NodeQ decision;
    class NodeC,NodeF,NodeI,NodeL,NodeO alert;
    class NodeG,NodeP greenBox;
    class NodeD,NodeM,NodeR,NodeS blueBox;
    class NodeJ,NodeA2 conn;
```

급성 신장 결석의 진단 및 관리 알고리듬\
Ref. Kidney Stones: Treatment and Prevention. AFP 2019;99(8). Fig 1.
...
* hydration : 목표 소변량 2\~2.5 L/d (소변 SG ＜1.010; 맑거나 옅은 노란색 소변), 수분 섭취량 2.5\~3 L/d (비만한 경우 더 많이 필요할 수 있음)
  * 음식을 통하여 0.5\~1 L/d를 섭취하므로 이를 고려하여 추가 수분 섭취량을 권고
  * 한계 : 수분 섭취를 늘리는 것으로 통증이 완화되거나 결석의 통과가 빨라지지 않음, 지나친 수분 섭취는 전해질 균형을 해칠 수 있음
...
* tamsulosin : 0.4 ㎎/d \[하루날 디] (☞ [전립선비대증](124_-benign-prostatic-hyperplasia-bph.md#a1-a1-adrenoceptor-antagonist))
* doxazosin : 4 ㎎/d \[카두라]
...
* scopolamine butylbromide : 10\~20 ㎎ tid\~qid \[부스코판]
...
* 대상 : ＞1.5\~2 ㎝, complex calculi(예: staghorn calculi), cystine stone, 해부학적 이상(예: horseshoe kidney, ureteropelvic junction obstruction), calyceal diverticula 내 결석

## 예방

* 충분한 수분 섭취; 재발 감소 효과가 있는 것으로 알려져 있으나 증거는 불확실함
* 균형잡힌 식단 : 충분한 식이 섬유 섭취, 적정 칼슘(1\~1.2 g/d), 소금 제한(4\~5 g/d)
  * 동물성 단백질 섭취 제한 : 계란, 생선, 육류, 가금류 등; 0.8\~1 g/㎏/d; 동물성 단백질은 소변 pH↓, Ca/oxalate/uric acid 배출↑, citrate 배출↓를 유발함
...
* Ca oxalate stone
  ...
  * 권고 : 적당한 식이 Ca, K, phytat
  - [ ] Ca 섭취 부족 → 장에서의 Ca 흡수률↑, Ca에 결합된 oxalate가 변으로 배출되지 못하고 함께 흡수됨 → 소변 내 oxalate 농도↑ → 요로 결석 형성↑; 식이 칼슘 섭취는 신장 결석의 빈도를 감소시킴
* Hypo-citraturia
...
후로스판 80 ㎎/T 6T #3\
타이레놀 이알 650 ㎎/T 3T #3\
애니펜 300 ㎎/T 3T #3 통증 완화를 위하여 필요시 타이레놀 사이에 투여

\
처방례 2. 배출 촉진\
알기론 50 ㎎/T 3T #3\
카두라 엑스엘 4 ㎎/T 1T qd (보험주의)
