...
<mark style="color:$warning;">**당일 평가 필요**</mark>
...
> ```
>
> _✽ NSAIDs가 1차 선택; 통증 재발 시 반복 투여 가능 (최대 5일). 고령·저체중·신기능 저하에서는 15 ㎎ 등 감량 고려. 신기능 저하·출혈 경향·탈수 등 급성 신손상 위험이 있으면 ketorolac을 피하고 원인·신기능·호흡억제 위험을 고려하여 acetaminophen 또는 적절한 대체 진통제를 개별화하여 선택. 통증 조절 후 비조영 CT로 결석 확인 권장(임신 시 초음파 우선)_

> **처방례 2. 중등도\~중증 급성 복통 (비opioid 진통제로 불충분하거나 금기인 경우, 진단 미확정 상태에서의 단기 통증 조절)**
>
> ```
> 아세트아미노펜 650 ㎎ q6h prn - 1차 고려
> 트라마돌 캡슐 50 ㎎ 1회 투여 후 재평가; 필요시 q6-8h prn (일반 성인에서 최대 400 ㎎/d)
> 맥페란정 10 ㎎ 오심 동반 시 필요시 투여 (1일 최대 3회)
> ```
>
> _✽ 진단 확인 전이라도 적절한 통증 조절은 표준 치료이나, 진단 미확정 상태에서의 반복적 opioid 투여는 증상 변화 인지와 재평가를 방해할 수 있으므로 경증\~중등도는 acetaminophen 또는 원인에 적합한 NSAID를 우선 고려하고, tramadol은 비opioid로 불충분하거나 금기인 경우 최소유효용량·최단기간·재평가를 전제로 사용. Tramadol은 세로토닌 증후군·경련·어지럼·낙상 위험이 있어 serotonergic drug(SSRI/SNRI/MAOI 등) 병용·간질 병력자에서 주의; 항경련제(gabapentinoid) 또는 항콜린제 병용 시 CNS 억제·호흡 억제 또는 진정·착란·변비 심화 위험이 상가적으로 증가하므로 병용 여부를 반드시 확인; 고령자, 신·간기능 저하에서는 감량 또는 투여 간격 연장 필요. Metoclopramide는 1회 10 ㎎, 1일 최대 3회, 단기간 사용을 원칙으로 하며, 장폐쇄뿐 아니라 위장관 출혈·천공이 의심되는 경우에도 사용을 피하고 추체외로 증상(EPS) 발생 가능성을 설명. 진단 미확정 환자는 1회 투여 후 통증·전신 상태 재평가와 disposition 결정이 우선임_

> **처방례 3. 급성 췌장염 의심 - 응급실 이송/입원 평가 전 통증 조절**
>
> ```
> 트라마돌 50 ㎎ 1회 IV/IM (경구 섭취·흡수가 불확실할 수 있어 비경구 경로 우선), 이송 전 1회 투여 후 재평가
> ```
>
> _✽ 급성 췌장염이 의심되면 응급실/입원 평가가 원칙이며, 위는 이송 전 1회성 통증 조절의 예시임. 반복 경구 처방(q6h prn 등)은 이송 전 상황에 적합하지 않으므로 지양하고, 통증·전신 상태를 재평가하며 disposition을 결정. 저혈량 소견이 있으면 IV fluid(LR 우선)를 활력징후·BUN/Cr·소변량 등에 따라 개별화하여 투여하고 반복 재평가(용량·속도는 환자 상태에 따라 달라지므로 처방례로 고정하지 않음); 과도한 aggressive hydration보다 임상 상태에 맞춘 조절이 원칙이며, 고령·심부전·CKD에서는 volume overload에 특히 주의. 급성 췌장염은 원칙적으로 NPO를 유지하는 질환이 아니며, 구역·구토가 없고 임상적으로 안정되면 24\~48시간 이내 저지방 고형식으로 조기 경구 섭취를 시작 \[ACG 2024]. 췌장염에서 opioid 사용은 과거 Oddi 괄약근 경련 우려로 기피했으나 임상적 근거는 불충분하며 적절한 통증 조절이 우선. 중등도 이상은 입원 의뢰_
