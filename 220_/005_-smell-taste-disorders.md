...
* **환각취(Phantosmia)** : 아무런 냄새 자극이 없는 상황에서 냄새를 느끼는 증상
  * 감별 중요 : 측두엽 간질(Temporal lobe epilepsy), 두개내 종양(특히 전두엽·측두엽), 파킨슨병, 정신질환(조현병, 우울증) - 새로 발생한 Phantosmia는 영상 검사 고려
...
* 새로 발생한 Phantosmia - 측두엽 간질, 두개내 종양 감별 필요
...
```mermaid
graph TD
    A([후각·미각 이상 호소]) --> B{"① 진짜 미각 문제?\n단맛·짠맛·신맛 구분 가능?"}
    B -->|"YES → 진짜 미각 장애\n구강·약물 원인 우선"| C["구강건조·약물 부작용\n구강 내 원인 확인"]
    B -->|"NO → 풍미·향 전체 감소\n→ 후각 문제 가능성 높음"| D{"② Red Flag?"}
    C --> D
    D -->|"편측 소실+신경학적 증상\n두통·시야 이상·안면마비"| E["🚨 영상검사\nMRI / CT\n신경과 의뢰"]
    D -->|없음| F{"③ 발병 양상 및 동반 증상"}
    F -->|"급성\nURI·COVID·약물 복용"| G["Post-viral / 약물\nSensorineural"]
    F -->|"두부 외상 후"| H["Post-traumatic\nSensorineural"]
    F -->|"코막힘·콧물·비용종 동반"| I["Conductive\n비부비동 원인"]
    F -->|"서서히 진행\n파킨슨 징후·인지저하"| J["신경퇴행성 의심"]
    G --> K["④ 약물 검토\n기본 검사\nCBC·TSH·B12·Zinc"]
    H --> K
    I --> L["Conductive 치료\nINCS Mygind position\nCRS → ENT 의뢰"]
    J --> M["신경과 의뢰\nParkinson/Alzheimer 선별"]
    K --> N["Sensorineural 치료\n후각 훈련 ≥12주 ⭐\n전신 스테로이드 routine X\nZinc: 결핍 확인 시만 보충\n3개월 지속 → ENT 의뢰"]
    style E fill:#f44336,stroke:#b71c1c,color:#fff,stroke-width:2px
