...
* 새로 발생하거나 지속되는 원인 불명의 Phantosmia → ENT 평가 및 비내시경 고려; 편측성, 신경학적 증상, 비전형적 경과 또는 원인 불명 시 두개내 병변·측두엽 간질 등을 감별하기 위한 영상검사/신경과 평가 고려
...
* 후각 기능 검사 : KVSS II (Korean Version of Sniffin' Sticks II) - 이비인후과 의뢰
  ...
  * TDI 점수 (3항목 합산, 최고 48점) : 연령 및 검사실 기준을 함께 고려하여 판정하며 단일 ＜30점 기준으로 일괄 판정하지 않음; 국내 연구에서 제시된 cut-off는 연구·대상에 따라 차이가 있음
* 미각 기능 검사 : 이비인후과 의뢰
...
* Modified Training Protocol : 12주마다 향 세트를 변경하는 방법이 연구되어 있으며 일부 후각 지표의 추가 개선 가능성이 보고됨
...
>
> ```
> Prednisolone   단기요법   (용량·기간은 CRS 중증도와 환자 위험에 따라 결정)
...
> 4. 12주마다 향 종류를 바꾸는 modified training도 연구되어 있으며 일부 후각 지표의 추가 개선 가능성이 보고되었습니다.
> 5. 냄새를 맡을 때 해당 냄새를 의식적으로 인지하고 해당 사물을 떠올리며 집중해 보십시오.
...
바이러스 감염(COVID-19 포함) 후 후각 손실은 많은 경우 수개월 내 자연 회복되며, 후각 훈련은 후각 회복을 돕는 가장 근거가 축적된 비약물 치료입니다.
{% endhint %}
