...
#### <mark style="color:$primary;">선별검사(Screening)</mark>

* 65세 이상에서는 진료 시 맥박 촉지 또는 리듬 스트립을 이용한 기회적 선별을 고려
* 75세 이상이거나 뇌졸중 고위험군에서는 장기 ECG 모니터링(패치·홀터·삽입형 기록계) 기반의 체계적 선별을 고려할 수 있음
* 스마트워치 등 소비자용 웨어러블의 AF 알림은 선별 단서로 활용할 수 있으나, 반드시 12-유도 ECG 또는 리듬 스트립으로 확진 후 진단
...
```mermaid
graph TD
    Start(["신규 AF 발견"]) --> Unstable["혈역학적 불안정 또는<br/>Red Flag 동반?"]
    Unstable -->|YES| Emergency["즉시 응급 이송<br/>필요 시 synchronized cardioversion"]
    Unstable -->|NO| Workup["<u>병력·신체검진·기본검사</u><br/>ECG, CBC, 전해질, 신·간기능, TSH, 심초음파"]
    Workup --> Score["뇌졸중 위험도 평가<br/>CHA₂DS₂-VASc(국내) / CHA₂DS₂-VA(ESC 2024)"]
    Score -->|"저위험"| NoOAC["항응고제 불필요"]
    Score -->|"항응고 적응증"| OAC["DOAC 우선 고려<br/>금기 시 VKA"]
    Score -->|"중간 위험"| Consider["추가 위험인자·출혈위험·<br/>환자선호를 고려하여 개별화"]
    OAC --> Rate["<u>심박수 조절</u><br/>β-차단제·non-DHP CCB·Digoxin"]
    Consider --> Rate
    NoOAC --> Rate
    Rate --> RhythmQ["증상 지속·젊은 환자·<br/>최근 진단·HFrEF 등?"]
    RhythmQ -->|YES| Rhythm["<u>조기 리듬 조절 논의</u><br/>항부정맥제·전기적 심율동전환·<br/>카테터 절제술 - 의뢰"]
    RhythmQ -->|NO| Followup["<u>위험인자 관리+재평가</u><br/>혈압·체중·음주·OSA·당뇨"]
    Rhythm --> Duration["AF 지속시간 ≤24시간<br/>확실한가?"]
    Duration -->|YES| Cardiovert["심율동전환 고려<br/>혈전·뇌졸중 위험도에 따라 OAC"]
    Duration -->|"NO / 불확실"| TEE["TEE로 혈전 배제<br/>또는 3주 이상 적절한 OAC 후 전환"]
    Cardiovert --> PostCV["대부분 심율동전환 후<br/>최소 4주 OAC"]
    TEE --> PostCV
    PostCV --> Followup
    Followup --> Reassess["<u>동적 재평가</u><br/>AF 재발·증상·신기능·위험인자"]

style Start fill:#eeeeee,stroke:#888888,stroke-width:2px
style Emergency fill:#ffcdd2,stroke:#c62828
style Rhythm fill:#fdebd0,stroke:#e67e22
classDef yellow fill:#fff9c4,stroke:#ffe082
class Unstable,Score,RhythmQ,Duration yellow
classDef lightGreen fill:#e8f8e8,stroke:#4caf50
class Workup lightGreen
classDef sky fill:#e3f2ff,stroke:#2196f3
class OAC,Rate,PostCV sky
classDef blue fill:#d0e8ff,stroke:#1a6abf
class Followup,Cardiovert,TEE,Reassess blue
```
...
* 국내 건강보험 급여기준 : 비판막성 AF이면서 CHA₂DS₂-VASc 1점 이상(또는 이에 준하는 뇌졸중 위험인자 동반) 시 NOAC 급여 인정; 처방 전 각 약제의 최신 급여기준 확인
...
<table data-header-hidden data-search="false"><thead><tr><th width="214"></th><th width="60"></th><th width="326"></th></tr></thead><tbody><tr><td><strong>항목</strong></td><td><strong>점수</strong></td><td><strong>내용</strong></td></tr><tr><td><strong>H</strong>ypertension</td><td>1</td><td>조절되지 않는 고혈압(수축기 ＞160 mmHg)</td></tr><tr><td><strong>A</strong>bnormal renal/liver function</td><td>1 또는 2</td><td>신기능 이상(투석·이식·Cr ＞2.26 ㎎/㎗) 1점, 간기능 이상(간경변·빌리루빈/효소 이상) 1점 - 각각 해당 시 가산</td></tr><tr><td><strong>S</strong>troke</td><td>1</td><td>뇌졸중 과거력</td></tr><tr><td><strong>B</strong>leeding</td><td>1</td><td>출혈 병력 또는 출혈 소인</td></tr><tr><td><strong>L</strong>abile INR</td><td>1</td><td>불안정한 INR(와파린 복용 시 치료 범위 내 시간 비율 낮음)</td></tr><tr><td><strong>E</strong>lderly</td><td>1</td><td>65세 초과</td></tr><tr><td><strong>D</strong>rugs/alcohol</td><td>1 또는 2</td><td>출혈 유발 약물(항혈소판제·NSAID) 병용 1점, 과음 1점 - 각각 해당 시 가산</td></tr></tbody></table>

* 3점 이상이면 출혈 고위험군으로 간주하여 더 자주 추적하고 교정 가능한 위험인자를 재평가
...
<table><thead><tr><th width="146">약제</th><th>처방 예시</th><th>주의</th></tr></thead><tbody><tr><td>β-차단제</td><td>콩코르(비소프롤롤) 2.5~5 ㎎ #1 또는<br>푸로롤서방정(메토프롤롤 숙신산염) 47.5 ㎎ #1</td><td>심박수 조절의 대표적 1차 약제; 천식·중증 기관지경련, 급성 비대상성 심부전에서는 주의; HFrEF에서는 적절한 β-차단제 선택</td></tr><tr><td>non-DHP CCB</td><td>헤르벤 서방정 90 ㎎ #2 또는<br>이솝틴 서방정 240 ㎎ #1</td><td>딜티아젬·베라파밀; LVEF ≤40%인 HFrEF에서는 사용하지 않음</td></tr><tr><td>Digoxin</td><td>디곡신 0.0625~0.25 ㎎ #1</td><td>HFrEF 또는 다른 약제의 효과·내약성이 제한적인 경우 고려; 운동 시 심박수 조절 효과가 제한적이며 신기능에 따라 용량 조절 필요</td></tr></tbody></table>
...
* 카테터 절제술 : 약물 치료에 실패하거나 불내성이 있는 증상성 AF에서 권고되며, **증상성 발작성 AF에서는 항부정맥제보다 먼저 시행하는 1차 치료 옵션도 가능**(2024 ESC Class I); 특히 주요 재발 위험인자가 없는 환자에서 고려. HFrEF 또는 빈맥유발 심근병증이 의심되는 경우에는 더 적극적으로 고려
...
구조적 심질환이 없고 드물게 재발하는 증상성 발작성 AF 환자 중 일부에서, 증상 발생 시 플레카이나이드 또는 프로파페논을 자가 단회 복용하는 전략을 고려할 수 있다. 심실 반응이 과도하게 빨라지는 것을 막기 위해 AV node 차단제 병용이 필요하며, 첫 투여는 반드시 의료진 감시 하에 안전성(저혈압·서맥·심실 리듬 이상)을 확인한 후 자가 투여로 전환한다.
{% endhint %}
...
* 심율동전환 후에는 뇌졸중 위험인자가 없는 환자를 포함하여 원칙적으로 최소 4주간 항응고를 유지하며, 이후 장기 항응고 여부는 심율동 회복 여부가 아니라 뇌졸중 위험도에 따라 결정
...
I48.1 지속성 심방세동\
I48.2 만성 심방세동\
I48.9 상세불명의 심방세동 및 심방조동

_(감별진단에서 다룬 심방조동은 I48.3 정형 심방조동, I48.4 비정형 심방조동으로 별도 코딩)_
...
>
> _(60세 남성, 고혈압 동반, 증상성 발작성 AF, 심초음파상 구조적 이상 없음, 진단 후 1년 이내; CHA₂DS₂-VASc 1점)_
>
> ```
> 자렐토 20 ㎎/T  1T  #1 저녁 식사와 함께
> ```
>
> ※ CHA₂DS₂-VASc 1점(남성)은 항응고를 고려할 수 있는 구간으로, 추가 위험인자·출혈위험·환자선호를 함께 평가하여 결정한다. 위험인자가 전혀 없는 경우에는 항응고제 없이 리듬 조절 논의를 위한 의뢰만 진행한다. 젊고 증상성인 조기 AF에서는 항부정맥제 또는 카테터 절제술을 통한 조기 리듬 조절 여부를 순환기내과와 상의한다.
...
> * 다비가트란(프라닥사)은 캡슐을 열거나 씹지 말고 그대로 삼키십시오(개봉 시 흡수율 증가로 출혈 위험 상승). 습기에 민감하므로 원래 포장(블리스터)에 보관하고 미리 다른 약통에 옮겨 담지 마십시오.
> * 복용을 잊은 경우 : 1일 2회 약제(다비가트란·아픽사반)는 다음 복용 시간이 6시간 이상 남았으면 즉시 복용, 그 이내면 건너뛰고 다음 정규 시간에 복용. 1일 1회 약제(리바록사반·에독사반)는 당일 중 기억나면 복용하되 다음날로 넘어갔다면 건너뛰기. 두 번 복용을 한 번에 하지 마십시오.

> **콩코르(비소프롤롤)·푸로롤(메토프롤롤) / 헤르벤(딜티아젬) - 심박수 조절제**
>
...
* **커피·카페인** : 음주와 달리 적당량의 커피 섭취가 심방세동 재발을 늘린다는 근거는 부족하므로, 증상을 악화시키지 않는다면 반드시 끊을 필요는 없습니다
