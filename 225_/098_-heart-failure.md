...
<table><thead><tr><th width="125">질환</th><th width="250">HF와의 차이점</th><th>핵심 감별 포인트</th></tr></thead><tbody><tr><td>COPD·천식</td><td>호흡곤란·야간 wheezing으로 HF와 혼동(cardiac asthma)</td><td>폐기능 검사, BNP/NT-proBNP 정상, 흉부 X선상 심비대·폐부종 소견 없음</td></tr><tr><td>폐색전증</td><td>급성 호흡곤란·저산소증 공존 가능</td><td>D-dimer, CT-PA; 급성 발생·흉통·위험 인자(부동·수술 등) 확인</td></tr><tr><td>만성 신질환 (체액 과부하)</td><td>부종·호흡곤란 유사</td><td>BUN/Cr·단백뇨, 심초음파상 EF·충만압 정상 여부</td></tr><tr><td>간경변·저알부민혈증</td><td>하지 부종·복수 유사</td><td>LFT·알부민, 경정맥 확장 없음, BNP 정상 범위</td></tr><tr><td>정맥부전·림프부종</td><td>양측 또는 편측 하지 부종</td><td>호흡곤란·기좌호흡 없음, BNP 정상, 하지 정맥 초음파</td></tr><tr><td>비만·탈조건화</td><td>운동 시 호흡곤란·피로</td><td>NP 정상, 심초음파 정상, 운동부하 검사로 감별</td></tr></tbody></table>
...
* CHAMPIT 감별(초기 60\~120분 내 배제) + 임상 프로파일
...
4. ICD : post-MI ≥40일, LVEF ≤30%, NYHA class I, GDMT 유지 중, 기대 여명 ＞1년 (1차예방)
5. Statin : MI/ACS 병력
...
flowchart LR
    ...
    subgraph SB["Pre-HF (Stage B)"]
        direction TB
        SB_IN((" "))
        style SB_IN fill:#aaa,stroke:#888,r:5px
        SB_IN -->|"LVEF ≤ 40%"| B1["ACEi"]
        SB_IN -->|"LVEF ≤ 40% &\n최근 MI / ACEi 불내성"| B2["ARB"]
        SB_IN -->|"LVEF ≤ 40%"| B3["β-blocker"]
        SB_IN -->|"post-MI ≥40일;\nLVEF ≤ 30%;\nNYHA I;\nGDMT 유지;\n기대 여명 >1년"| B4["ICD"]
        SB_IN -.->|"비허혈성\n심근병증"| B5["유전 상담\n& 검사\n(별도 고려사항)"]
    end

    style SA fill:#f0fdf4,stroke:#86efac,stroke-width:1.5px
    style SB fill:#fefce8,stroke:#fde047,stroke-width:1.5px
    SA_OUT --> ARROW
    ARROW --> SB_IN
...
_✽유전 상담·검사(점선)는 비허혈성 심근병증 의심 시 별도 고려사항이며, Stage B 핵심 권고(ACEi/ARB/β-blocker/ICD/Statin)에 포함되지 않음_

_\*NP biomarker 선별 검사는 AHA/ACC/HFSA 2022에서 고려 가능하다고 언급된 정도이며 ESC 2021에서는 별도 권고하지 않음_
...
* 백신 접종 : 인플루엔자, 폐렴구균, COVID-19, RSV(60세 이상 고령 HF 환자에서 고려) (Stage C 이상)
...
* **Finerenone** <mark style="color:blue;">\[케렌디아]</mark> (비스테로이드성 선택적 MRA) : T2DM + CKD 환자의 심혈관 사고 및 HF 입원 예방 목적 - HFrEF의 표준 MRA를 대체하지 않음; 두 가지를 구분하여 처방 고려
{% endhint %}
...
{% hint style="info" %}
**SGLT2i 심부전 적응증 급여**\
비당뇨 HF 환자에서도 dapagliflozin·empagliflozin의 HF 적응증 급여가 인정되나, LVEF 구간(HFrEF/HFmrEF/HFpEF)별 급여 기준 및 병용 조건이 약제·시기에 따라 다를 수 있음\
...
{% hint style="info" %}
**엔트레스토 급여 기준**\
NYHA II\~IV, LVEF ≤40% (또는 기준에 따라 상이) 증상성 HFrEF에서 ACEi/ARB 대체 투여 시 급여 인정. NT-proBNP·LVEF 등 세부 요건은 고시에 따라 다르므로 처방 전 최신 기준 확인 필요\
✽정확한 급여 기준은 HIRA 고시를 반드시 확인할 것 (기준 변경 가능)
{% endhint %}
...
* ✽일부 오래된 post-hoc 분석에서 여성의 독성 위험이 더 높다는 보고가 있었으나 근거 수준이 낮아 별도 목표 농도를 강조하지 않음; 남녀 모두 0.5\~0.9 ng/㎖ 목표
...
Tirzepatide(GIP/GLP-1 이중 작용제) : SUMMIT 시험에서 비만 동반 HFpEF에서 CV 사망 및 HF 악화 이벤트 유의 감소 - GLP-1 RA 중 최초의 HF 하드 엔드포인트 감소 증거. 비만 표현형(obesity phenotype) HFpEF에서 우선적으로 고려할 수 있음.

단, HFrEF에서의 효과는 불확실(일부 데이터에서 심박수 증가·부정맥 위험 가능). 현재 가이드라인 공식 권고 이전이나, 비만 동반 HFpEF에서 SGLT2i와 병용 고려 가능; 두 약제의 상가 효과는 추가 연구 진행 중.

✽Tirzepatide는 2026년 기준 국내에서 비만·T2DM 적응증으로는 도입되어 있으나, 심부전 적응증은 아직 국내 승인 전이므로 처방 시 이를 명확히 인지할 것
{% endhint %}

**권고하지 않음 또는 회피**

* Non-DHP CCB(verapamil·diltiazem) : HFrEF에서 금기(음성 변력작용); DHP CCB(amlodipine·felodipine)는 HF 치료 목적은 아니나 병존 고혈압·협심증 등에 필요 시 사용 가능(mortality benefit 없음)
...
* CRT-D : NYHA II\~III(또는 보행 가능 IV) + LVEF ≤35% + 동성 리듬 + QRS ≥150 ms with LBBB (Class I); QRS 130\~149 ms with LBBB는 Class IIa
