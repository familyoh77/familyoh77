...
```mermaid
graph TD
    Start([눈꺼풀 가장자리 증상]) --> Red{Red Flag?}
    Red -- Yes --> Refer[응급 또는 당일 안과 평가]
    Red -- No --> Type{원인·표현형 감별}
    Type -- 딱지·궤양 --> Staph[포도알균성 - 세척 ± 국소 항생제]
    Type -- 기름진 인설 --> Seb[지루성 - 세척 + 피부질환 관리]
    Type -- 가려움·노출력 --> Contact[접촉피부염 - 원인 회피]
    Type -- 마이봄샘 이상 --> MGD[MGD - 온찜질·마사지·건성안 치료]
    Type -- Collarette --> Demo[Demodex - 구충 표적치료]
    Staph --> Follow{반응 충분?}
    Seb --> Follow
    Contact --> Follow
    MGD --> Follow
    Demo --> Follow
    Follow -- Yes --> Maint[원인별 유지 관리]
    Follow -- No --> Step4[진단 재평가·안과 의뢰]
