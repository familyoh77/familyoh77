...
* 혈액 요인 : 항응고제(와파린, DOAC-아픽사반·리바록사반·다비가트란 등)·항혈소판제(아스피린·클로피도그렐) 복용, 출혈 질환(혈소판 감소증 등)
  * 건강기능식품 문진 필요 : 오메가-3 고용량, 비타민 E, 은행잎 추출물(징코) 등도 출혈 경향에 영향을 줄 수 있음; 처방약이 아니어서 환자가 자발적으로 보고하지 않는 경우 많음
...
* 중등도 이상의 안구 외상, 관통상/고속 이물, 시력 저하·심한 통증·비정상 동공·전방출혈·심한 chemosis·안구돌출 동반 시 → 안구 파열 의심 → 즉시 안과 의뢰
* 외상 후 발생한 360° 또는 심한 bullous/circumferential 결막하출혈 → occult globe injury 의심 소견 → 즉시 안과 의뢰

<mark style="color:$warning;">**당일 또는 조기 의뢰**</mark>

* 3주 이상 지속 → 자연 흡수되지 않는 원인 감별 필요
* 재발성 SCH (특히 양측성·지속성 출혈이거나 다른 출혈 소견 동반 시) → 혈액 질환·혈관 질환 등 전신 원인 평가; 재발 간격이 짧을수록(예: 수주 이내) 응고 이상(coagulopathy) 평가 우선순위를 높임
* 항응고제 복용 환자 → Warfarin : INR 확인 및 용량 적절성 협의; DOAC : INR을 치료 모니터링 지표로 사용하지 않으며 복용 이행·용량·신기능 확인; 임의 중단 금지
...
* 임상적 진단 : 특별한 검사 없이 병력 청취와 육안·세극등 소견만으로 대부분 진단 가능
* 기본 확인 항목
  * 시력 (정상 여부 - 단순 SCH에서는 시력에 영향 없음)
  * 동공 크기 및 반응
  * 통증 유무 및 눈부심(photophobia) 여부
  * 안구 운동 시 통증 유무
  * 외상 여부
  * 각막 투명도 (혼탁 여부)
  * 필요 시 형광 염색(fluorescein staining) - 각막 손상 동반 여부 확인
  * 혈압 측정
* 위 항목이 모두 정상이면 추가 영상 또는 검사실 검사는 일반적으로 불필요
...
<table><thead><tr><th width="110">감별 질환</th><th width="155">충혈 양상</th><th width="170">SCH에는 없는 것</th><th>추가 감별 포인트</th></tr></thead><tbody><tr><td><strong>결막염</strong></td><td>전반적 미만성 충혈</td><td>명확한 경계 없음</td><td>가려움·분비물·미만성 충혈이 주로 나타남; 경계가 명확한 출혈반과는 다름</td></tr><tr><td><strong>상공막염</strong><br><strong>/공막염</strong></td><td>국소 자색(violet) 충혈</td><td>통증 없음</td><td>국소 압통; 공막염은 심한 안구통, 안구 운동 시 악화</td></tr><tr><td><strong>급성 폐쇄각</strong><br><strong>녹내장</strong></td><td>모양 충혈(ciliary flush)</td><td>두통·구토 없음, 시력 저하 없음</td><td>심한 안구통·두통·오심/구토, 각막부종, 안압 상승, 동공 이상(중등도 산대·반응 저하)이 흔히 동반</td></tr><tr><td><strong>각막염</strong><br><strong>/포도막염</strong></td><td>모양 충혈(ciliary flush)</td><td>눈부심 없음, 시력 저하 없음</td><td>눈부심·통증·시력 저하가 핵심 감별점; 각막 혼탁 또는 전방 염증 동반 가능</td></tr><tr><td><strong>결막하 종양</strong><br><strong>/혈관종</strong></td><td>국소 융기성 병변</td><td>-</td><td>재발성; 경계 불규칙·융기; 경과 관찰에 무반응</td></tr></tbody></table>

***

```mermaid
graph TD
    Start([급성 편측성 안구충혈 평가]) --> CheckSymptoms{통증 / 시력 저하 /<br/>광과민?}
    
    CheckSymptoms -- YES --> Serious[중증 안구 질환 의심]
    Serious --> Refer1[안과 의뢰]
    
    CheckSymptoms -- NO --> CheckTrauma{외상 여부?}
    
    CheckTrauma -- YES --> CheckSevere{중등도 이상 외상 /<br/>시력저하·전방출혈·<br/>360° 출혈 등 동반?}
    
    CheckSevere -- YES --> Refer2[즉시 안과 의뢰<br/>occult globe injury 배제]
    CheckSevere -- NO --> Observe1[경과 관찰]
    
    CheckTrauma -- NO --> CheckAppearance{경계 명확한<br/>평평한 출혈반?}
    
    CheckAppearance -- YES --> Action[안심시키고 경과 관찰]
    CheckAppearance -- NO --> Other[다른 원인 평가<br/>예, 결막염]

    style Refer1 fill:#f96,stroke:#e65100,stroke-width:2px
    style Refer2 fill:#f96,stroke:#e65100,stroke-width:2px
    style Action fill:#e1f5fe,stroke:#01579b,stroke-width:2px
    style Start fill:#f3e5f5,stroke:#4a148c,stroke-width:2px
```

<p align="center"><strong>결막하출혈 감별 및 관리 알고리듬</strong></p>

<p align="center"><em><mark style="color:$info;">Ref. EyeWiki (AAO), Subconjunctival Hemorrhage; StatPearls (NCBI Bookshelf)</mark></em></p>
...
* 이물감·자극감 호소 시 인공눈물 점안으로 증상 완화
* 냉찜질은 초기 불편감 완화에 사용할 수 있으나, 출혈 흡수를 촉진한다는 근거는 부족함; 급성기 온찜질은 권장하지 않음
* 원인에 따른 조치
  * 고혈압 → SCH와 직접 인과보다 연관성이 높음; 혈압 측정 및 조절 권장
  * 항응고제 복용 → Warfarin: INR 확인 및 용량 적절성 협의 / DOAC: INR은 치료 모니터링 지표로 사용하지 않으며 복용 이행·용량·신기능 확인; 처방 의사와 협의; 환자가 임의로 중단하지 않도록 주의
  * 출혈 질환 의심 → 의뢰
...
S05.0 이물이 없는 결막찰과상 및 각막찰과상 (하위 세부코드는 좌/우/미상 구분) — 외상성 결막 손상이 실제로 확인된 경우에 한해 적용; 정확한 세부코드는 KCD-8 코드집에서 확인 필요

***

## <mark style="color:purple;">처방례</mark>

> **처방례 1. 자발성 결막하출혈 (이물감·자극감 동반 시)**
>
> ```
> 히알루 미니 점안액(무방부제) 0.5 ㎖×30개/box  1방울 필요 시 (prn)
> ```
>
...
> * 점안 후 수 초 동안 눈을 살며시 감아 약물이 눈 표면에 고르게 퍼지도록 하십시오.
> * 단회용(일회용) 제품은 개봉 후 즉시 사용하고 남은 양은 버리십시오.
> * 다회용 제품은 제품 설명서에 표시된 개봉 후 사용기간을 준수하십시오.

> **경과 및 주의사항 안내**
>
> * 결막하출혈은 눈 속의 작은 혈관이 터진 것으로, 전형적으로 **통증·분비물·시력 저하가 없습니다.**
> * 처음 며칠간 출혈 범위가 조금 더 넓어지거나 색이 짙어질 수 있으나 이는 정상 경과입니다.
> * 출혈은 크기에 따라 **1\~3주 내에 저절로 흡수**됩니다 (개인차 있음). 빨간색 → 짙은 빨간색 → 황록색으로 변해가며 사라집니다.
> * 눈을 비비거나 과도하게 만지지 마십시오. 출혈이 악화될 수 있습니다.
> * 항응고제(와파린, 아픽사반·리바록사반 등 DOAC, 아스피린·클로피도그렐 등)를 복용 중인 경우 담당 의사에게 알려 주십시오. **결막하출혈이 생겼다고 해서 항응고제를 임의로 중단해서는 안 됩니다.** (와파린 복용 중이라면 INR 수치 확인이 필요할 수 있으며, DOAC는 INR로 약효를 모니터링하지 않으므로 복용 이행·용량·신기능을 의사가 확인합니다)
> * 오메가-3 고용량, 비타민 E, 은행잎 추출물(징코) 등 건강기능식품을 드시고 있다면 의사에게 알려 주십시오. 이러한 제품도 출혈 경향에 영향을 줄 수 있습니다.

> **언제 다시 병원을 방문해야 하나요?**
>
> * 출혈과 함께 **눈의 통증** 또는 **시력 저하**가 동반되는 경우 - 즉시 내원
> * 소량 출혈이 **1주, 대량 출혈이 3주가 지나도 흡수되지 않는** 경우 (개인차로 다소 늦어질 수 있으나 지속 시 평가 필요)
> * 결막하출혈이 **반복적으로 재발**하는 경우 (재발 간격이 짧거나 다른 출혈 소견이 동반되면 검사가 필요할 수 있음)
> * **외상 후** 출혈 부위가 결막 전체를 둘러싸거나 결막이 부풀어 오르는 경우 - 즉시 내원
...
* 오메가-3 고용량, 비타민 E, 은행잎 추출물 등 건강기능식품도 출혈 경향에 영향을 줄 수 있습니다.
...
* 소량 출혈은 **1주 이내**, 대량 출혈은 **3주까지** 걸릴 수 있습니다 (개인차가 있을 수 있음).
...
* **외상 후** 결막이 부풀어 오르거나 출혈이 각막(검은 눈동자) 주변 전체를 둘러싸는 경우
* 소량 출혈은 1주, 대량 출혈은 **3주가 지나도 흡수되지 않는** 경우
* 결막하출혈이 **자주 반복되는** 경우, 특히 재발 간격이 짧은 경우
...
※ 위 기간은 대략적인 참고치이며, 나이·전신 질환에 따라 개인차가 있을 수 있습니다.
