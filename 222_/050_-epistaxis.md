...
* 고혈압은 대부분 코피의 직접적인 시작 원인은 아니지만, 혈관 취약성 증가와 지혈 지연에 관여하며 특히 후방 출혈·고령 환자에서 연관성이 큼. 급성 코피 상황에서 혈압 상승은 원인이라기보다 스트레스 반응인 경우도 흔함
...
| 상황                      | 의심 상황                          | 확인 사항                            | 우선 조치           |
| ----------------------- | ------------------------------ | -------------------------------- | --------------- |
| 15분 압박에도 출혈 지속          | Active bleeding 지속 / 후방 출혈     | 압박 위치 정확성, 항응고제 여부, 활력징후         | 즉시 의뢰           |
| 피가 목 뒤로 계속 넘어감          | 후방 출혈 (Woodruff's plexus)      | 삼키는 혈액량, 구토·흑색변, 고령/HTN 여부       | 즉시 의뢰           |
| 창백 + 어지럼 + 빈맥 동반        | Significant blood loss         | BP, HR, 실신 여부, Hb 필요성 판단         | 즉시 의뢰           |
| 전방 패킹에도 출혈 지속           | 후방 출혈 과소 평가                    | 인후 혈류 지속 여부, 활력징후                | 즉시 의뢰           |
| 외상 후 지속 출혈              | 비중격 혈종 / 코뼈 골절                 | 코 변형, 비중격 종창·파동, 코막힘             | 즉시 의뢰           |
| 항응고제 복용 중 반복 출혈         | Anticoagulant-associated bleed | Warfarin vs NOAC 구별, 마지막 복용, INR | INR 확인 / 의뢰     |
| 한쪽 코에서 반복 출혈            | 비강 종양 / 구조적 병변                 | 코막힘, 체중감소, 얼굴 통증, 청소년 남아 여부      | 의뢰, 내시경 고려      |
| 쉽게 멍 + 코피 반복 / 양쪽 동시 출혈 | 혈소판 감소증 / VWD                  | 발치 후 출혈, 생리과다, 가족력, CBC/PT/aPTT  | 혈액학적 검사 후 평가    |
| 아침마다 반복되는 소량 코피         | 점막 건조 / 보습 부족                  | 겨울철·난방 환경, 가습기 사용 여부             | 식염수 세척 + 바셀린    |
| 코 후비기 직후 출혈             | Anterior septal bleed          | 소아 흔함, 손톱 길이, 습관 여부              | 교육 + 보습 + 소작 고려 |
...
* 혈관수축제를 넣기 전 큰 혈전을 먼저 제거해야 약물이 점막에 직접 닿아 효과가 극대화됨. 환자에게 코를 한 번 부드럽게 풀도록(blow) 안내한 뒤 적용
* **oxymetazoline** 0.05%을 비강 분무 또는 솜에 적셔 비강 내 적용 - 심혈관 기저 질환이 있는 고령 환자에서 1차 선택 <mark style="color:blue;">\[레스피비엔, 오트리빈]</mark>
...
* **방법** : TXA 500 ㎎/5 ㎖ 앰플을 거즈 또는 솜에 적셔 출혈 부위에 10\~15분간 삽입
  * NoPAC trial 등에서 packed gauze 대비 우월성은 확인되지 않았으나, 비침습적이고 항응고제 복용자·패킹 회피를 원하는 환자에서 여전히 유용한 옵션으로 간주됨
...
> ※ **양측 비중격 동시 소작 절대 금기** : 비중격은 양측 점막에서 혈액 공급을 받음. 양측 대칭 부위를 동시에 소작하면 비중격 혈관 공급이 양측에서 차단되어 허혈성 괴사 및 비중격 천공이 발생할 수 있음. 양측 출혈 시 한 번에 한쪽만 시행하고, 반대쪽은 최소 4\~6주 후 시행
...
graph TD
    ...
    B -- "안정" --> D["1단계: 앞으로 숙이고 <br/>15분간 강력 압박<br/>(코 연골 연부 부위)"]
    D --> E["지혈 되었는가?"]
    ...
    I --> J2["지혈 확인?"]
    J --> J2
    J2 -- "Yes" --> K(["패킹 48시간 유지<br/>고위험군 항생제 개별 판단"])
    J2 -- "No\n(후방 출혈 의심)" --> C
...
> ```
>
> _✽근거 불충분하여 일률 투여는 권고되지 않음. 면역 저하자, 48시간 이상 패킹 유지, 당뇨 등 고위험군에서 개별 판단. 패킹 유지 기간에만 투여하고 제거 즉시 중단. ✽375 ㎎ 제형 사용 시에는 2T #3(1일 3회)으로 처방_
