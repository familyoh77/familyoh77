...
{% hint style="danger" %}
🔋 **원판(단추형) 배터리 — 최우선 응급 | 1시간 이내 제거**

전기분해로 강알칼리성 물질이 누출되어 **수 시간 내 심각한 액화 괴사(liquefaction necrosis)·천공** 발생 가능.

* ⛔ **절대 세척(irrigation) 금지** — 습기가 닿으면 전기화학 반응이 가속화됨
* ⛔ **관찰·대기 금지** — 증상이 없어도 즉각 대응
* ✅ **발견 즉시 응급실로** — 이상적으로 **1시간 이내** 제거
{% endhint %}
...
```mermaid
graph TD
    Start["외이도 이물 확인"] --> RedFlags{"즉시 의뢰\n상황?"}

    RedFlags -- "Yes" --> Refer["🏥 이비인후과/응급실\n즉시 의뢰"]
    RedFlags -- "No" --> FBType{"이물의 종류"}

    Refer --- ReferBox["🔋 원판 배터리 — 1시간 내 응급 제거\n고막 손상·천공 의심\n심한 통증·출혈·외이도 부종\n이전 제거 시도 실패 또는 비협조 소아\n날카로운 이물 (바늘·유리 조각)"]
    ...
    style ReferBox fill:#fff8f8,stroke:#ffcccc,text-align:left
```
...
* 고막 천공이 발생한 경우 (☞ [외상성 고막 천공](049_-ear-injury.md#외상성-고막-천공-traumatic-tm-perforation))
...
표준 청력 검사(순음청력검사)가 정상으로 나왔더라도, 시끄러운 환경에서 말소리를 듣기 어렵거나 대화 내용을 자주 되묻는다면 '숨겨진 난청(청각 시냅스병증)'일 수 있습니다. 이 경우 정밀 청력 검사가 필요하므로 이비인후과에 알리십시오.
{% endhint %}
...
{% hint style="danger" %}
| 상황 | 골든타임 | 지연 시 결과 | 핵심 주의 |
|---|---|---|---|
| 🔋 **원판(단추형) 배터리** | **1시간 이내** | 액화 괴사·조직 천공 | 절대 세척 금지, 즉시 응급실 |
| 🩸 **이개혈종** | **24시간 이내** | 연골 섬유화 → cauliflower ear | 흡인 후 반드시 압박 고정 |
| 💥 **폭발음·급성 음향 외상** | **48시간 이내** | 청력 회복 불가 (스테로이드 창 마감) | 소음 환경 즉시 이탈 |
| 👂 **외상성 고막 천공** | **24시간 이내** | 2차 세균 감염·중이염 | 건조 유지, 귀 후비기 금지 |
{% endhint %}

> ✽이 표는 Quick Reference 또는 챕터 앞머리 배치에 적합합니다.
