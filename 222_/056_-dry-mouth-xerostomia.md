...
* 다른 이름 : 구강건조증, 침샘기능저하증(hyposalivation)
* **Xerostomia vs Hyposalivation 임상 구분**

  | | Xerostomia | Hyposalivation |
  |---|---|---|
  | 정의 | 주관적 건조감 (증상) | 객관적 타액 분비 감소 (징후) |
  | 측정 | 문진, 설문 | 타액 분비량 측정(sialometry) |
  | 기준 | — | 비자극 타액 < 0.1 ㎖/분; 자극 타액 < 0.7 ㎖/분 |

  * 둘은 불일치하는 경우가 흔함 — 임상적으로 중요한 이유:
    * Xerostomia + 정상 분비량 → 심인성(불안, 공황), 구강 호흡, burning mouth 고려
    * Hyposalivation + 건조감 없음 → 고령, 신경병증, 감각 둔화
* 침의 기능 : 구강 점막 보호, 소화 효소(amylase, lipase) 함유, 항균 작용(lactoferrin, lysozyme, **sIgA**, lactoperoxidase), 완충 작용, 발음 보조
* 유병률 : 성인의 약 10~46%; 노인 및 다약제 복용자에서 높음
* 합병증 : 삼킴곤란, 수면 장애, 구취, 충치(특히 치경부 우식증), 치주 질환, 침샘 결석, 구강 칸디다증, 미각 장애, 영양 섭취 저하

{% hint style="danger" %}
⚠️ 침의 항균 작용을 담당하는 주요 면역글로불린은 **sIgA (분비형 면역글로불린 A)**입니다. IgE는 알레르기 반응 관련 항체로 침에는 소량만 포함됩니다.
{% endhint %}
...
#### <mark style="color:$primary;">약물 (가장 흔한 원인)</mark>
...
* 기분안정제 : **리튬(Lithium)** — 직접 침샘 영향; 항부정맥제 **disopyramide** — 강한 항콜린 작용
