...
* 심한 피로가 만성적으로 지속되면서 객관적 염증성 질환 근거가 부족한 경우 → [만성피로증후군](../230_/194_-cfs.md), [섬유근육통](153_-fibromyalgia.md) 등 감별
...
```mermaid
flowchart TD
    A(["관절통·근육통·전신 증상"]) --> B{"Red Flag 존재?"}
    B -->|예| C["즉시/당일 평가·의뢰<br>시각 증상+GCA 의심 · 객혈+혈뇨/AKI<br>급성 호흡곤란·흉통 · 중증 고혈압+AKI<br>면역억제 중 중증 감염 의심"]
    B -->|아니오| D{"Inflammatory pattern?"}
    D -->|아니오| E["비염증성 원인 우선 평가<br>퇴행성·기계적·과사용·섬유근육통<br>내분비·감염·약물 등 감별"]
    D -->|예/불확실| F{"주된 phenotype"}
    F --> G["관절염 phenotype<br>관절 종창·열감·조조강직"]
    F --> H["CTD phenotype<br>발진·구강궤양·Raynaud·sicca·피부경화"]
    F --> I["근육 phenotype<br>근위부 근력저하 ± 발진"]
    F --> J["혈관염 phenotype<br>자반·신경병증·신장/폐 침범"]
    F --> K["PMR/GCA phenotype<br>≥50세, 어깨·골반대 강직<br>새 두통·턱 파행·시각 증상"]
    F --> L["전신 염증 phenotype<br>반복 고열·발진·관절통(AOSD 등)"]
    G --> M["기본검사 + RF/anti-CCP"]
    H --> N["기본검사 + ANA 우선<br>선택: anti-dsDNA, C3/C4, anti-SSA/Ro"]
    I --> O["CK ± aldolase, AST/ALT, LDH<br>선택: myositis-specific Ab"]
    J --> P["CBC, ESR/CRP, Cr/eGFR, UA<br>± ANCA, C3/C4"]
    K --> Q["ESR/CRP, CBC<br>시각 증상 시 즉시 치료 + 전문 평가"]
    L --> R["CBC, ESR/CRP, ferritin 등<br>감염·악성종양 우선 배제"]
    M --> S{"지속성 염증성 질환 의심?"}
    N --> S
    O --> S
    P --> S
    Q --> S
    R --> S
    S -->|예| T["류마티스내과 조기 의뢰<br>객관적 활막염은 6주를 기다리지 않음"]
    S -->|아니오/불확실| U["임상 경과 추적<br>새 Red Flag 발생 시 즉시 재분류"]

    style B fill:#fff3e0,stroke:#e65100,stroke-width:2px
    style C fill:#ffebee,stroke:#c62828,stroke-width:2px
    style D fill:#fff3e0,stroke:#e65100,stroke-width:2px
    style S fill:#fff3e0,stroke:#e65100,stroke-width:2px
    style T fill:#f96,stroke:#e65100,stroke-width:2px
    style E fill:#f5f5f5,stroke:#757575,stroke-width:1px
    style U fill:#f5f5f5,stroke:#757575,stroke-width:1px
```

<p align="center"><strong>류마티스 질환 일차진료 Triage 알고리듬</strong></p>

<p align="center"><em><mark style="color:$info;">확진이 아닌 진입 경로(phenotype) 결정을 위한 알고리듬이며, 세부 질환 감별은 위 증상·징후 기반 표를 참고</mark></em></p>
