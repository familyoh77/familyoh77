...
```mermaid
flowchart LR
    A["≥3개월 지속되는<br>광범위 통증 + 피로/수면장애"] --> B{"Red flag 또는<br>객관적 이상 소견?"}
    B -->|"예"| C["염증성 관절염·PMR·myositis·<br>신경학적 질환·내분비/대사질환 등<br>의심 원인 우선 평가"]
    B -->|"아니오"| D["필요한 범위의 최소 검사<br>CBC, ESR/CRP, TSH ± CK 등"]
    D --> E["ACR 2016 기준 확인<br>WPI/SSS + generalized pain<br>+ ≥3개월"]
    E --> F{"진단기준 충족?"}
    F -->|"아니오"| G["다른 원인 재평가 / 추적"]
    F -->|"예"| H["섬유근육통 진단<br>다른 질환과 동반 가능"]
    H --> I["기본 치료<br>질환 교육 + 규칙적 운동<br>+ 수면·생활습관 관리"]
    I --> J{"반응 충분?"}
    J -->|"예"| K["유지 + 주기적 재평가"]
    J -->|"아니오"| L["phenotype별 추가 치료"]
    L --> M1["우울/불안·비적응적 대처<br>→ CBT ± 항우울제"]
    L --> M2["심한 통증/수면장애<br>→ duloxetine·pregabalin·<br>저용량 amitriptyline 등 개별화"]
    L --> M3["심한 기능장애<br>→ 재활치료/다학제 접근"]
```
