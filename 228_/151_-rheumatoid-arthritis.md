...
```mermaid
graph TD
    A(["RA의 임상적 진단 1)"]) --> B{"MTX에 대한 금기?"}
    B -->|금기 없음| C["MTX 시작2 + 단기 Glucocorticoid 병용"]
    B -->|금기 있음| D["leflunomide 또는 sulfasalazine + 단기 Glucocorticoid"]
    C --> E{3개월 내 호전 &\n6개월 내 목표3 달성?}
    D --> E
    E -->|Yes| F[치료 지속]
    F --> G[완화 지속4 시 GC 우선 감량 → DMARD 감량 고려]
    E -->|No| H["csDMARD 전략 실패\n→ bDMARD 추가5\n(JAK 억제제도 위험 평가 후 고려 가능)6"]
    H --> I{3개월 내 호전 &\n6개월 내 목표 달성?}
    I -->|Yes| J[치료 지속]
    J --> K[완화 지속 시 감량/투여 간격 연장]
    I -->|No| L["다른 bDMARD 또는 tsDMARD로 교체7\n(동일 또는 다른 기전)"]
    L --> I
