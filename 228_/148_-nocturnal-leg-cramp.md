...
```mermaid
graph TD
    A["야간 하지 통증/경련 호소"] --> B["병력 청취·신체검사<br/>약물 및 혈관/신경 증상 확인"]
    B --> C{"전형적 NLC?<br/>(통증성 근육 경직,<br/>수초~수분, 스트레칭으로 완화)"}
    C -->|No/비전형적| E["감별진단 평가<br/>(RLS, PAD, neuropathy,<br/>PLMD, myositis 등)"]
    C -->|Yes| D{"Red Flags?"}
    D -->|Yes| F["위험도에 따라<br/>즉각 또는 조기 평가/의뢰"]
    D -->|No| G["특발성 NLC 우선 진단<br/>routine 검사는 불필요"]
    G --> H["비-약물 치료<br/>스트레칭·생활 습관·유발약 검토"]
    H --> I{"4~6주 후<br/>충분히 호전?"}
    I -->|Yes| J["유지·재발 시 재교육"]
    I -->|No| K["이차 원인 재평가<br/>증상 부담·수면장애 확인"]
    K --> L{"빈번·중증이고<br/>생활/수면 영향 큼?"}
    L -->|No| J
    L -->|Yes| M["근거 제한 설명 후<br/>선별적 off-label 약물 trial 고려"]
    M --> N["반응·부작용 재평가<br/>무효 시 중단/전문과 의뢰"]
    E --> O["해당 질환별 평가·치료"]
    ...
    style F fill:#fdebed,stroke:#ffcdd2,stroke-width:2px
    style G fill:#fff3e0,stroke:#e65100,stroke-width:1px
    style H fill:#e1f5fe,stroke:#01579b,stroke-width:1px
    style I fill:#fef9e7,stroke:#fff59d,stroke-width:1px
    style J fill:#e1f5fe,stroke:#01579b,stroke-width:1px
    style K fill:#fef9e7,stroke:#fff59d,stroke-width:1px
    style L fill:#fef9e7,stroke:#fff59d,stroke-width:1px
    style M fill:#e1f5fe,stroke:#01579b,stroke-width:1px
    style N fill:#fdebed,stroke:#ffcdd2,stroke-width:1px
    style O fill:#f3e5f5,stroke:#4a148c,stroke-width:1px
```
